...
void ResourceLocation::generate(std::optional<std::string> resourceloc, std::string resourceName)
{
	...
	{		
		std::transform(resourceloc.value.begin(), resourceloc.value.end(), resourceloc.value.begin(), [&](const char c) {
	...
	std::transform(resourceName.begin(), resourceName.end(), resourceName.begin(), [&](const char c) {
		return std::tolower(c, loc); });
	path = resourceName;
}
