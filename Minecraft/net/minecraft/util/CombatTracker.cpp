...
CombatTracker::CombatTracker(EntityLivingBase *fighterIn) : fighter(fighterIn)
{
}
...
void CombatTracker::trackDamage(const DamageSource::DamageSource &damageSrc, const float healthIn,
        ...
        if (combatentry1->getDamageSrc() == DamageSource::FALL)
        {
                ...
                combatentry->getDamageSrc() != DamageSource::OUT_OF_WORLD)
            {
                if (itextcomponent2 == nullptr || itextcomponent1 != nullptr && itextcomponent2 == itextcomponent1)
                {
                    ...
                    {
                        auto itemstack = Util:: instanceof
                            <EntityLivingBase>(entity)
                                ? reinterpret_cast<EntityLivingBase *>(entity)->getHeldItemMainhand()
                                : ItemStack::EMPTY;
                        if (!itemstack.isEmpty() && itemstack.hasDisplayName())
                        {
                            itextcomponent = std::make_unique<TextComponentTranslation>(
                                "death.fell.finish.item", fighter->getDisplayName(), itextcomponent1,
                                itemstack.getTextComponent());
                        }
                        else
                        {
                            itextcomponent = std::make_unique<TextComponentTranslation>(
                                "death.fell.finish", fighter->getDisplayName(), itextcomponent1);
                        }
                    }
                    else
                    {
                        itextcomponent =
                            std::make_unique<TextComponentTranslation>("death.fell.killer", fighter->getDisplayName());
                    }
                }
                else
                {
                    ...
                    ItemStack itemstack1 = Util:: instanceof
                        <EntityLivingBase>(entity1)
                            ? reinterpret_cast<EntityLivingBase *>(entity1)->getHeldItemMainhand()
                            : ItemStack::EMPTY;
                    if (!itemstack1.isEmpty() && itemstack1.hasDisplayName())
                    {
                        itextcomponent = std::make_unique<TextComponentTranslation>(
                            "death.fell.assist.item", fighter->getDisplayName(), itextcomponent2,
                            itemstack1.getTextComponent());
                    }
                    else
                    {
                        itextcomponent = std::make_unique<TextComponentTranslation>(
                            "death.fell.assist", fighter->getDisplayName(), itextcomponent2);
                    }
                }
            }
            else
            {
                itextcomponent = std::make_unique<TextComponentTranslation>(
                    "death.fell.accident." + getFallSuffix(combatentry), fighter->getDisplayName());
            }
        }
        else
        {
    ...
    while (true)
    {
        ...
        do
        {
            do
            {
                ...
                combatentry = (*ite).get();
                if (Util:: instanceof <EntityPlayer>(combatentry->getDamageSrc().getTrueSource()) &&
                                          (entityplayer == nullptr || combatentry->getDamage() > f1))
                {
...
CombatEntry *CombatTracker::getBestCombatEntry()
{
    ...
    for (size_t i = 0; i < combatEntries.size(); ++i)
    {
        ...
        if ((combatentry2->getDamageSrc() == DamageSource::FALL ||
             combatentry2->getDamageSrc() == DamageSource::OUT_OF_WORLD) &&
            combatentry2->getDamageAmount() > 0.0F && (combatentry == nullptr || combatentry2->getDamageAmount() > f1))
        {
            if (i > 0)
            {
        ...
        if (!combatentry2->getFallSuffix().empty() && (combatentry1 == nullptr || combatentry2->getDamage() > f))
        {
