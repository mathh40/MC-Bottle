...
template <class T> std::optional<T> runTask(FutureTask task, spdlog::logger &logger);

int64_t getcurrent_time();

template <typename Base, typename T> bool instanceof (const T *a)
{
    return std::is_base_of<Base, T>::value;
}

template <typename Collection1, typename Collection2> void removeAll(Collection1 &c1, const Collection2 &c2)
{
    for (auto &&i : c2)
...
template <typename T, typename G> void KeySet(T &KeySet, G maps)
{
    for (auto &entry : maps)
    {
...
template <typename T, typename G> void Collection(T &Collection, G maps)
{
    for (auto &entry : maps)
    {
        KeySet.emplace(entry.first);
...
template <typename T, typename G> void erase(T &v, G predig)
{
    v.erase(std::remove(v.begin(), v.end(), predig), v.end());
}

template <typename T, typename G> void erase_if(T &v, G predig)
{
