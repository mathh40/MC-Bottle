...
WeightedSpawnerEntity::WeightedSpawnerEntity() : WeightedItem(1)
{
    nbt->setString("id", "minecraft:pig");
}

WeightedSpawnerEntity::WeightedSpawnerEntity(std::shared_ptr<NBTTagCompound> nbtIn)
    : WeightedSpawnerEntity(nbtIn->hasKey("Weight", 99) ? nbtIn->getInteger("Weight") : 1,
                            nbtIn->getCompoundTag("Entity"))
{
}
