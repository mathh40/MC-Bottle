...
std::shared_ptr<ITextComponent> DamageSource::EntityDamageSource::getDeathMessage(EntityLivingBase *entityLivingBaseIn)
{
    auto itemstack = Util:: instanceof
        <EntityLivingBase *>(damageSourceEntity)
            ? reinterpret_cast<EntityLivingBase *>(damageSourceEntity)->getHeldItemMainhand()
            : ItemStack::EMPTY;
    auto s  = "death.attack." + damageType;
    auto s1 = s + ".item";
    return !itemstack.isEmpty() && itemstack.hasDisplayName() && I18n::canTranslate(s1)
               ? std::make_shared<TextComponentTranslation>(s1, entityLivingBaseIn->getDisplayName(),
                                                            damageSourceEntity->getDisplayName(),
                                                            itemstack.getTextComponent())
               : new TextComponentTranslation(
                     s, new Object[]{entityLivingBaseIn.getDisplayName(), damageSourceEntity.getDisplayName()});
}

bool DamageSource::EntityDamageSource::isDifficultyScaled()
{
    return damageSourceEntity.has_value() && instanceof
        <EntityLivingBase>(damageSourceEntity.value()) && !(instanceof <EntityPlayer>(damageSourceEntity.value()));
}
