...
{
DamageSource::DamageSource(std::string_view damageTypeIn) : damageType(damageTypeIn)
{
}
...
{
    return indirectEntityIn == nullptr
               ? std::make_unique<EntityDamageSourceIndirect>("onFire", fireball, fireball)
                     ->setFireDamage()
                     .setProjectile()
               : std::make_unique<EntityDamageSourceIndirect>("fireball", fireball, indirectEntityIn)
                     ->setFireDamage()
                     .setProjectile();
}
...
{
    return std::make_unique<EntityDamageSourceIndirect>("indirectMagic", source, indirectEntityIn)
...
{
    return explosionIn.has_value() && explosionIn->getExplosivePlacedBy() != nullptr
               ? std::make_unique<EntityDamageSource>("explosion.player", explosionIn->getExplosivePlacedBy())
                     ->setDifficultyScaled()
                     .setExplosion()
               : DamageSource("explosion").setDifficultyScaled().setExplosion();
}
DamageSource &DamageSource::causeExplosionDamage(EntityLivingBase *entityLivingBaseIn)
{
    entityLivingBaseIn->has_value() ? std::make_unique<EntityDamageSource>("explosion.player", entityLivingBaseIn)
                                          ->setDifficultyScaled()
                                          .setExplosion()
                                    : DamageSource("explosion").setDifficultyScaled().setExplosion();
}
...
std::shared_ptr<ITextComponent> DamageSource::getDeathMessage(EntityLivingBase *entityLivingBaseIn)
{
    ...
    return I18n::canTranslate(s1) ? new TextComponentTranslation(s1, new Object[]{entityLivingBaseIn->getDisplayName(),
