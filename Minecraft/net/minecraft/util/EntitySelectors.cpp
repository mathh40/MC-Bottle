...
{
    return !(Util:: instanceof <EntityPlayer>(lhs)) ||
           !((EntityPlayer *)lhs)->isSpectator() && !((EntityPlayer *)lhs)->isCreative();
}
...
std::function<bool(const Entity *)> EntitySelectors::getTeamCollisionPredicate(const Entity *entityIn)
{
    ...
    const Team::CollisionRule team$collisionrule =
        team == nullptr ? Team::CollisionRule::ALWAYS : team->getCollisionRule();
    auto ret = team$collisionrule == Team::CollisionRule::NEVER
                   ? [](const Entity *lhs) { return false; }
                   : [&](const Entity *lhs) {
                         if (!lhs->canBePushed())
                         {
                             return false;
                                  ...
                                  (!(Util:: instanceof <EntityPlayer>(lhs)) || !((EntityPlayer *)lhs)->isUser()))
                         {
                             return false;
                         }
                         else
                         {
                             ...
                             Team::CollisionRule team$collisionrule1 =
                                 team1 == nullptr ? Team::CollisionRule::ALWAYS : team1->getCollisionRule();
                             ...
                             else
                             {
                                 ...
                                 if ((team$collisionrule == Team::CollisionRule::HIDE_FOR_OWN_TEAM ||
                                      team$collisionrule1 == Team::CollisionRule::HIDE_FOR_OWN_TEAM) &&
                                     flag)
                                 {
                                     return false;
                                 }
                                 else
                                 {
                                     return team$collisionrule != Team::CollisionRule::HIDE_FOR_OTHER_TEAMS &&
                                                team$collisionrule1 != Team::CollisionRule::HIDE_FOR_OTHER_TEAMS ||
                                            flag;
                                 }
                             }
                         }
