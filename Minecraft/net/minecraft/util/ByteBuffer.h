...
class ByteBuffer
{
  public:
    ...
    std::unique_ptr<ByteBuffer> clone()
        const; // Return a new instance of a ByteBuffer with the exact same contents and the same state (rpos, wpos)
    ...
    std::byte peek() const; // Relative peek. Reads and returns the next std::byte in the buffer from the current
                            // position but does not increment the read position
    std::byte get()
        const; // Relative get method. Reads the std::byte at the buffers current position then increments the position
