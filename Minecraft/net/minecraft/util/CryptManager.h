...
class CryptManager
{
  public:
    ...
    std::array<unsigned char, SHA_DIGEST_LENGTH> getServerIdHash(std::string serverId, EVP_PKEY *publicKey,
                                                                 std::array<unsigned char, (128 / 8)> secretKey);
    template <typename T, class... Args>
    std::vector<unsigned char> digestOperation(std::string algorithm, T data, Args... datas);
    template <typename T> std::vector<unsigned char> digestOperation(std::string algorithm, T data);
    ...
    std::vector<unsigned char> encryptDataAES(std::array<unsigned char, (128 / 8)> key,
                                              std::vector<unsigned char> data);
    std::vector<unsigned char> decryptDataRSA(EVP_PKEY *key, std::vector<unsigned char> data);
    std::vector<unsigned char> decryptDataAES(std::array<unsigned char, (128 / 8)> key,
                                              std::vector<unsigned char> data);
...
template <typename T> inline std::vector<unsigned char> CryptManager::digestOperation(std::string algorithm, T data)
{
