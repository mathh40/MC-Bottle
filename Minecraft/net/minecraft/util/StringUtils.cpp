...
int StringUtils::indexOfDifference(std::vector<std::string> css)
{
	...
	int arrayLen = css.size();
	int shortestStrLen = std::numeric_limits<int>::max();
	int longestStrLen = 0;
	...
	for (auto cs : css) {
		if (cs.empty()) {
			...
			shortestStrLen = std::min(cs.length(), shortestStrLen);
			longestStrLen = std::max(cs.length(), longestStrLen);
...
std::string StringUtils::replace_all(std::string str, std::string_view& from, std::string_view& to)
{
	size_t start_pos = 0;
