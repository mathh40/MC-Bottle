...
BlockPos::BlockPos(int32_t x, int32_t y, int32_t z) : Vec3i(x, y, z)
{
}

BlockPos::BlockPos(double x, double y, double z) : Vec3i(x, y, z)
{
}

BlockPos::BlockPos(const Entity *source) : Vec3i(source->posX, source->posY, source->posZ)
{
}

BlockPos::BlockPos(const Vec3d &vec) : Vec3i(vec.getx(), vec.gety(), vec.getz())
{
}

BlockPos::BlockPos(Vec3i source) : Vec3i(source.getx(), source.gety(), source.getz())
{
}

BlockPos::BlockPos(uint32_t i2, uint32_t j4, uint32_t k2) : Vec3i(x, y, z)
{
}
...
BlockPos BlockPos::add(double x, double y, double z) const
{
    return x == 0.0 && y == 0.0 && z == 0.0 ? *this
                                            : BlockPos((double)getx() + x, (double)gety() + y, (double)getz() + z);
}
...
{
    return n == 0 ? *this
                  : BlockPos(getx() + facing.getXOffset() * n, gety() + facing.getYOffset() * n,
...
{
    return BlockPos(gety() * vec.getz() - getz() * vec.gety(), getz() * vec.getx() - getx() * vec.getz(),
...
{
    return getAllInBoxMutable(MathHelper::min(from.getx(), to.getx()), MathHelper::min(from.gety(), to.gety()),
                              MathHelper::min(from.getz(), to.getz()), MathHelper::max(from.getx(), to.getx()),
                              MathHelper::max(from.gety(), to.gety()), MathHelper::max(from.getz(), to.getz()));
}
