...
AxisAlignedBB::AxisAlignedBB(double x1, double y1, double z1, double x2, double y2, double z2)
    : minX(std::min(x1, x2)), minY(std::min(y1, y2)), minZ(std::min(z1, z2)), maxX(std::max(x1, x2)),
      maxY(std::max(y1, y2)), maxZ(std::max(z1, z2))
{
}
...
AxisAlignedBB::AxisAlignedBB(Vec3d min, Vec3d max) : AxisAlignedBB(min.x, min.y, min.z, max.x, max.y, max.z)
{
}

bool operator==(const AxisAlignedBB &a, const AxisAlignedBB &b)
{
    return b.minX == a.minX && b.minY == a.minY && b.minZ == a.minZ && b.maxX == a.maxX && b.maxY == a.maxY &&
           b.maxZ == a.maxZ;
}
...
{
    return AxisAlignedBB(minX + (double)pos.getX(), minY + (double)pos.getY(), minZ + (double)pos.getZ(),
                         maxX + (double)pos.getX(), maxY + (double)pos.getY(), maxZ + (double)pos.getZ());
}
...
{
    return std::isnan(minX) || std::isnan(minY) || std::isnan(minZ) || std::isnan(maxX) || std::isnan(maxY) ||
           std::isnan(maxZ);
}
...
{
    return intersects(std::min(min.x, max.x), std::min(min.y, max.y), std::min(min.z, max.z), std::max(min.x, max.x),
                      std::max(min.y, max.y), std::max(min.z, max.z));
}
