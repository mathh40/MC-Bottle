...
constexpr uint32_t MULTIPLY_DE_BRUIJN_BIT_POSITION[] = {0,  1,  28, 2,  29, 14, 24, 3, 30, 22, 20, 15, 25, 17, 4,  8,
...
template <typename T> T DifferenceOfProducts(T a, T b, T c, T d)
{
    T cd  = c * d;
...
template <class T> T sqrt(T value)
{
    return sqrt(value);
}
...
template <class T> T round(T value)
{
...
template <class T> constexpr T clamp(T num, T min, T max)
{
    if (num < min)
    {
        return min;
...
template <typename T> T getInt(pcg32 &random, T minimum, T maximum)
{
...
template <class T> T pow(T x, T x1)
{
    return std::pow(x, x1);
}

template <class T> T fmod(T x, T x1)
{
