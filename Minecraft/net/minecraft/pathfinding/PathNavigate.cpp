...
PathNavigate::PathNavigate(EntityLiving *entityIn, World *worldIn)
  :lastPosCheck(Vec3d::ZERO),timeoutCachedNode(Vec3d::ZERO),maxDistanceToWaypoint(0.5)
,entity(entityIn),world(worldIn),pathSearchRange(entityIn->getEntityAttribute(SharedMonsterAttributes::FOLLOW_RANGE)),
...
void PathNavigate::onUpdateNavigation()
{
  ...
  if (!noPath()) 
  {
    ...
    else if (currentPath.has_value() && currentPath->getCurrentPathIndex() < currentPath->getCurrentPathLength()) 
    {
      ...
      if (vec3d2.gety() > vec3d1.gety() && !entity->onGround && MathHelper::floor(vec3d2.getx()) == MathHelper::floor(vec3d1.getx()) && MathHelper::floor(vec3d2.getz()) == MathHelper::floor(vec3d1.getz())) 
      {
...
void PathNavigate::pathFollow()
{
  ...
  if (MathHelper::abs((entity->posX - (vec3d1.getx() + 0.5))) < maxDistanceToWaypoint && MathHelper::abs((entity->posZ - (vec3d1.getz() + 0.5))) < maxDistanceToWaypoint && MathHelper::abs(entity->posY - vec3d1.gety()) < 1.0) 
  {
    currentPath->setCurrentPathIndex(currentPath->getCurrentPathIndex() + 1);
  }
