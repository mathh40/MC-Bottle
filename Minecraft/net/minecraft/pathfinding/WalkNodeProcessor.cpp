...
PathPoint WalkNodeProcessor::getStart()
{
    ...
    if (getCanSwim() && entity->isInWater())
    {
        ...
        BlockPos blockpos$mutableblockpos =
            BlockPos(MathHelper::floor(entity->posX), i, MathHelper::floor(entity->posZ));
    ...
    {
        for (blockpos = BlockPos(entity);
             (blockaccess->getBlockState(blockpos)->getMaterial() == Material::AIR ||
                                           ...
                                           PathPoint targetPoint, float maxDistance)
{
    ...
    double d0         = (double)currentPoint.y -
                (1.0 - blockaccess->getBlockState(blockpos).getBoundingBox(blockaccess, blockpos).maxY);
...
PathNodeType WalkNodeProcessor::getPathNodeType(IBlockAccess *p_193577_1_, int32_t x, int32_t y, int32_t z,
                                                int32_t xSize, int32_t ySize, int32_t zSize, bool canOpenDoorsIn,
                                                bool canEnterDoorsIn, EnumSet p_193577_10_, PathNodeType p_193577_11_,
...
PathNodeType WalkNodeProcessor::getPathNodeType(IBlockAccess *blockaccessIn, int32_t x, int32_t y, int32_t z)
{
    ...
    if (pathnodetype == PathNodeType::OPEN && y >= 1)
    {
        ...
        pathnodetype               = pathnodetype1 != PathNodeType::WALKABLE && pathnodetype1 != PathNodeType::OPEN &&
                               pathnodetype1 != PathNodeType::WATER && pathnodetype1 != PathNodeType::LAVA
    ...
    if (p_193578_5_ == PathNodeType::WALKABLE)
    {
        for (int32_t i = -1; i <= 1; ++i)
        {
            for (int32_t j = -1; j <= 1; ++j)
            {
                if (i != 0 || j != 0)
                {
                    auto block = p_193578_1_
                                     ->getBlockState(blockpos$pooledmutableblockpos.setPos(i + p_193578_2_, p_193578_3_,
                                                                                           j + p_193578_4_))
                                     .getBlock();
                                                   ...
                                                   int32_t p_189553_4_)
{
    ...
    else if (block != Blocks::TRAPDOOR && block != Blocks::IRON_TRAPDOOR && block != Blocks::WATERLILY)
    {
        ...
        else if (Util:: instanceof <BlockDoor>(block) && material == Material::WOOD &&
                                       !iblockstate->getValue(BlockDoor::OPEN))
        {
        ...
        else if (!(instanceof <BlockFence>(block)) && !(instanceof <BlockWall>(block)) &&
                 (!(instanceof <BlockFenceGate>(block)) || iblockstate->getValue(BlockFenceGate::OPEN)))
        {
        ...
        else
        {
                ...
                pathnodetype != PathNodeType::TRAPDOOR)
            {
                    ...
                    entity->width < 1.0F)
                {
                    ...
                    AxisAlignedBB axisalignedbb = AxisAlignedBB(d2 - d1, (double)y + 0.001, d3 - d1, d2 + d1,
                                                                (double)((float)y + entity->height), d3 + d1);
            ...
            if (pathnodetype == PathNodeType::OPEN)
            {
                AxisAlignedBB axisalignedbb3 =
                    AxisAlignedBB((double)x - d1 + 0.5, (double)y + 0.001, (double)z - d1 + 0.5, (double)x + d1 + 0.5,
                                  (double)((float)y + entity->height), (double)z + d1 + 0.5);
...
{
    return getPathNodeType(blockaccess, x, y, z, entitylivingIn, entitySizeX, entitySizeY, entitySizeZ,
                           getCanOpenDoors(), getCanEnterDoors());
}
