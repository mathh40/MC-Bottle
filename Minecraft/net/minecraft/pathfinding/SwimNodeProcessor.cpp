...
{
    return openPoint(MathHelper::floor(x - (double)(entity->width / 2.0F)), MathHelper::floor(y + 0.5), MathHelper::floor(z - (double)(entity->width / 2.0F)));
}
    ...
    PathPoint targetPoint, float maxDistance)
{
    ...
    for(int32_t var8 = 0; var8 < var6.size(); ++var8) 
    {
        ...
        auto pathpoint = getWaterNode(currentPoint.x + enumfacing.getXOffset(), currentPoint.y + enumfacing.getYOffset(), currentPoint.z + enumfacing.getZOffset());
