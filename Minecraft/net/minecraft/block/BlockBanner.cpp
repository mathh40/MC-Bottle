...
BlockBanner::BlockBanner()
	:BlockContainer(Material.WOOD)
{

}

std::optional<AxisAlignedBB> BlockBanner::getCollisionBoundingBox(IBlockState blockState, IBlockAccess worldIn, BlockPos pos)
{
	return std::nullopt;
}
...
ItemStack BlockBanner::getTileDataItemStack(World worldIn, BlockPos pos)
{
	...
	return Util::instanceof< TileEntityBanner>(tileentity) ? ((TileEntityBanner)tileentity).getItem() : ItemStack.EMPTY;
}
...
void BlockBanner::dropBlockAsItemWithChance(World worldIn, BlockPos pos, IBlockState state, float chance, int32_t fortune)
{
	auto itemstack = getTileDataItemStack(worldIn, pos);
...
void BlockBanner::harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, std::optional<TileEntity> te, ItemStack stack)
{
...
void BlockBannerHanging::neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos)
{
