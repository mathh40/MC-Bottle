...
BlockFalling::BlockFalling()
	:Block(Material.SAND)
{
	setCreativeTab(CreativeTabs.BUILDING_BLOCKS);
}

BlockFalling::BlockFalling(Material materialIn)
	: Block(materialIn)
{
...
void BlockFalling::checkFallable(World worldIn, BlockPos pos)
{
	if (canFallThrough(worldIn.getBlockState(pos.down())) && pos.gety() >= 0) {
		...
		if (!fallInstantly && worldIn.isAreaLoaded(pos.add(-32, -32, -32), pos.add(32, 32, 32))) {
			if (!worldIn.isRemote) {
				auto entityfallingblock = EntityFallingBlock(worldIn, (double)pos.getx() + 0.5, (double)pos.gety(), (double)pos.getz() + 0.5, worldIn.getBlockState(pos));
