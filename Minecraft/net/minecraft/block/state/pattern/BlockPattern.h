...
class BlockPattern
{
  public:
    ...
    {
      public:
        PatternHelper(const BlockPos &posIn, EnumFacing fingerIn, EnumFacing thumbIn,
                      lru_cache_t<BlockPos, BlockWorldState> &lcacheIn, uint32_t widthIn, uint32_t heightIn,
                      uint32_t depthIn);
  ...
  protected:
    static BlockPos translateOffset(const BlockPos &pos, EnumFacing finger, EnumFacing thumb, uint32_t palmOffset,
                                    uint32_t thumbOffset, uint32_t fingerOffset);

  private:
    std::optional<BlockPattern::PatternHelper> checkPatternAt(const BlockPos &pos, EnumFacing finger, EnumFacing thumb,
                                                              lru_cache_t<BlockPos, BlockWorldState> &lcache);
