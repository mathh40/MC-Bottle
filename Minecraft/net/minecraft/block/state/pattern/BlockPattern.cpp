...
std::optional<BlockPattern::PatternHelper> BlockPattern::checkPatternAt(const BlockPos &pos, EnumFacing finger,
                                                                        EnumFacing thumb,
...
BlockPos BlockPattern::translateOffset(const BlockPos &pos, EnumFacing finger, EnumFacing thumb, uint32_t palmOffset,
                                       uint32_t thumbOffset, uint32_t fingerOffset)
{
...
BlockPattern::PatternHelper::PatternHelper(const BlockPos &posIn, EnumFacing fingerIn, EnumFacing thumbIn,
                                           lru_cache_t<BlockPos, BlockWorldState> &lcacheIn, uint32_t widthIn,
                                           uint32_t heightIn, uint32_t depthIn)
    : frontTopLeft(posIn), forwards(fingerIn), up(thumbIn), lcache(lcacheIn), width(widthIn), height(heightIn),
      depth(depthIn)
{
}
...
BlockWorldState BlockPattern::PatternHelper::translateOffset(uint32_t palmOffset, uint32_t thumbOffset,
                                                             uint32_t fingerOffset) const
{
    return lcache.Get(
        BlockPattern::translateOffset(frontTopLeft, getForwards(), getUp(), palmOffset, thumbOffset, fingerOffset));
}
