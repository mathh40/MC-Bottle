    ...
    if (!aisleIn.empty() && !aisleIn[0].empty())
    {
        ...
        if (aisleIn.size() != aisleHeight)
        {
            throw std::runtime_error(
                fmt::format("Expected aisle with height of {}, but was given one with a height of {})", aisleHeight,
                            aisleIn.size()));
        }
        else
        {
            for (auto &s : aisleIn)
            {
                if (s.size() != rowWidth)
                {
                    throw std::runtime_error(fmt::format(
                        "Not all rows in the given aisle are the correct width (expected {}, found one with {})",
                        rowWidth, s.size()));
                }
...
multivec3<> &FactoryBlockPattern::makePredicateArray()
{
    ...
    multivec3<> predicate = (Predicate[][][])((Predicate[][][])((Predicate[][][])Array.newInstance(
        Predicate.class, new int[]{this.depth.size(), this.aisleHeight, this.rowWidth})));
