...
public:
  ...
  public:
    ...
    void buildPropertyValueTable(std::unordered_map<std::unordered_map<IProperty *, std::set<std::any>>, IBlockState *> &map);
    ...
    void addCollisionBoxToList(World *worldIn,
      ...
      std::optional<std::vector<AxisAlignedBB>> &collidingBoxes,
      Entity *entityIn,
  ...
  private:
    std::unordered_map<uint64_t, IProperty *> getPropertiesWithValue(IProperty *property, std::function<bool()> value);
    Block* block;
