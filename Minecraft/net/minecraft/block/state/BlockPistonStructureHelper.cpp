...
BlockPistonStructureHelper::BlockPistonStructureHelper(World *worldIn, const BlockPos &posIn, EnumFacing pistonFacing,
                                                       bool extending)
    ...
    else
    {
        ...
        else
        {
            while (block == Blocks::SLIME_BLOCK)
            {
                ...
                if (iblockstate->getMaterial() == Material::AIR ||
                    !BlockPistonBase::canPush(iblockstate, world, blockpos, moveDirection, false,
            ...
            while (true)
            {
                ...
                auto k =
                    std::distance(toMove.begin(),
                                  std::ranges::find_if(toMove, [&](const BlockPos &pos) { return pos == blockpos1; }));
                if (k > -1)
                {
                    ...
                    {
                        if (world->getBlockState(blockpos2)->getBlock() == Blocks::SLIME_BLOCK &&
                            !addBranchingBlocks(blockpos2))
                        {
                            return false;
                        }
                    }
