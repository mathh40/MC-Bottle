...
std::string BlockStateContainer::validateProperty(Block *block, IProperty *propertie)
{
    ...
    else
    {
        ...
        do
        {
            if (var3 == propertie->getAllowedValues().end())
            {
                return s;
            }
        ...
        throw std::runtime_error("Block: " + block->getClass() + " has property: " + s +
...
std::vector<std::set<std::any>> BlockStateContainer::getAllowedValues()
{
    ...
    for (auto &iproperty : properties)
    {
        list.emplace_back(iproperty.second->getAllowedValues());
    }
