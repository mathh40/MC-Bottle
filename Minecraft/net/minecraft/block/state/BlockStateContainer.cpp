	...
	:block(blockIn)
{
	...
	{
		BlockStateContainer.StateImplementation blockstatecontainer$stateimplementation1 = (BlockStateContainer.StateImplementation)var12.next();
...
std::string BlockStateContainer::validateProperty(Block* block, IProperty* property)
{
	...
	else 
	{
		...
		throw std::runtime_error("Block: " + block->getLocalizedName() + " has property: " + s + " with invalidly named value: " + s1);
	}
}

std::vector<std::set<std::any>>  BlockStateContainer::getValidStates() const
{
	return validStates;
}

IBlockState* BlockStateContainer::getBaseState()
{
	return std::any_cast<IBlockState*>(validStates[0]);
}

Block* BlockStateContainer::getBlock()
{
	return block;
}
...
StateImplementation::StateImplementation(Block* blockIn, std::unordered_map<std::string, IProperty*> propertiesIn)
	:block(blockIn), properties(propertiesIn)
{
}
