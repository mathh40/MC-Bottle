...
bool BlockAnvil::isFullCube(IBlockState state)
{
	return false;
}
...
bool BlockAnvil::isOpaqueCube(IBlockState state)
{
	return false;
}

IBlockState BlockAnvil::getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int32_t meta, EntityLivingBase placer)
{
	EnumFacing enumfacing = placer.getHorizontalFacing().rotateY();

	try {
		return BlockFalling::getStateForPlacement(worldIn, pos, facing, hitX, hitY, hitZ, meta, placer).withProperty(FACING, enumfacing).withProperty(DAMAGE, meta >> 2);
	}
		...
		if (!worldIn.isRemote) {
			...
			if (Util::instanceof< EntityPlayer>(placer)) {
				placer.sendMessage(TextComponentTranslation("Invalid damage property. Please pick in [0, 1, 2]", new Object[0]));
			}
		}

		return BlockFalling::getStateForPlacement(worldIn, pos, facing, hitX, hitY, hitZ, 0, placer).withProperty(FACING, enumfacing).withProperty(DAMAGE, 0);
	}
}

bool BlockAnvil::onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
{
	if (!worldIn.isRemote) {
...
int32_t BlockAnvil::damageDropped(IBlockState state)
{
...
void BlockAnvil::onStartFalling(EntityFallingBlock fallingEntity)
{
...
void BlockAnvil::onBroken(World worldIn, BlockPos pos)
{
	worldIn.playEvent(1029, pos, 0);
}

bool BlockAnvil::shouldSideBeRendered(IBlockState blockState, IBlockAccess blockAccess, BlockPos pos, EnumFacing side)
{
	return true;
}
...
{
	return getDefaultState().withProperty(FACING, EnumFacing::byHorizontalIndex(meta & 3)).withProperty(DAMAGE, (meta & 15) >> 2);
}
...
std::string BlockAnvil::Anvil::getName() const
{
	return "anvil";
}

bool BlockAnvil::Anvil::hasCustomName() const
{
	return false;
}

std::unique_ptr<ITextComponent> BlockAnvil::Anvil::getDisplayName() const
{
	return std::make_unique<TextComponentTranslation>(Blocks.ANVIL.getTranslationKey() + ".name");
}

std::string BlockAnvil::Anvil::getGuiID() const
{
	return "minecraft:anvil";
}

BlockAnvil::Anvil::Anvil(World worldIn, BlockPos pos)
	:world(worldIn), position(pos)
{
	
}

Container BlockAnvil::Anvil::createContainer(InventoryPlayer playerInventory, EntityPlayer playerIn)
{
...
{
	return BlockStateContainer(this, IProperty[]{ FACING, DAMAGE });
}
