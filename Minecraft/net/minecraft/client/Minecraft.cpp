...
Minecraft::Minecraft(const GameConfiguration &gameConfig)
    : gameDir(gameConfig.folderInfo.gameDir), fileAssets(gameConfig.folderInfo.assetsDir),
      ...
      defaultResourcePack(gameConfig.folderInfo.getAssetsIndex()),
      // sessionService(YggdrasilAuthenticationService(this.proxy,
      // UUID.randomUUID().toString())).createMinecraftSessionService()),
      session(gameConfig.userInfo.session), isDemo(gameConfig.gameInfo.isDemo),
      displayWidth(gameConfig.displayInfo.width > 0 ? gameConfig.displayInfo.width : 640),
      ...
      fullscreen(gameConfig.displayInfo.fullscreen), integratedServer(std::nullopt),
      dataFixer(DataFixesManager.createFixer()), toastGui(this), tutorial(this)
{
