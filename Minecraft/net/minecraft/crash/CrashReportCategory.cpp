...
std::string CrashReportCategory::getCoordinateInfo(int32_t x, int32_t y, int32_t z) {
    ...
    try {
        ...
        j4 = (k2 + 1 << 4) - 1;
        k4 = (l2 + 1 << 4) - 1;
        stringbuilder += fmt::format("Chunk: (at {:d},{:d},{:d} in {:d},{:d}; contains blocks {:d},0,{:d} to {:d},255,{:d})", i3,
                                           j3, k3, k2, l2, l3, i4, j4, k4);
    } catch (std::exception& e) {
    ...
    try {
        ...
        k4 = (k2 + 1 << 9) - 1;
        int j2 = (l2 + 1 << 9) - 1;
        stringbuilder += (fmt::format(
            "Region: (%d,%d; contains chunks %d,%d to %d,%d, blocks %d,0,%d to %d,255,%d)", k2, l2, i3, j3, k3, l3, i4,
            j4, k4, j2));
    } catch (std::exception& e) {
        stringbuilder += "(Error finding world loc)";
    }
...
bool CrashReportCategory::firstTwoElementsOfStackTraceMatch(const StackTraceElement &s1, const StackTraceElement &s2) {
    if (stackTrace.size() != 0 && s1 != nullptr) {
        ...
        if (stacktraceelement.isNativeMethod() == s1.isNativeMethod() && stacktraceelement.getClassName().
            equals(s1.getClassName()) && stacktraceelement.getFileName().equals(s1.getFileName()) && stacktraceelement.
            getMethodName().equals(s1.getMethodName())) {
