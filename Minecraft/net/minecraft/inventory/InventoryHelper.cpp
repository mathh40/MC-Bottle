...
void InventoryHelper::spawnItemStack(World *worldIn, double x, double y, double z, ItemStack stack)
{
    ...
    while (!stack.isEmpty())
    {
        EntityItem *entityitem =
            new EntityItem(worldIn, x + (double)f, y + (double)f1, z + (double)f2, stack.splitStack(RANDOM(21) + 10));
        float f3            = 0.05F;
...
bool InventoryHelper::canMergeStacks(ItemStack stack1, ItemStack stack2, IInventory *inventory)
{
    return !stack1.isEmpty() && stackEqualExact(stack1, stack2) && stack1.isStackable() &&
           stack1.getCount() < stack1.getMaxStackSize() && stack1.getCount() < inventory->getInventoryStackLimit();
}
...
{
    if (std::adjacent_find(inventory.begin(), inventory.end(),
                           [](const ItemStack &stack) { return !stack.isEmpty(); }) == inventory.end())
    {
        return true;
    }
    return false;
