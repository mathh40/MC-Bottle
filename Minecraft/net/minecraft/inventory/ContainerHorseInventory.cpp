...
{
  public:
    SaddleSlot(const AbstractHorse *horse, IInventory *inventoryIn, int32_t index, int32_t xPosition,
               int32_t yPosition);
    ...
    : horseInventory(horseInventoryIn), horse(horse)
{
    ...
    if (Util:: instanceof <AbstractChestHorse>(horse) && ((AbstractChestHorse)horse).hasChest())
    {
        for (auto j1 = 0; j1 < 3; ++j1)
        {
            ...
            {
                addSlotToContainer(Slot(horseInventoryIn,
                                        2 + k1 + j1 * ((AbstractChestHorse)horse).getInventoryColumns(), 80 + k1 * 18,
                                        18 + j1 * 18));
            }
        }
    }
...
ItemStack ContainerHorseInventory::transferStackInSlot(EntityPlayer *playerIn, int32_t index)
{
    ...
    if (slot.has_value() && slot->getHasStack())
    {
        ...
        else if (horseInventory->getSizeInventory() <= 2 ||
                 !mergeItemStack(itemstack1, 2, horseInventory->getSizeInventory(), false))
        {
            return ItemStack::EMPTY;
        }
