...
SlotMerchantResult::SlotMerchantResult(EntityPlayer *player, IMerchant *merchant,
                                       InventoryMerchant *merchantInventoryIn, int32_t slotIndex, int32_t xPosition,
                                       int32_t yPosition)
    : Slot(merchantInventoryIn, slotIndex, xPosition, yPosition), player(player), merchant(merchant),
      merchantInventory(merchantInventoryIn)
{
}
...
bool SlotMerchantResult::doTrade(const MerchantRecipe &trade, ItemStack firstItem, ItemStack secondItem)
{
    ...
    {
        if (!itemstack1.isEmpty() && !secondItem.isEmpty() && itemstack1.getItem() == secondItem.getItem() &&
