...
ItemBow::ItemBow()
{
    ...
    addPropertyOverride(
        ResourceLocation("pull"), [](ItemStack stack, World *worldIn, EntityLivingBase *entityIn) -> float {
            if (entityIn == nullptr)
            {
                return 0.0F;
            }
    ...
    addPropertyOverride(
        ResourceLocation("pulling"), [](ItemStack stack, World *worldIn, EntityLivingBase *entityIn) -> float {
...
void ItemBow::onPlayerStoppedUsing(ItemStack stack, World *worldIn, EntityLivingBase *entityLiving, int32_t timeLeft)
{
    if (Util:: instanceof <EntityPlayer>(entityLiving))
    {
        ...
        bool flag                  = entityplayer->capabilities.isCreativeMode ||
        ...
        if (!itemstack.isEmpty() || flag)
        {
            ...
            if ((double)f >= 0.1)
            {
                ...
                if (!worldIn->isRemote)
                {
                    ItemArrow itemarrow      = (ItemArrow)((ItemArrow)(itemstack.getItem() instanceof
                                                                  ItemArrow ? itemstack.getItem() : Items::ARROW));
                ...
                worldIn->playSound(nullptr, entityplayer->posX, entityplayer->posY, entityplayer->posZ,
                                   ...
                                   1.0F / (MathHelper::nextFloat(itemRand) * 0.4F + 1.2F) + f * 0.5F);
