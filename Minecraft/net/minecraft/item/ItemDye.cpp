...
void ItemDye::spawnBonemealParticles(World *worldIn, BlockPos pos, int32_t amount)
{
    ...
    if (iblockstate->getMaterial() != Material::AIR) 
    {
        for(int32_t i = 0; i < amount; ++i) 
        {
            ...
            worldIn->spawnParticle(EnumParticleTypes::VILLAGER_HAPPY, (double)((float)pos.getx() + MathHelper::nextFloat(itemRand)), (double)pos.gety() + (double)MathHelper::nextFloat(itemRand) * iblockstate->getBoundingBox(worldIn, pos).maxY, (double)((float)pos.getz() + MathHelper::nextFloat(itemRand)), d0, d1, d2);
        }
    }
}
