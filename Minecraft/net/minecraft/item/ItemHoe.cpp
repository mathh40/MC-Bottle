...
ItemHoe::ItemHoe(const ToolMaterial &material) : toolMaterial(material)
{
    maxStackSize = 1;
...
std::unordered_multimap<std::string, AttributeModifier> ItemHoe::getItemAttributeModifiers(
    EntityEquipmentSlot equipmentSlot)
{
    ...
    if (equipmentSlot == EntityEquipmentSlot::MAINHAND)
    {
        multimap.emplace(SharedMonsterAttributes::ATTACK_DAMAGE.getName(),
                         AttributeModifier(ATTACK_DAMAGE_MODIFIER, "Weapon modifier", 0.0, 0));
        multimap.emplace(SharedMonsterAttributes::ATTACK_SPEED.getName(),
                         AttributeModifier(ATTACK_SPEED_MODIFIER, "Weapon modifier", (double)(speed - 4.0F), 0));
    }
