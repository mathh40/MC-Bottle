...
void ItemWrittenBook::addInformation(ItemStack stack, World* worldIn, std::vector<std::string>& tooltip, ITooltipFlag* flagIn)
{
    if (stack.hasTagCompound()) 
    {
        NBTTagCompound* nbttagcompound = stack.getTagCompound();
        ...
        tooltip.emplace(TextFormatting::GRAY + I18n::translateToLocal("book.generation." + nbttagcompound->getInteger("generation")));
    }
}
        ...
        if (!nbttagcompound->getBoolean("resolved")) 
        {
            ...
            if (validBookTagContents(nbttagcompound)) 
            {
                ...
                for(auto i = 0; i < nbttaglist->tagCount(); ++i) 
                {
                    ...
                    nbttaglist->set(i, NBTTagString(ITextComponent::Serializer.componentToJson((ITextComponent*)itextcomponent)));
                }
