    ...
    Entity *targetEntity)
{
    ...
    if (minecraftserver != nullptr && targetEntity != nullptr) 
    {
        ...
        {
            if (!entityWorld->isRemote && targetEntity->ignoreItemEntityData() && (player == nullptr || !minecraftserver->getPlayerList().canSendCommands(player->getGameProfile()))) 
            {
                return;
            }
    ...
    if (entityID != nullptr && EntityList::ENTITY_EGGS.containsKey(entityID)) 
    {
        ...
        for(auto i = 0; i < 1; ++i) 
        {
            ...
            if (Util::instanceof<EntityLiving>(entity)) 
            {
                ...
                entity->setLocationAndAngles(x, y, z, MathHelper::wrapDegrees(MathHelper::nextFloat(worldIn->rand) * 360.0F), 0.0F);
