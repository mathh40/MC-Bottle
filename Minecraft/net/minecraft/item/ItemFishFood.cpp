...
ItemFishFood::ItemFishFood(bool cooked) : ItemFood(0, 0.0f, false), cooked(cooked)
{
}
...
int32_t ItemFishFood::getHealAmount(ItemStack stack) const
{
    ...
    return cooked && itemfishfood$fishtype.canCook() ? itemfishfood$fishtype.getCookedHealAmount()
...
{
    const FishType itemfishfood$fishtype = FishType::byItemStack(stack);
    return cooked && itemfishfood$fishtype.canCook() ? itemfishfood$fishtype.getCookedSaturationModifier()
...
{
    FishType itemfishfood$fishtype = FishType::byItemStack(stack);
    return getTranslationKey() + "." + itemfishfood$fishtype.getTranslationKey() + "." +
           (cooked && itemfishfood$fishtype.canCook() ? "cooked" : "raw");
}
