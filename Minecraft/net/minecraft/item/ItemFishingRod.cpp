...
ItemFishingRod::ItemFishingRod()
{
    ...
    addPropertyOverride(ResourceLocation("cast"),[&](ItemStack stack, World *worldIn, EntityLivingBase *entityIn)->float
    {
        if (entityIn == nullptr) 
        {
            return 0.0F;
        ...
        else 
        {
            ...
            return (flag || flag1) && Util::instanceof<EntityPlayer>(entityIn) && ((EntityPlayer*)entityIn)->fishEntity != nullptr ? 1.0F : 0.0F;
        }
    });
}
...
ActionResult ItemFishingRod::onItemRightClick(World *worldIn, EntityPlayer *playerIn, EnumHand handIn)
{
    ...
    if (playerIn->fishEntity != nullptr) 
    {
        ...
        worldIn->playSound(nullptr, playerIn->posX, playerIn->posY, playerIn->posZ, SoundEvents::ENTITY_BOBBER_RETRIEVE, SoundCategory::NEUTRAL, 1.0F, 0.4F / (MathHelper::nextFloat(itemRand) * 0.4F + 0.8F));
    }
    else 
    {
        worldIn->playSound(nullptr, playerIn->posX, playerIn->posY, playerIn->posZ, SoundEvents::ENTITY_BOBBER_THROW, SoundCategory::NEUTRAL, 0.5F, 0.4F / (MathHelper::nextFloat(itemRand) * 0.4F + 0.8F));
        if (!worldIn->isRemote) 
        {
