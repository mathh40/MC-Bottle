...
void ItemLingeringPotion::addInformation(ItemStack stack, World* worldIn, std::vector<std::string>& tooltip, ITooltipFlag* flagIn)
{
...
ActionResult ItemLingeringPotion::onItemRightClick(World *worldIn, EntityPlayer *playerIn, EnumHand handIn)
{
    ...
    worldIn->playSound(nullptr, playerIn->posX, playerIn->posY, playerIn->posZ, SoundEvents::ENTITY_LINGERINGPOTION_THROW, SoundCategory::NEUTRAL, 0.5F, 0.4F / (MathHelper::nextFloat(itemRand) * 0.4F + 0.8F));
    if (!worldIn->isRemote) 
    {
