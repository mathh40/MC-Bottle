...
std::vector<xg::Guid> ItemArmor::ARMOR_MODIFIERS = {xg::Guid("845DB27C-C624-495F-8C9F-6020A9A58B6B"), xg::Guid("D8499B04-0E66-4726-AB29-64469D734E0D"), xg::Guid("9F3D476D-C118-4544-8365-64846904B48E"), xg::Guid("2AD3F246-FEE1-4E67-B886-69FD380BB150")};
...
ItemStack ItemArmor::dispenseArmor(IBlockSource *blockSource, ItemStack stack)
{
    ...
    auto list = blockSource->getWorld()->getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB(blockpos), Predicates.and(EntitySelectors.NOT_SPECTATING, EntitySelectors.ArmoredMob(stack)));
    if (list.isEmpty()) 
    {
...
ItemArmor::ItemArmor(ArmorMaterial materialIn, int32_t renderIndexIn, EntityEquipmentSlot equipmentSlotIn)
    :material(materialIn),armorType(equipmentSlotIn),renderIndex(renderIndexIn),damageReduceAmount(materialIn.getDamageReductionAmount(equipmentSlotIn))
...
bool ItemArmor::hasColor(ItemStack stack)
{
    ...
    else 
    {
        ...
        return nbttagcompound != nullptr && nbttagcompound->hasKey("display", 10) ? nbttagcompound->getCompoundTag("display").hasKey("color", 3) : false;
    }
}
...
std::unordered_multimap<std::string,AttributeModifier> ItemArmor::getItemAttributeModifiers(EntityEquipmentSlot equipmentSlot)
{
    auto multimap = Item::getItemAttributeModifiers(equipmentSlot);
    if (equipmentSlot == armorType) 
    {
        multimap.emplace(SharedMonsterAttributes::ARMOR.getName(), AttributeModifier(ARMOR_MODIFIERS[equipmentSlot.getIndex()], "Armor modifier", (double)damageReduceAmount, 0));
        multimap.emplace(SharedMonsterAttributes::ARMOR_TOUGHNESS.getName(), AttributeModifier(ARMOR_MODIFIERS[equipmentSlot.getIndex()], "Armor toughness", (double)toughness, 0));
    }
