    ...
    if (aint.size() > 0)
    {
        ...
        for (auto var8 = 0; var8 < aint.size(); ++var8)
        {
            ...
            for (auto j = 0; j < ItemDye::DYE_COLORS.size(); ++j)
            {
                ...
                {
                    flag1 = true;
                    s     = s + I18n::translateToLocal("item.fireworksCharge." +
                                                       EnumDyeColor::byDyeDamage(j)->getTranslationKey());
                    break;
                }
            }
    ...
    if (!aint1.empty())
    {
        ...
        for (auto i = 0; i < aint1.size(); ++i)
        {
            ...
            for (int k = 0; k < 16; ++k)
            {
                ...
                {
                    flag5 = true;
                    s1    = s1 + I18n::translateToLocal("item.fireworksCharge." +
                                                        EnumDyeColor::byDyeDamage(k)->getTranslationKey());
                    break;
                }
            }
...
void ItemFireworkCharge::addInformation(ItemStack stack, World *worldIn, std::vector<std::string> &tooltip,
                                        ITooltipFlag *flagIn)
{
    if (stack.hasTagCompound())
    {
