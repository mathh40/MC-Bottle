...
ItemBanner::ItemBanner()
    :ItemBlock(Blocks::STANDING_BANNER)
{
    maxStackSize = 16;
...
void ItemBanner::appendHoverTextFromTileEntityTag(ItemStack stack, std::vector<std::string> &p_185054_1_)
{
    ...
    if (nbttagcompound != nullptr && nbttagcompound->hasKey("Patterns")) 
    {
        ...
        for(int i = 0; i < nbttaglist->tagCount() && i < 6; ++i) 
        {
            ...
            {
                p_185054_1_.push_back(I18n::translateToLocal("item.banner." + bannerpattern->getFileName() + "." + enumdyecolor.getTranslationKey()));
            }
        }
    }
}

void ItemBanner::addInformation(ItemStack stack, World* worldIn, std::vector<std::string>& tooltip, ITooltipFlag* flagIn)
{
