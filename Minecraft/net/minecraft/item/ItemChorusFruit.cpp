...
ItemChorusFruit::ItemChorusFruit(int32_t amount, float saturation)
    :ItemFood(amount, saturation, false)
{
}
...
ItemStack ItemChorusFruit::onItemUseFinish(ItemStack stack, World *worldIn, EntityLivingBase *entityLiving)
{
    ...
    if (!worldIn->isRemote) 
    {
        ...
        for(int i = 0; i < 16; ++i) 
        {
            ...
            {
                worldIn->playSound((EntityPlayer*)nullptr, d0, d1, d2, SoundEvents::ITEM_CHORUS_FRUIT_TELEPORT, SoundCategory::PLAYERS, 1.0F, 1.0F);
