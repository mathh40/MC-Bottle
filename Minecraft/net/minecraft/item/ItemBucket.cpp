...
ItemBucket::ItemBucket(Block *containedBlockIn) : containedBlock(containedBlockIn)
{
    maxStackSize = 2;
            ...
            else
            {
                ...
                if (material == Material::WATER && iblockstate->getValue(BlockLiquid::LEVEL) == 0)
                {
                    ...
                    return ActionResult(EnumActionResult::SUCCESS,
                                        fillBucket(itemstack, playerIn, Items::WATER_BUCKET));
                }
    ...
    else
    {
        ...
        else
        {
            if (worldIn->provider.doesWaterVaporize() && containedBlock == Blocks::FLOWING_WATER)
            {
                ...
                auto j = posIn.getz();
                worldIn->playSound(
                    player, posIn, SoundEvents::BLOCK_FIRE_EXTINGUISH, SoundCategory::BLOCKS, 0.5F,
                    2.6F + (MathHelper::nextFloat(worldIn->rand) - MathHelper::nextFloat(worldIn->rand)) * 0.8F);

                for (int k = 0; k < 8; ++k)
                {
                    worldIn->spawnParticle(EnumParticleTypes::SMOKE_LARGE, (double)l + MathHelper::random(),
                                           (double)i + MathHelper::random(), (double)j + MathHelper::random(), 0.0, 0.0,
                                           0.0);
                }
            }
            else
            {
                ...
                SoundEvent soundevent = containedBlock == Blocks::FLOWING_LAVA ? SoundEvents::ITEM_BUCKET_EMPTY_LAVA
