...
ItemBoat::ItemBoat(EntityBoat::Type typeIn) : type(typeIn)
{
    maxStackSize = 1;
...
ActionResult ItemBoat::onItemRightClick(World *worldIn, EntityPlayer *playerIn, EnumHand handIn)
{
    ...
    else
    {
        ...
        auto list    = worldIn->getEntitiesWithinAABBExcludingEntity(
               playerIn, playerIn->getEntityBoundingBox()
                             .expand(vec3d2.getx() * 5.0, vec3d2.gety() * 5.0, vec3d2.getz() * 5.0)
                             .grow(1.0));
        ...
        else
        {
            ...
            EntityBoat *entityboat = new EntityBoat(worldIn, raytraceresult.hitVec.x,
