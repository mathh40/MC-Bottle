                    ...
                    if (k1 >= 0 && l1 >= -1 && k1 < 128 && l1 < 128) 
                    {
                        ...
                        if (!chunk.isEmpty()) 
                        {
                            ...
                            if (worldIn->provider->isNether()) 
                            {
                                ...
                                if ((l3 >> 20 & 1) == 0) 
                                {
                                    multiset.emplace(Blocks::DIRT::getDefaultState().withProperty(BlockDirt::VARIANT, BlockDirt::DirtType::DIRT).getMapColor(worldIn, BlockPos::ORIGIN), 10);
                                }
                                else 
                                {
                                    multiset.emplace(Blocks.STONE.getDefaultState().withProperty(BlockStone.VARIANT, BlockStone.EnumType.STONE).getMapColor(worldIn, BlockPos::ORIGIN), 100);
                                }
...
void ItemMap::renderBiomePreviewMap(World *worldIn, ItemStack map)
{
    if (map.getItem() == Items::FILLED_MAP) 
    {
        ...
        if (mapdata != nullptr && worldIn->provider->getDimensionType().getId() == mapdata.dimension) 
        {
            ...
            abiome = worldIn->getBiomeProvider().getBiomes(abiome, (j / i - 64) * i, (k / i - 64) * i, 128 * i, 128 * i, false);
...
void ItemMap::onUpdate(ItemStack stack, World *worldIn, Entity *entityIn, int32_t itemSlot, bool isSelected)
{
    if (!worldIn->isRemote) 
    {
        ...
        if (isSelected || Util::instanceof< EntityPlayer>(entityIn) && ((EntityPlayer*)entityIn).getHeldItemOffhand() == stack) 
        {
