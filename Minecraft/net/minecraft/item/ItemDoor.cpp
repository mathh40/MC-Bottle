...
void ItemDoor::placeDoor(World *worldIn, BlockPos pos, EnumFacing facing, Block *door, bool isRightHinge)
{
    ...
    bool flag = worldIn->getBlockState(blockpos1).getBlock() == door || worldIn->getBlockState(blockpos1.up()).getBlock() == door;
    bool flag1 = worldIn->getBlockState(blockpos).getBlock() == door || worldIn->getBlockState(blockpos.up()).getBlock() == door;
    if ((!flag || flag1) && j <= i) 
    {
        if (flag1 && !flag || j < i) 
        {
