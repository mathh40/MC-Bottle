...
{
  public:
    ShapedRecipes(std::string group, int32_t width, int32_t height, std::vector<Ingredient> ingredients,
                  ItemStack result);
...
std::vector<Ingredient> deserializeIngredients(std::vector<std::string> &pattern,
                                               std::unordered_map<std::string, Ingredient> &keys, int32_t patternWidth,
...
namespace nlohmann
{
template <> struct adl_serializer<ShapedRecipes>
{
