...
ActionResult ItemSplashPotion::onItemRightClick(World *worldIn, EntityPlayer *playerIn, EnumHand handIn)
{
    ...
    worldIn->playSound(nullptr, playerIn->posX, playerIn->posY, playerIn->posZ, SoundEvents.ENTITY_SPLASH_POTION_THROW, SoundCategory::PLAYERS, 0.5F, 0.4F / (MathHelper::nextFloat(itemRand) * 0.4F + 0.8F));
    if (!worldIn->isRemote) 
    {
