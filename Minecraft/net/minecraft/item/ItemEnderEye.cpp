...
ActionResult ItemEnderEye::onItemRightClick(World *worldIn, EntityPlayer *playerIn, EnumHand handIn)
{
    ...
    if (raytraceresult != nullptr && raytraceresult.typeOfHit == RayTraceResult::Type::BLOCK && worldIn->getBlockState(raytraceresult.getBlockPos()).getBlock() == Blocks::END_PORTAL_FRAME) 
    {
    ...
    else 
    {
        ...
        if (!worldIn->isRemote) 
        {
            ...
            if (blockpos != nullptr) 
            {
                ...
                worldIn->playSound(nullptr, playerIn->posX, playerIn->posY, playerIn->posZ, SoundEvents::ENTITY_ENDEREYE_LAUNCH, SoundCategory::NEUTRAL, 0.5F, 0.4F / (MathHelper::nextFloat(itemRand) * 0.4F + 0.8F));
