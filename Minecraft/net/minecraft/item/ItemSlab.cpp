                                     ...
                                     EnumFacing facing, float hitX, float hitY, float hitZ)
{
    ...
    if (!itemstack.isEmpty() && player->canPlayerEdit(pos.offset(facing), facing, itemstack))
    {
        ...
        return tryPlace(player, itemstack, worldIn, pos.offset(facing), comparable)
                   ? EnumActionResult::SUCCESS
                   : ItemBlock::onItemUse(player, worldIn, pos, hand, facing, hitX, hitY, hitZ);
    }
    else
    {
...
bool ItemSlab::canPlaceBlockOnSide(World *worldIn, BlockPos pos, EnumFacing side, EntityPlayer *player, ItemStack stack)
{
    ...
    return iblockstate1->getBlock() == singleSlab && comparable == iblockstate1->getValue(iproperty)
               ? true
               : ItemBlock::canPlaceBlockOnSide(worldIn, blockpos, side, player, stack);
}

bool ItemSlab::tryPlace(EntityPlayer *player, ItemStack stack, World *worldIn, BlockPos pos, Comparable itemSlabType)
{
    IBlockState *iblockstate = worldIn->getBlockState(pos);
    if (iblockstate->getBlock() == singleSlab)
    {
