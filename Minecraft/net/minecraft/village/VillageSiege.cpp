...
VillageSiege::VillageSiege(World *worldIn) : world(worldIn)
{
}
...
bool VillageSiege::spawnZombie()
{
    ...
    else
    {
        ...
        try
        {
            ...
            entityzombie->onInitialSpawn(world->getDifficultyForLocation(BlockPos(entityzombie)),
        ...
        entityzombie->setLocationAndAngles(vec3d.getx(), vec3d.gety(), vec3d.getz(),
...
std::optional<Vec3d> VillageSiege::findRandomSpawnPos(BlockPos pos)
{
    for (int i = 0; i < 10; ++i)
    {
        ...
        if (village.isBlockPosWithinSqVillageRadius(blockpos) &&
            WorldEntitySpawner::canCreatureTypeSpawnAtLocation(EntityLiving::SpawnPlacementType.ON_GROUND, world,
                                                               blockpos))
        {
