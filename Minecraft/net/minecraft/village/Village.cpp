...
Village::Village() : centerHelper(BlockPos::ORIGIN), center(BlockPos::ORIGIN)
{
}

Village::Village(World *worldIn) : centerHelper(BlockPos::ORIGIN), center(BlockPos::ORIGIN), world(worldIn)
{
}
...
{
    auto list = world->getEntitiesWithinAABB(
        EntityIronGolem.class,
        AxisAlignedBB((double)(center.getx() - villageRadius), (double)(center.gety() - 4),
                      (double)(center.getz() - villageRadius), (double)(center.getx() + villageRadius),
                      (double)(center.gety() + 4), (double)(center.getz() + villageRadius)));
...
void Village::removeDeadAndOldAgressors()
{
    ...
    while (true)
    {
        ...
        } while (village$villageaggressor.agressor.isEntityAlive() &&
                 MathHelper::abs(tickCounter - village$villageaggressor.agressionTime) <= 300);
...
void Village::removeDeadAndOutOfRangeDoors()
{
    ...
    while (true)
    {
        ...
        } while (isWoodDoor(villagedoorinfo.getDoorBlockPos()) &&
                 MathHelper::abs(tickCounter - villagedoorinfo.getLastActivityTimestamp()) <= 1200);
...
void Village::updateVillageRadiusAndCenter()
{
    ...
    else
    {
        ...
        VillageDoorInfo villagedoorinfo;
        for (Iterator var3 = villageDoorInfoList.iterator(); var3.hasNext();
             j             = MathHelper::max(villagedoorinfo.getDistanceToDoorBlockSq(center), j))
        {
    ...
    for (auto villagedoorinfo1 : villageDoorInfoList)
    {
        ...
        if (j < i)
        {
            ...
            if (world->getBlockState(blockpos.offset(enumfacing, 1))
                    .getBlock()
                    .isPassable(world, blockpos.offset(enumfacing, 1)) &&
                world->getBlockState(blockpos.offset(enumfacing, -1))
                    .getBlock()
                    .isPassable(world, blockpos.offset(enumfacing, -1)) &&
                world->getBlockState(blockpos.up().offset(enumfacing, 1))
                    .getBlock()
                    .isPassable(world, blockpos.up().offset(enumfacing, 1)) &&
                world->getBlockState(blockpos.up().offset(enumfacing, -1))
                    .getBlock()
                    .isPassable(world, blockpos.up().offset(enumfacing, -1)))
            {
