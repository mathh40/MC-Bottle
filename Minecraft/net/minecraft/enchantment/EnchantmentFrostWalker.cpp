...
void EnchantmentFrostWalker::freezeNearby(EntityLivingBase *living, World *worldIn, const BlockPos &pos, int32_t level)
{
    if (living->onGround)
    {
        float f            = MathHelper::min(16, 2 + level);
        BlockPos block_pos = BlockPos(0, 0, 0);
        auto var6 = getAllInBoxMutable(pos.add((double)(-f), -1.0, (double)(-f)), pos.add((double)f, -1.0, (double)f));
        for (auto block_pos1 : var6)
        {
            if (block_pos1.distanceSqToCenter(living->posX, living->posY, living->posZ) <= (double)(f * f))
            {
                block_pos.setPos(block_pos1.getx(), block_pos1.gety() + 1, block_pos1.getz());
                ...
                if (iblockstate->getMaterial() == Material::AIR)
                {
                    ...
                    if (iblockstate1->getMaterial() == Material::WATER &&
                        ...
                        worldIn->mayPlace(Blocks::FROSTED_ICE, block_pos1, false, EnumFacing::DOWN, nullptr))
                    {
