...
{
public:
	Playerwalker(DataFixer fixer)
		:fixer(fixer)
	{
	}
	std::unique_ptr<NBTTagCompound> process(IDataFixer* type, std::unique_ptr < NBTTagCompound> compound, int32_t versionIn) override
	{
...
WorldInfo::WorldInfo(NBTTagCompound* nbt)
...
WorldInfo::WorldInfo(const WorldInfo& worldInformation)
	: gameType(GameType::SURVIVAL), terrainType(WorldType::DEFAULT)
{
	generatorOptions = "";
	borderSize = 6.0E7;
