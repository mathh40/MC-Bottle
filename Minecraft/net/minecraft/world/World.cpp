...
{
    return !isOutsideBuildHeight(pos) && pos.getx() >= -30000000 && pos.getz() >= -30000000 && pos.getx() < 30000000 &&
           pos.getz() < 30000000;
}
            ...
            if ((!flag2 || !flag3) && isBlockLoaded(pooledmutableblockpos.setPos(k1, 64, l1)))
            {
                for (int32_t i2 = k; i2 < l; ++i2)
                {
                    if (!flag2 && !flag3 || i2 != l - 1)
                    {
                        ...
                        iblockstate1->addCollisionBoxToList(this, pooledmutableblockpos, aabb, outList, entityIn,
                                                            false);
...
{
    return isAreaLoaded(center.getx() - radius, center.gety() - radius, center.getz() - radius, center.getx() + radius,
...
{
    BlockPos blockpos;
    for (blockpos = BlockPos(pos.getx(), getSeaLevel(), pos.getz()); !isAirBlock(blockpos.up());
...
{
    markBlockRangeForRenderUpdate(rangeMin.getx(), rangeMin.gety(), rangeMin.getz(), rangeMax.getx(), rangeMax.gety(),
                                  rangeMax.getz());
}
...
void World::observedNeighborChanged(BlockPos &pos, const Block *changedBlock, BlockPos &changedBlockPos)
{
    if (!isRemote)
    {
        ...
        if (iblockstate->getBlock() == Blocks::OBSERVER)
        {
            try
            {
                ((BlockObserver)iblockstate.getBlock())
                    .observedNeighborChanged(iblockstate, this, pos, changedBlock, changedBlockPos);
            }
            catch (Throwable var8)
            {
                CrashReport crashreport = CrashReport.makeCrashReport(var8, "Exception while updating neighbours");
...
std::optional<RayTraceResult> World::rayTraceBlocks(Vec3d &vec31, Vec3d &vec32, bool stopOnLiquid,
                                                    bool ignoreBlockWithoutBoundingBox,
...
void World::playSound(EntityPlayer *player, BlockPos pos, SoundEvent soundIn, SoundCategory category, float volume,
                      float pitch)
{
    playSound(player, (double)pos.getx() + 0.5, (double)pos.gety() + 0.5, (double)pos.getz() + 0.5, soundIn, category,
              volume, pitch);
}

void World::playSound(EntityPlayer *player, double x, double y, double z, SoundEvent soundIn, SoundCategory category,
...
void World::spawnParticle(EnumParticleTypes particleType, double xCoord, double yCoord, double zCoord, double xSpeed,
                          double ySpeed, double zSpeed, std::initializer_list<int32_t> parameters)
{
    spawnParticle(particleType.getParticleID(), particleType.getShouldIgnoreRange(), xCoord, yCoord, zCoord, xSpeed,
...
{
    spawnParticle(particleType.getParticleID(), particleType.getShouldIgnoreRange() || ignoreRange, xCoord, yCoord,
...
BlockPos World::getTopSolidOrLiquidBlock(BlockPos &pos)
{
    ...
    BlockPos blockpos;
    BlockPos blockpos1;
    for (blockpos = BlockPos(pos.getx(), chunk.getTopFilledSegment() + 16, pos.getz()); blockpos.gety() >= 0;
         blockpos = blockpos1)
    {
...
void World::updateEntities()
{
    ...
    for (auto tileentity : tickableTileEntities)
    {
        if (!tileentity.isInvalid() && tileentity.hasWorld())
        {
            ...
            if (isBlockLoaded(blockpos) && worldBorder.contains(blockpos))
            {
                try
                {
                    profiler.func_194340_a(()->{ return String.valueOf(TileEntity.getKey(tileentity.getClass())); });
    ...
    const auto remite = std::remove_if(tickableTileEntities.begin(), tickableTileEntities.end(),
                                       [](TileEntity *x) { return x->isInvalid(); });
                ...
                for (auto j4 = j3; j4 < k3; ++j4)
                {
                    ...
                    if (iblockstate1.getMaterial() == materialIn)
                    {
                        double d0 = (double)((float)(i4 + 1) - BlockLiquid.getLiquidHeightPercent(
                                                                   (Integer)iblockstate1.getValue(BlockLiquid.LEVEL)));
                        if ((double)i3 >= d0)
                        {
                            flag  = true;
...
void World::setTileEntity(BlockPos &pos, TileEntity *tileEntityIn)
{
    if (!isOutsideBuildHeight(pos) && tileEntityIn != nullptr && !tileEntityIn.isInvalid())
    {
        if (processingLoadedTiles)
        {
            ...
            addedTileEntityList.erase(std::remove_if(addedTileEntityList.begin(), addedTileEntityList.end(),
                                                     [&](TileEntity *x) {
                                                         if (x->getPos() == pos)
                                                         {
                                                             x->invalidate();
    ...
    else
    {
        if (tileentity2 != nullptr)
        {
            ...
            loadedTileEntityList.erase(
                std::find(loadedTileEntityList.begin(), loadedTileEntityList.end(), tileentity2));
            tickableTileEntities.erase(
                std::find(tickableTileEntities.begin(), tickableTileEntities.end(), tileentity2));
        }
...
bool World::checkNoEntityCollision(AxisAlignedBB &bb, std::optional<Entity *> entityIn)
{
    ...
    for (auto entity4 : list)
    {
        if (!entity4.isDead && entity4.preventEntitySpawning && entity4 != entityIn &&
            (!entityIn || entity4.isRidingSameEntity(entityIn)))
        {
            return false;
        }
    }
        ...
        label92:
            while (true)
            {
                ...
                for (auto enumfacing : EnumFacing::values())
                {
                    ...
                    {
                        lightUpdateBlockList[k2++] =
                            j6 - j3 + 32 | k6 - k3 + 32 << 6 | l6 - l3 + 32 << 12 | i5 - i7 << 18;
                    }
                }
            }
        }
...
std::vector<Entity *> World::getEntitiesInAABBexcluding(Entity *entityIn, AxisAlignedBB &boundingBox,
...
{
    return getNearestAttackablePlayer(entityIn.posX, entityIn.posY, entityIn.posZ, maxXZDistance, maxYDistance,
                                      std::nullopt, std::nullopt);
}

EntityPlayer *World::getNearestAttackablePlayer(BlockPos &pos, double maxXZDistance, double maxYDistance)
{
    return getNearestAttackablePlayer((double)((float)pos.getx() + 0.5F), (double)((float)pos.gety() + 0.5F),
                                      (double)((float)pos.getz() + 0.5F), maxXZDistance, maxYDistance, std::nullopt,
                                      std::nullopt);
}
...
{
    uint64_t j2 = static_cast<uint64_t>(seedX) * 341873128712 + static_cast<uint64_t>(seedY) * 132897987541 +
                  getWorldInfo().getSeed() + static_cast<uint64_t>(seedZ);
    rand.setSeed(j2);
    return rand;
}
