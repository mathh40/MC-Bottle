...
WorldServer::WorldServer(MinecraftServer *server, ISaveHandler *saveHandlerIn, WorldInfo &info, int dimensionId,
                         Profiler &profilerIn)
    : World(saveHandlerIn, info, DimensionType::getById(dimensionId).createDimension(), profilerIn, false),
      server(server), entityTracker(this), playerChunkMap(this), chunkProvider(createChunkProvider()),
      worldTeleporter(this)
{
...
World *WorldServer::init()
{
    ...
    advancementManager =
        AdvancementManager(new File(new File(saveHandler.getWorldDirectory(), "data"), "advancements"));
...
void WorldServer::tick()
{
    ...
    {
        entitySpawner.findChunksForSpawning(this, spawnHostileMobs, spawnPeacefulMobs,
                                            worldInfo.getWorldTotalTime() % 400 == 0);
    }
...
std::optional<SpawnListEntry> WorldServer::getSpawnListEntryForTypeAt(EnumCreatureType creatureType, BlockPos &pos)
{
    ...
    return list != nullptr && !list.isEmpty() ? (Biome.SpawnListEntry)WeightedRandom.getRandomItem(rand, list)
                                              : std::nullopt;
}
...
bool WorldServer::isBlockTickPending(BlockPos &pos, Block *blockType) const
{
    ...
    auto ite = std::find_if(pendingTickListEntriesThisTick.begin(), pendingTickListEntriesThisTick.end(),
                            [&](const NextTickListEntry list) { return nextticklistentry == list; });
                        ...
                        Block.isEqualTo(iblockstate->getBlock(), nextticklistentry1.getBlock()))
                    {
                        ...
                        catch (Throwable var10)
                        {
                            CrashReport crashreport =
                                CrashReport.makeCrashReport(var10, "Exception while ticking a block");
...
void WorldServer::wakeAllPlayers()
{
    ...
    std::copy_if(playerEntities.begin(), playerEntities.end(), std::back_inserter(plysleep),
                 [](const EntityPlayer *a) { return a->isPlayerSleeping(); });
        ...
        for (Iterator iterator = playerChunkMap.getChunkIterator(); iterator.hasNext(); profiler.endSection())
        {
            ...
            if (flag && flag1 && rand(100000) == 0)
            {
                ...
                if (isRainingAt(blockpos1))
                {
                    ...
                    if (getGameRules().getBoolean("doMobSpawning") &&
                        rand.nextDouble() < (double)difficultyinstance.getAdditionalDifficulty() * 0.01D)
                    {
                        EntitySkeletonHorse entityskeletonhorse = new EntitySkeletonHorse(this);
                        entityskeletonhorse.setTrap(true);
                        entityskeletonhorse.setGrowingAge(0);
                        entityskeletonhorse.setPosition((double)blockpos1.getX(), (double)blockpos1.getY(),
                                                        (double)blockpos1.getZ());
                        spawnEntity(entityskeletonhorse);
                        addWeatherEffect(new EntityLightningBolt(
                            this, (double)blockpos1.getX(), (double)blockpos1.getY(), (double)blockpos1.getZ(), true));
                    }
                    else
                    {
                        addWeatherEffect(new EntityLightningBolt(
                            this, (double)blockpos1.getX(), (double)blockpos1.getY(), (double)blockpos1.getZ(), false));
                    }
                }
            }
...
BlockPos WorldServer::adjustPosToNearbyEntity(BlockPos &pos)
{
    ...
    AxisAlignedBB axisalignedbb =
        (AxisAlignedBB(blockpos, BlockPos(blockpos.getx(), getHeight(), blockpos.getz()))).grow(3.0);
...
void WorldServer::tickPlayers()
{
    ...
    for (auto entity : playerEntities)
    {
        ...
        if (entity->isDead)
        {
            ...
            loadedEntityList.erase(std::remove_if(loadedEntityList.begin(), loadedEntityList.end(),
                                                  [](const Entity *lhs) { return lhs == entity; }));
...
{
    if (!canSpawnAnimals() &&
        (Util:: instanceof <EntityAnimal>(entityIn) || Util:: instanceof <EntityWaterMob>(entityIn)))
    {
        entityIn->setDead();
    }
...
void WorldServer::updateWeather()
{
    ...
    {
        server.getPlayerList().sendPacketToAllPlayersInDimension(SPacketChangeGameState(7, rainingStrength),
                                                                 provider->getDimensionType().getId());
    }

    if (prevThunderingStrength != thunderingStrength)
    {
        server.getPlayerList().sendPacketToAllPlayersInDimension(SPacketChangeGameState(8, thunderingStrength),
                                                                 provider->getDimensionType().getId());
    }
...
bool WorldServer::canAddEntity(Entity *entityIn)
{
    ...
    else
    {
        ...
        if (ite != entitiesByUuid.end())
        {
            ...
            auto findentity = std::find_if(unloadedEntityList.begin(), unloadedEntityList.end(),
                                           [&](const Entity *lhs) { return lhs == entity });
            ...
            else
            {
                ...
                {
                    LOGGER->warn("Keeping entity {} that already exists with UUID {}", EntityList.getKey(entity),
                                 uuid.toString());
                    return false;
                }
...
bool WorldServer::addWeatherEffect(Entity *entityIn)
{
    ...
    {
        server.getPlayerList().sendToAllNearExcept(nullptr, entityIn.posX, entityIn.posY, entityIn.posZ, 512.0,
                                                   provider->getDimensionType().getId(),
                                                   SPacketSpawnGlobalEntity(entityIn));
        return true;
    }
    else
    {
        ...
        while (var2.hasNext())
        {
            ...
            if (fireBlockEvent(blockeventdata))
            {
                server.getPlayerList().sendToAllNearExcept(
                    (EntityPlayer)null, (double)blockeventdata.getPosition().getX(),
                    (double)blockeventdata.getPosition().getY(), (double)blockeventdata.getPosition().getZ(), 64.0D,
    ...
    for (auto entityplayer : playerEntities)
    {
        if (entityplayer.getDistanceSq(x, y, z) < 4096.0)
        {
            ((EntityPlayerMP)entityplayer)
                .connection.sendPacket(SPacketExplosion(x, y, z, strength, explosion.getAffectedBlockPositions(),
...
bool WorldServer::fireBlockEvent(BlockEventData event)
{
    ...
    return iblockstate.getBlock() == event.getBlock()
               ? iblockstate.onBlockEventReceived(this, event.getPosition(), event.getEventID(),
...
void WorldServer::spawnParticle(EnumParticleTypes particleType, double xCoord, double yCoord, double zCoord,
                                int32_t numberOfParticles, double xOffset, double yOffset, double zOffset,
                                double particleSpeed, std::initializer_list<int32_t> particleArguments)
{
    spawnParticle(particleType, false, xCoord, yCoord, zCoord, numberOfParticles, xOffset, yOffset, zOffset,
...
void WorldServer::spawnParticle(EnumParticleTypes particleType, bool longDistance, double xCoord, double yCoord,
                                double zCoord, int32_t numberOfParticles, double xOffset, double yOffset,
                                double zOffset, double particleSpeed, std::initializer_list<int32_t> particleArguments)
{
    SPacketParticles spacketparticles(particleType, longDistance, (float)xCoord, (float)yCoord, (float)zCoord,
                                      (float)xOffset, (float)yOffset, (float)zOffset, (float)particleSpeed,
...
void WorldServer::spawnParticle(EntityPlayerMP *player, EnumParticleTypes particle, bool longDistance, double x,
                                double y, double z, int32_t count, double xOffset, double yOffset, double zOffset,
                                double speed, std::initializer_list<int32_t> particleArguments)
{
    SPacketParticles packet(particle, longDistance, (float)x, (float)y, (float)z, (float)xOffset, (float)yOffset,
                            (float)zOffset, (float)speed, count, particleArguments);
...
void WorldServer::sendPacketWithinDistance(EntityPlayerMP *player, bool longDistance, double x, double y, double z,
                                           Packet &packetIn)
{
