...
IChunkGenerator& WorldProvider::createChunkGenerator()
{
	...
	{
		return ChunkGeneratorFlat(world, world->getSeed(), world->getWorldInfo().isMapFeaturesEnabled(), generatorSettings);
	}
	else if (terrainType == WorldType::DEBUG_ALL_BLOCK_STATES) 
	{
		...
		return terrainType == WorldType::CUSTOMIZED ? ChunkGeneratorOverworld(world, world->getSeed(), world->getWorldInfo().isMapFeaturesEnabled(), generatorSettings) : ChunkGeneratorOverworld(world, world->getSeed(), world->getWorldInfo().isMapFeaturesEnabled(), generatorSettings);
	}
}
