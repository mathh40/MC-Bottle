	...
	if (i > 4096L) 
	{
		...
		if (getIslandHeightValue(x, z, 1, 1) > 40.0F) 
		{
			...
			for (auto l1 = 0; l1 < j; ++l1) 
			{
				...
				{
					auto k1 = k2 - 1;
					if (world->isAirBlock(blockpos.add(i2, k1 + 1, j2)) && world->getBlockState(blockpos.add(i2, k1, j2)).getBlock() == Blocks::END_STONE) 
					{
...
std::optional<BlockPos> ChunkGeneratorEnd::getNearestStructurePos(World* worldIn, std::string_view structureName, BlockPos& position,
	bool findUnexplored)
{
	return "EndCity" == structureName ? endCityGen.getNearestStructurePos(worldIn, position, findUnexplored) : std::nullopt;
}
...
std::vector<double> ChunkGeneratorEnd::getHeights(std::vector<double>& p_185963_1_, int32_t p_185963_2_, int32_t p_185963_3_, int32_t p_185963_4_, int32_t p_185963_5_, int32_t p_185963_6_, int32_t p_185963_7_)
{
	...
	auto d1 = 684.412;
	d0 *= 2.0;
	pnr = perlinNoise1.generateNoiseOctaves(pnr, p_185963_2_, p_185963_3_, p_185963_4_, p_185963_5_, p_185963_6_, p_185963_7_, d0 / 80.0, 4.277575000000001, d0 / 80.0);
	ar = lperlinNoise1.generateNoiseOctaves(ar, p_185963_2_, p_185963_3_, p_185963_4_, p_185963_5_, p_185963_6_, p_185963_7_, d0, 684.412, d0);
	br = lperlinNoise2.generateNoiseOctaves(br, p_185963_2_, p_185963_3_, p_185963_4_, p_185963_5_, p_185963_6_, p_185963_7_, d0, 684.412, d0);
	...
	int32_t p_185960_4_)
{
	...
	for (auto i = -12; i <= 12; ++i)
	{
		for (auto j = -12; j <= 12; ++j)
		{
			...
			{
				auto f3 = MathHelper::positiveModulo(MathHelper::abs((float)k) * 3439.0F + MathHelper::abs((float)l) * 147.0F ,13.0F) + 9.0F;
