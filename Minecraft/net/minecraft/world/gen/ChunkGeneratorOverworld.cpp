...
bool ChunkGeneratorOverworld::isInsideStructure(World* worldIn, std::string_view structureName, BlockPos& pos)
{
	...
	{
		return "Temple" == (structureName) && scatteredFeatureGenerator != nullptr ? scatteredFeatureGenerator.isInsideStructure(pos) : false;
	}
}
	...
	BlockPos& position, bool findUnexplored)
{
	...
	{
		return "Temple" == (structureName) && scatteredFeatureGenerator != nullptr ? scatteredFeatureGenerator.getNearestStructurePos(worldIn, position, findUnexplored) : std::nullopt;
	}
}
...
void ChunkGeneratorOverworld::populate(int32_t x, int32_t z)
{
	...
	if (biome != Biomes::DESERT && biome != Biomes::DESERT_HILLS && settings.useWaterLakes && !flag && rand(settings.waterLakeChance) == 0) 
	{
		auto k2 = rand(16) + 8;
		auto j3 = rand(256);
		auto l3 = rand(16) + 8;
