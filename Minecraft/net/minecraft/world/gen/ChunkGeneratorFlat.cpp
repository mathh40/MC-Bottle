...
ChunkGeneratorFlat::ChunkGeneratorFlat(World* worldIn, int64_t seed, bool generateStructures,
	...
	:world(worldIn), random(seed), flatWorldGenInfo(FlatGeneratorInfo::createFlatGeneratorFromString(flatGeneratorSettings))
{
...
std::optional<BlockPos> ChunkGeneratorFlat::getNearestStructurePos(World* worldIn, std::string_view structureName, BlockPos& position,
	bool findUnexplored)
{
	auto mapgenstructure = structureGenerators.get(structureName);
	return mapgenstructure != nullptr ? mapgenstructure.getNearestStructurePos(worldIn, position, findUnexplored) : std::nullopt;
}
