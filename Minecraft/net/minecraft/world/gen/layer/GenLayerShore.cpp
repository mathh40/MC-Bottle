...
GenLayerShore::GenLayerShore(int64_t p_i2130_1_, std::shared_ptr<GenLayer> p_i2130_3_) : GenLayer(p_i2130_1_)
{
    parent = p_i2130_3_;
}
    ...
    for (auto i = 0; i < areaHeight; ++i)
    {
        for (auto j = 0; j < areaWidth; ++j)
        {
            ...
            if (k == Biome::getIdForBiome(Biomes::MUSHROOM_ISLAND))
            {
                ...
                if (l != Biome::getIdForBiome(Biomes::OCEAN) && i1 != Biome::getIdForBiome(Biomes::OCEAN) &&
                    j1 != Biome::getIdForBiome(Biomes::OCEAN) && k1 != Biome::getIdForBiome(Biomes::OCEAN))
                {
                     ...
                     k != Biome::getIdForBiome(Biomes::EXTREME_HILLS_EDGE))
            {
                ...
                else if (k != Biome::getIdForBiome(Biomes::MESA) && k != Biome::getIdForBiome(Biomes::MESA_ROCK))
                {
                    if (k != Biome::getIdForBiome(Biomes::OCEAN) && k != Biome::getIdForBiome(Biomes::DEEP_OCEAN) &&
                        k != Biome::getIdForBiome(Biomes::RIVER) && k != Biome::getIdForBiome(Biomes::SWAMPLAND))
                    {
                        auto l  = aint[j + 1 + (i + 1 - 1) * (areaWidth + 2)];
                        auto i1 = aint[j + 1 + 1 + (i + 1) * (areaWidth + 2)];
                        auto j1 = aint[j + 1 - 1 + (i + 1) * (areaWidth + 2)];
...
void GenLayerShore::replaceIfNeighborOcean(const std::vector<int32_t> p_151632_1_, std::vector<int32_t> &p_151632_2_,
                                           int32_t p_151632_3_, int32_t p_151632_4_, int32_t p_151632_5_,
