...
{
	for (auto iblockstate = worldIn->getBlockState(position); (iblockstate->getMaterial() == Material::AIR || iblockstate->
		     getMaterial() == Material::LEAVES) && position.gety() > 1; iblockstate = worldIn->getBlockState(position))
	{
	...
	if (position.gety() < 1) 
	{
		...
		for (auto i = 0; i < 4; ++i) 
		{
			...
			if (worldIn->isAirBlock(blockpos) && worldIn->getBlockState(blockpos.down()).isTopSolid()) 
			{
				...
				{
					((TileEntityChest)tileentity)->setLootTable(LootTableList.CHESTS_SPAWN_BONUS_CHEST, MathHelper::nextLong(rand));
				}
