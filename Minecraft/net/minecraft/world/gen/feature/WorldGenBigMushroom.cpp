...
WorldGenBigMushroom::WorldGenBigMushroom(Block* p_i46449_1_)
	:WorldGenerator(true), mushroomType (p_i46449_1_)
{

}

WorldGenBigMushroom::WorldGenBigMushroom()
	: WorldGenerator(false), mushroomType(nullptr)
{
}
	...
	if (position.gety() >= 1 && position.gety() + i + 1 < 256) 
	{
		for (auto j = position.gety(); j <= position.gety() + 1 + i; ++j)
		{
			...
			for (auto j3 = position.getx() - k; j3 <= position.getx() + k && flag; ++j3)
			{
				for (auto k3 = position.getz() - k; k3 <= position.getz() + k && flag; ++k3)
				{
					if (j >= 0 && j < 256) 
					{
						auto material = worldIn->getBlockState(blockpos$mutableblockpos.setPos(j3, j, k3)).getMaterial();
		...
		if (!flag) 
		{
			...
			for (auto l2 = 0; l2 < i; ++l2) 
			{
				...
				{
					setBlockAndNotifyAdequately(worldIn, position.up(l2), block->getDefaultState().withProperty(BlockHugeMushroom::VARIANT, BlockHugeMushroom::EnumType::STEM));
				}
			}
