...
bool WorldGenShrub::generate(World* worldIn, pcg32& rand, const BlockPos& positionin)
{
	...
	for (auto iblockstate = worldIn->getBlockState(position); (iblockstate->getMaterial() == Material::AIR || iblockstate->getMaterial() == Material::LEAVES) && position.gety() > 0; iblockstate = worldIn->getBlockState(position)) 
	{
