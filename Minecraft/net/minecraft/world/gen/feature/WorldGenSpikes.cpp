...
WorldGenSpikes::EndSpike::EndSpike(int32_t centerXIn, int32_t centerZIn, int32_t radiusIn, int32_t heightIn,
    ...
    : centerX(centerXIn), centerZ(centerZIn), radius(radiusIn), height(heightIn), guarded(guardedIn),
      topBoundingBox((double)(centerXIn - radiusIn), 0.0, (double)(centerZIn - radiusIn),
                     (double)(centerXIn + radiusIn), 256.0, (double)(centerZIn + radiusIn))
{
}
...
bool WorldGenSpikes::generate(World *worldIn, pcg32 &rand, const BlockPos &position)
{
	...
	while (true)
	{
        ...
        {
            if (blockpos$mutableblockpos.distanceSq((double)position.getx(), (double)blockpos$mutableblockpos.gety(),
                                                    (double)position.getz()) <= (double)(i * i + 1) &&
                blockpos$mutableblockpos.gety() < spike.getHeight())
            {
                ...
                for (auto k = -2; k <= 2; ++k)
                {
                    if (MathHelper::abs(j) == 2 || MathHelper::abs(k) == 2)
                    {
                        setBlockAndNotifyAdequately(
                            worldIn, BlockPos(position.getx() + j, spike.getHeight(), position.getz() + k),
                            Blocks::IRON_BARS.getDefaultState());
                        setBlockAndNotifyAdequately(
                            worldIn, BlockPos(position.getx() + j, spike.getHeight() + 1, position.getz() + k),
                            Blocks::IRON_BARS.getDefaultState());
                        setBlockAndNotifyAdequately(
                            worldIn, BlockPos(position.getx() + j, spike.getHeight() + 2, position.getz() + k),
                            Blocks::IRON_BARS.getDefaultState());
                    }

                    setBlockAndNotifyAdequately(
                        worldIn, BlockPos(position.getx() + j, spike.getHeight() + 3, position.getz() + k),
                        Blocks::IRON_BARS.getDefaultState());
                }
            }
        }
        ...
        entityendercrystal.setLocationAndAngles(
            (double)((float)position.getx() + 0.5F), (double)(spike.getHeight() + 1),
            (double)((float)position.getz() + 0.5F), MathHelper::nextFloat(rand) * 360.0F, 0.0F);
        worldIn->spawnEntity(entityendercrystal);
        setBlockAndNotifyAdequately(worldIn, BlockPos(position.getx(), spike.getHeight(), position.getz()),
