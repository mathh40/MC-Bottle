...
IBlockState *WorldGenTaiga2::TRUNK =
    Blocks::LOG.getDefaultState().withProperty(BlockOldLog.VARIANT, BlockPlanks.EnumType.SPRUCE);
IBlockState *WorldGenTaiga2::LEAF = Blocks::LEAVES.getDefaultState()
                                        .withProperty(BlockOldLeaf.VARIANT, BlockPlanks.EnumType.SPRUCE)
                                        .withProperty(BlockLeaves.CHECK_DECAY, false);

WorldGenTaiga2::WorldGenTaiga2(bool notif) : WorldGenAbstractTree(notif)
{
}
        ...
        for (auto i1 = position.gety(); i1 <= position.gety() + 1 + i && flag; ++i1)
        {
            ...
            for (auto k1 = position.getx() - i3; k1 <= position.getx() + i3 && flag; ++k1)
            {
                for (auto l1 = position.getz() - i3; l1 <= position.getz() + i3 && flag; ++l1)
                {
                    ...
                    {
                        Material material =
                            worldIn->getBlockState(blockpos$mutableblockpos.setPos(k1, i1, l1)).getMaterial();
