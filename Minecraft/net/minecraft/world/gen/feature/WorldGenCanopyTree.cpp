...
IBlockState* WorldGenCanopyTree::DARK_OAK_LOG = Blocks.LOG2.getDefaultState().withProperty(BlockNewLog.VARIANT, BlockPlanks.EnumType.DARK_OAK);
IBlockState* WorldGenCanopyTree::DARK_OAK_LEAVES = Blocks.LEAVES2.getDefaultState().withProperty(BlockNewLeaf.VARIANT, BlockPlanks.EnumType.DARK_OAK).withProperty(BlockLeaves.CHECK_DECAY, false);

WorldGenCanopyTree::WorldGenCanopyTree(bool notify)
	...
	if (k >= 1 && k + i + 1 < 256) 
	{
		...
		else 
		{
			...
			for (auto k3 = -3; k3 <= 4; ++k3) 
			{
				...
				{
					if ((k3 != -3 || j4 != -3) && (k3 != -3 || j4 != 4) && (k3 != 4 || j4 != -3) && (k3 != 4 || j4 != 4) && (MathHelper::abs(k3) < 3 || MathHelper::abs(j4) < 3)) 
					{
