...
IBlockState* WorldGenTaiga1::TRUNK = Blocks.LOG.getDefaultState().withProperty(BlockOldLog.VARIANT, BlockPlanks.EnumType.SPRUCE);
IBlockState* WorldGenTaiga1::LEAF = Blocks.LEAVES.getDefaultState().withProperty(BlockOldLeaf.VARIANT, BlockPlanks.EnumType.SPRUCE).withProperty(BlockLeaves.CHECK_DECAY, false);
		...
		for (auto i1 = position.gety(); i1 <= position.gety() + 1 + i && flag; ++i1) 
		{
			...
			for (auto j3 = position.getx() - k2; j3 <= position.getx() + k2 && flag; ++j3) 
			{
				for (auto k3 = position.getz() - k2; k3 <= position.getz() + k2 && flag; ++k3) 
				{
					if (i1 >= 0 && i1 < 256) 
					{
						if (!canGrowInto(worldIn->getBlockState(blockpos$mutableblockpos.setPos(j3, i1, k3)).getBlock())) 
						{
							flag = false;
						}
					}
