...
IBlockState* WorldGenTrees::DEFAULT_TRUNK = Blocks::LOG.getDefaultState().withProperty(BlockOldLog.VARIANT, BlockPlanks.EnumType.OAK);
IBlockState* WorldGenTrees::DEFAULT_LEAF = Blocks::LEAVES.getDefaultState().withProperty(BlockOldLeaf.VARIANT, BlockPlanks.EnumType.OAK).withProperty(BlockLeaves.CHECK_DECAY, false);

WorldGenTrees::WorldGenTrees(bool notify)
:WorldGenTrees(notify, 4, DEFAULT_TRUNK, DEFAULT_LEAF, false)
{
}
		...
		for (auto j = position.gety(); j <= position.gety() + 1 + i; ++j) 
		{
			...
			for (auto l3 = position.getx() - k; l3 <= position.getx() + k && flag; ++l3)
			{
				for (auto i4 = position.getz() - k; i4 <= position.getz() + k && flag; ++i4)
				{
					if (j >= 0 && j < 256) 
					{
						if (!canGrowInto(worldIn->getBlockState(blockpos$mutableblockpos.setPos(l3, j, i4))->getBlock())) 
						{
							flag = false;
						}
					}
						...
						for (auto l3 = 0; l3 < 2; ++l3) 
						{
							for(auto enumfacing : Plane::HORIZONTAL)
							{
								if (rand(4 - l3) == 0) 
								{
									...
									placeCocoa(worldIn, rand(3), position.add(enumfacing1.getXOffset(), i - 5 + l3, enumfacing1.getZOffset()), enumfacing);
								}
							}
						}
					}
