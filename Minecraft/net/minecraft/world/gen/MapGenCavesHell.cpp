...
{
    addTunnel(p_180705_1_, p_180705_3_, p_180705_4_, p_180705_5_, p_180705_6_, p_180705_8_, p_180705_10_,
              1.0F + MathHelper::nextFloat(rand) * 6.0F, 0.0F, 0.0F, -1, -1, 0.5);
}

void MapGenCavesHell::addTunnel(int64_t p_180704_1_, int32_t p_180704_3_, int32_t p_180704_4_, ChunkPrimer &p_180704_5_,
                                double p_180704_6_, double p_180704_8_, double p_180704_10_, float p_180704_12_,
                                float p_180704_13_, float p_180704_14_, int32_t p_180704_15_, int32_t p_180704_16_,
                                double p_180704_17_)
{
    ...
    {
        auto d2 =
            1.5 + (double)(MathHelper::sin((float)p_180704_15_ * 3.1415927F / (float)p_180704_16_) * p_180704_12_);
        ...
        {
            addTunnel(MathHelper::nextLong(random), p_180704_3_, p_180704_4_, p_180704_5_, p_180704_6_, p_180704_8_,
                      p_180704_10_, MathHelper::nextLong(random) * 0.5F + 0.5F, p_180704_13_ - 1.5707964F,
                      p_180704_14_ / 3.0F, p_180704_15_, p_180704_16_, 1.0);
            addTunnel(MathHelper::nextLong(random), p_180704_3_, p_180704_4_, p_180704_5_, p_180704_6_, p_180704_8_,
                      p_180704_10_, MathHelper::nextLong(random) * 0.5F + 0.5F, p_180704_13_ + 1.5707964F,
                      p_180704_14_ / 3.0F, p_180704_15_, p_180704_16_, 1.0);
            return;
        ...
        if (flag1 || random(4) != 0)
        {
            ...
            if (p_180704_6_ >= d0 - 16.0 - d2 * 2.0 && p_180704_10_ >= d1 - 16.0 - d2 * 2.0 &&
                p_180704_6_ <= d0 + 16.0 + d2 * 2.0 && p_180704_10_ <= d1 + 16.0 + d2 * 2.0)
            {
...
void MapGenCavesHell::recursiveGenerate(World *worldIn, int32_t chunkX, int32_t chunkZ, int32_t originalX,
                                        int32_t originalZ, ChunkPrimer &chunkPrimerIn)
{
    ...
    for (auto j = 0; j < i; ++j)
    {
        ...
        for (auto l = 0; l < k; ++l)
        {
            ...
            addTunnel(MathHelper::nextLong(rand), originalX, originalZ, chunkPrimerIn, d0, d1, d2, f2 * 2.0F, f, f1, 0,
                      0, 0.5);
        }
    }
}
