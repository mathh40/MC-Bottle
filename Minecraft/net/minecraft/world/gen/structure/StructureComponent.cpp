...
void StructureComponent::buildComponent(StructureComponent* componentIn, const std::vector<StructureComponent*>& listIn, pcg32& rand)
{
}

StructureComponent::StructureComponent(int32_t type)
	: componentType(type)
{
}
...
StructureComponent* StructureComponent::findIntersecting(const std::vector<StructureComponent*>& listIn, std::optional<StructureBoundingBox> boundingboxIn)
{
	...
	while (structurecomponent->getBoundingBox() == nullptr || !structurecomponent->getBoundingBox().intersectsWith(boundingboxIn.value()));
...
void StructureComponent::setBlockState(World* worldIn, IBlockState* blockstateIn, int32_t x, int32_t y, int32_t z, StructureBoundingBox boundingboxIn)
{
...
IBlockState* StructureComponent::getBlockStateFromPos(World* worldIn, int32_t x, int32_t y, int32_t z, StructureBoundingBox boundingboxIn)
{
...
int32_t StructureComponent::getSkyBrightness(World* worldIn, int32_t x, int32_t y, int32_t z, StructureBoundingBox boundingboxIn)
{
	auto i = getXWithOffset(x, z);
...
void StructureComponent::fillWithAir(World* worldIn, StructureBoundingBox structurebb, int32_t minX, int32_t minY, int32_t minZ, int32_t maxX, int32_t maxY, int32_t maxZ)
{
...
void StructureComponent::fillWithBlocks(World* worldIn, StructureBoundingBox boundingboxIn, int32_t xMin, int32_t yMin, int32_t zMin, int32_t xMax, int32_t yMax, int32_t zMax, IBlockState* boundaryBlockState, IBlockState* insideBlockState, bool existingOnly)
{
...
void StructureComponent::fillWithRandomizedBlocks(World* worldIn, StructureBoundingBox boundingboxIn, int32_t minX, int32_t minY, int32_t minZ, int32_t maxX, int32_t maxY, int32_t maxZ, bool alwaysReplace, pcg32& rand, StructureComponent::BlockSelector blockselector)
{
	for (auto i = minY; i <= maxY; ++i) 
	{
		for (auto j = minX; j <= maxX; ++j) 
...
void StructureComponent::generateMaybeBox(World* worldIn, StructureBoundingBox sbb, pcg32& rand, float chance, int32_t x1, int32_t y1, int32_t z1, int32_t x2, int32_t y2, int32_t z2, IBlockState* edgeState, IBlockState* state, bool requireNonAir, int32_t requiredSkylight)
{
	for (auto i = y1; i <= y2; ++i) 
	{
		for (auto j = x1; j <= x2; ++j) 
		{
			...
			{
				if (MathHelper::nextFloat(rand) <= chance && (!requireNonAir || getBlockStateFromPos(worldIn, j, i, k, sbb)->getMaterial() != Material::AIR) && (requiredSkylight <= 0 || getSkyBrightness(worldIn, j, i, k, sbb) < requiredSkylight)) 
				{
...
void StructureComponent::randomlyPlaceBlock(World* worldIn, StructureBoundingBox boundingboxIn, pcg32& rand, float chance, int32_t x, int32_t y, int32_t z, IBlockState* blockstateIn)
{
...
void StructureComponent::randomlyRareFillWithBlocks(World* worldIn, StructureBoundingBox boundingboxIn, int32_t minX, int32_t minY, int32_t minZ, int32_t maxX, int32_t maxY, int32_t maxZ, IBlockState* blockstateIn, bool excludeAir)
{
...
void StructureComponent::clearCurrentPositionBlocksUpwards(World* worldIn, int32_t x, int32_t y, int32_t z, StructureBoundingBox structurebb)
{
	BlockPos blockpos(getXWithOffset(x, z), getYWithOffset(y), getZWithOffset(x, z));
...
void StructureComponent::replaceAirAndLiquidDownwards(World* worldIn, IBlockState* blockstateIn, int32_t x, int32_t y, int32_t z, StructureBoundingBox boundingboxIn)
{
	auto i = getXWithOffset(x, z);
	auto j = getYWithOffset(y);
	auto k = getZWithOffset(x, z);
	...
	{
		while ((worldIn->isAirBlock(BlockPos(i, j, k)) || worldIn->getBlockState(BlockPos(i, j, k))->getMaterial().isLiquid()) && j > 1) 
		{
...
bool StructureComponent::generateChest(World* worldIn, StructureBoundingBox structurebb, pcg32& randomIn, int32_t x, int32_t y, int32_t z, ResourceLocation loot)
{
...
bool StructureComponent::generateChest(World* p_191080_1_, StructureBoundingBox p_191080_2_, pcg32& p_191080_3_, BlockPos p_191080_4_, ResourceLocation p_191080_5_, IBlockState* p_191080_6_)
{
...
bool StructureComponent::createDispenser(World* worldIn, StructureBoundingBox sbb, pcg32& rand, int32_t x, int32_t y, int32_t z, EnumFacing facing, ResourceLocation lootTableIn)
{
	BlockPos blockpos(getXWithOffset(x, z), getYWithOffset(y), getZWithOffset(x, z));
...
void StructureComponent::generateDoor(World* worldIn, StructureBoundingBox sbb, pcg32& rand, int32_t x, int32_t y, int32_t z, EnumFacing facing, BlockDoor door)
{
