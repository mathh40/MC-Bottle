...
MapGenStronghold::MapGenStronghold()
	: structureCoords(128), distance(32.0), spread(3)
{
...
MapGenStronghold::MapGenStronghold(std::unordered_map<std::string, std::string> p_i2068_1_)
	:MapGenStronghold()
{
	...
	:StructureStart(chunkX, chunkZ)
{
	...
	StructureStrongholdPieces.Stairs2 structurestrongholdpieces$stairs2 = StructureStrongholdPieces.Stairs2(0, random, (chunkX << 4) + 2, (chunkZ << 4) + 2);
...
StructureStart MapGenStronghold::getStructureStart(int32_t chunkX, int32_t chunkZ)
{
	...
	for(mapgenstronghold$start = Start(world, rand, chunkX, chunkZ); mapgenstronghold$start.getComponents().empty() || ((Stairs2)mapgenstronghold$start.getComponents().get(0)).strongholdPortalRoom == nullptr; mapgenstronghold$start = Start(world, rand, chunkX, chunkZ)) 
	{
	}
...
void MapGenStronghold::generatePositions()
{
	...
	if (l < structureCoords.size()) 
	{
		...
		{
			double d0 = 4.0 * distance + distance * (double)j * 6.0 + (MathHelper::nextDouble(random) - 0.5) * distance * 2.5;
			int j1 = MathHelper::round(MathHelper::cos(d1) * d0);
			int k1 = MathHelper::round(MathHelper::sin(d1) * d0);
			auto blockpos = world->getBiomeProvider().findBiomePosition((j1 << 4) + 8, (k1 << 4) + 8, 112, allowedBiomes, random);
			if (blockpos.has_value()) 
			{
