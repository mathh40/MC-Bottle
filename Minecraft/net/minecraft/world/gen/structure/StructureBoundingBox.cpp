...
:minX(MathHelper::min(vec1.getx(), vec2.getx())),minY(MathHelper::min(vec1.gety(), vec2.gety())),minZ(MathHelper::min(vec1.getz(), vec2.getz())),maxX(MathHelper::max(vec1.getx(), vec2.getx())),maxY(MathHelper::max(vec1.gety(), vec2.gety())),maxZ(MathHelper::max(vec1.getz(), vec2.getz()))
{
}
...
{
	return StructureBoundingBox(std::numeric_limits<int32_t>::max(), std::numeric_limits<int32_t>::max(), std::numeric_limits<int32_t>::max(), std::numeric_limits<int32_t>::min(), std::numeric_limits<int32_t>::min(), std::numeric_limits<int32_t>::min());
}

StructureBoundingBox StructureBoundingBox::getComponentToAddBoundingBox(int32_t structureMinX, int32_t structureMinY,
	int32_t structureMinZ, int32_t xMin, int yMin, int32_t zMin, int32_t xMax, int32_t yMax, int32_t zMax,
	EnumFacing facing)
{
	switch(facing)
	{
      case EnumFacing::NORTH:
		  return StructureBoundingBox(structureMinX + xMin, structureMinY + yMin, structureMinZ - zMax + 1 + zMin, structureMinX + xMax - 1 + xMin, structureMinY + yMax - 1 + yMin, structureMinZ + zMin);
      case EnumFacing::SOUTH:
		  return StructureBoundingBox(structureMinX + xMin, structureMinY + yMin, structureMinZ + zMin, structureMinX + xMax - 1 + xMin, structureMinY + yMax - 1 + yMin, structureMinZ + zMax - 1 + zMin);
      case EnumFacing::WEST:
		  return StructureBoundingBox(structureMinX - zMax + 1 + zMin, structureMinY + yMin, structureMinZ + xMin, structureMinX + zMin, structureMinY + yMax - 1 + yMin, structureMinZ + xMax - 1 + xMin);
      case EnumFacing::EAST:
		  return StructureBoundingBox(structureMinX + zMin, structureMinY + yMin, structureMinZ + xMin, structureMinX + zMax - 1 + zMin, structureMinY + yMax - 1 + yMin, structureMinZ + xMax - 1 + xMin);
      default:
		  return StructureBoundingBox(structureMinX + xMin, structureMinY + yMin, structureMinZ + zMin, structureMinX + xMax - 1 + xMin, structureMinY + yMax - 1 + yMin, structureMinZ + zMax - 1 + zMin);
      }
}
...
StructureBoundingBox StructureBoundingBox::createProper(int32_t x1, int32_t y1, int32_t z1, int32_t x2, int32_t y2,
	int32_t z2)
{
	return StructureBoundingBox(MathHelper::min(x1, x2), MathHelper::min(y1, y2), MathHelper::min(z1, z2), MathHelper::max(x1, x2), MathHelper::max(y1, y2), MathHelper::max(z1, z2));
}
