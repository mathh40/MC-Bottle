                          ...
                          double p_180703_6_, double p_180703_8_, double p_180703_10_)
{
    ...
    addTunnel(p_180703_1_, p_180703_3_, p_180703_4_, p_180703_5_, p_180703_6_, p_180703_8_, p_180703_10_,
              1.0F + dist(rand) * 6.0F, 0.0F, 0.0F, -1, -1, 0.5);
}

void MapGenCaves::addTunnel(int64_t p_180702_1_, int32_t p_180702_3_, int32_t p_180702_4_, ChunkPrimer &p_180702_5_,
                            double p_180702_6_, double p_180702_8_, double p_180702_10_, float p_180702_12_,
                            float p_180702_13_, float p_180702_14_, int32_t p_180702_15_, int32_t p_180702_16_,
                            double p_180702_17_)
{
    ...
    {
        auto d2 =
            1.5 + (double)(MathHelper::sin((float)p_180702_15_ * 3.1415927F / (float)p_180702_16_) * p_180702_12_);
        ...
        {
            addTunnel(MathHelper::nextLong(random), p_180702_3_, p_180702_4_, p_180702_5_, p_180702_6_, p_180702_8_,
                      p_180702_10_, dist(random) * 0.5F + 0.5F, p_180702_13_ - 1.5707964F, p_180702_14_ / 3.0F,
        ...
        if (flag2 || random(4) != 0)
        {
            ...
            if (p_180702_6_ >= d0 - 16.0 - d2 * 2.0 && p_180702_10_ >= d1 - 16.0 - d2 * 2.0 &&
                p_180702_6_ <= d0 + 16.0 + d2 * 2.0 && p_180702_10_ <= d1 + 16.0 + d2 * 2.0)
            {
                            ...
                            if (d10 * d10 + d8 * d8 < 1.0)
                            {
                                for (auto j2 = l; j2 > l2; --j2)
                                {
                                    ...
                                    if (d9 > -0.7 && d10 * d10 + d9 * d9 + d8 * d8 < 1.0)
                                    {
                                        ...
                                        auto iblockstate2 = (IBlockState)MoreObjects.firstNonNull(
                                            p_180702_5_.getBlockState(j3, j2 + 1, i2), BLK_AIR);
                                        ...
                                        if (canReplaceBlock(iblockstate1, iblockstate2))
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if (flag1 &&
                                                    p_180702_5_.getBlockState(j3, j2 - 1, i2).getBlock() == Blocks.DIRT)
                                                {
                                    ...
                                    int32_t originalZ, ChunkPrimer &chunkPrimerIn)
{
    ...
    for (int j = 0; j < i; ++j)
    {
        ...
        for (auto l = 0; l < k; ++l)
        {
            ...
            addTunnel(MathHelper::nextLong(rand), originalX, originalZ, chunkPrimerIn, d0, d1, d2, f2, f, f1, 0, 0,
                      1.0);
        }
    }
}
