...
DragonSpawnManager DragonSpawnManager::START([](WorldServer* worldIn, DragonFightManager& manager, std::vector<EntityEnderCrystal*> crystals, int32_t ticks, BlockPos& pos)
{
...
DragonSpawnManager DragonSpawnManager::PREPARING_TO_SUMMON_PILLARS([](WorldServer* worldIn, DragonFightManager& manager, std::vector<EntityEnderCrystal*> crystals, int32_t ticks, BlockPos& pos)
{
	if (ticks < 100) 
	{
...
DragonSpawnManager DragonSpawnManager::SUMMONING_PILLARS([](WorldServer* worldIn, DragonFightManager& manager, std::vector<EntityEnderCrystal*> crystals, int32_t ticks, BlockPos& pos)
{
	...
	if (flag || flag1) 
	{
		...
		if (j < aworldgenspikes$endspike.length) 
		{
			...
			else 
			{
				...
				BlockPos pos1(worldgenspikes$endspike.getCenterX() - 10, worldgenspikes$endspike.getHeight() - 10, worldgenspikes$endspike.getCenterZ() - 10);
				BlockPos pos2(worldgenspikes$endspike.getCenterX() + 10, worldgenspikes$endspike.getHeight() + 10, worldgenspikes$endspike.getCenterZ() + 10);
				for(auto blockpos$mutableblockpos : getAllInBoxMutable(pos1, pos2))
				{
					worldIn->setBlockToAir(blockpos$mutableblockpos);
				}

				worldIn->createExplosion(nullptr, (double)((float)worldgenspikes$endspike.getCenterX() + 0.5F), (double)worldgenspikes$endspike.getHeight(), (double)((float)worldgenspikes$endspike.getCenterZ() + 0.5F), 5.0F, true);
				...
				pcg32 rand;
				worldgenspikes.generate(worldIn, rand, BlockPos(worldgenspikes$endspike.getCenterX(), 45, worldgenspikes$endspike.getCenterZ()));
			}
		}
		else if (flag) 
		{
...
DragonSpawnManager DragonSpawnManager::SUMMONING_DRAGON([](WorldServer* worldIn, DragonFightManager& manager, std::vector<EntityEnderCrystal*> crystals, int32_t ticks, BlockPos& pos)
{
	if (ticks >= 100) 
	{
...
DragonSpawnManager DragonSpawnManager::END([](WorldServer* worldIn, DragonFightManager& manager, std::vector<EntityEnderCrystal*> crystals, int32_t ticks, BlockPos& pos)
{
});
