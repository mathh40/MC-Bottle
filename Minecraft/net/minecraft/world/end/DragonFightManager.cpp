...
auto VALID_PLAYER = [](const Entity &lhs) {
    return EntitySelectors::IS_ALIVE(lhs) && EntitySelectors::withinRange(lhs, 0.0, 128.0, 0.0, 192.0);
};
...
DragonFightManager::DragonFightManager(WorldServer *worldIn, NBTTagCompound *compound)
    : bossInfo(std::make_unique<TextComponentTranslation>("entity->EnderDragon.name"), BossInfo::Color::PINK,
...
BlockPattern.PatternHelper DragonFightManager::findExitPortal()
{
            ...
            for (auto tileentity : chunk.getTileEntityMap())
            {
                if (tileentity instanceof TileEntityEndPortal)
                {
                    BlockPattern.PatternHelper blockpattern$patternhelper =
                        portalPattern.match(world, tileentity.getPos());
    ...
    for (auto l = i; l >= 0; --l)
    {
        BlockPattern.PatternHelper blockpattern$patternhelper1 =
            this.portalPattern.match(this.world, new BlockPos(WorldGenEndPodium.END_PODIUM_LOCATION.getX(), l,
...
void DragonFightManager::generatePortal(bool active)
{
    ...
    {
        for (exitPortalLocation = world->getTopSolidOrLiquidBlock(WorldGenEndPodium.END_PODIUM_LOCATION).down();
             world->getBlockState(exitPortalLocation).getBlock() ==
             Blocks.BEDROCK exitPortalLocation.gety() > world->getSeaLevel();
...
void DragonFightManager::respawnDragon(std::vector<Entity *> crystalsIn)
{
    ...
    {
        for (BlockPattern::PatternHelper blockpattern$patternhelper         = findExitPortal();
             blockpattern$patternhelper != null; blockpattern$patternhelper = findExitPortal())
        {
...
void DragonFightManager::respawnDragon()
{
    if (dragonKilled && respawnState == nullptr)
    {
        ...
        {
            auto list =
                world->getEntitiesWithinAABB<EntityEnderCrystal>(AxisAlignedBB(blockpos1.offset(enumfacing, 2)));
            if (list.empty())
            {
                return;
            }
...
void DragonFightManager::resetSpikeCrystals()
{
    ...
    for (int var3 = 0; var3 < var2; ++var3)
    {
        ...
        Iterator var5 =
            world->getEntitiesWithinAABB<EntityEnderCrystal>(worldgenspikes$endspike.getTopBoundingBox()).iterator();
...
void DragonFightManager::processDragonDeath(EntityDragon *dragon)
{
    if (dragon->getUniqueID() == dragonUniqueId)
    {
        ...
        if (!previouslyKilled)
        {
            world->setBlockState(world->getHeight(WorldGenEndPodium::END_PODIUM_LOCATION),
