...
class World : public IBlockAccess
{
  public:
    ...
    std::optional<RayTraceResult> rayTraceBlocks(Vec3d &vec31, Vec3d &vec32, bool stopOnLiquid,
                                                 bool ignoreBlockWithoutBoundingBox, bool returnLastUncollidableBlock);
    void playSound(EntityPlayer *player, BlockPos pos, SoundEvent soundIn, SoundCategory category, float volume,
                   float pitch);
    void playSound(EntityPlayer *player, double x, double y, double z, SoundEvent soundIn, SoundCategory category,
                   float volume, float pitch);
    void playSound(double x, double y, double z, SoundEvent soundIn, SoundCategory category, float volume, float pitch,
                   bool distanceDelay);
    void playRecord(BlockPos &blockPositionIn, std::optional<SoundEvent> soundEventIn);

    void spawnParticle(EnumParticleTypes particleType, double xCoord, double yCoord, double zCoord, double xSpeed,
                       double ySpeed, double zSpeed, std::initializer_list<int32_t> parameters);
    void spawnAlwaysVisibleParticle(int32_t id, double x, double y, double z, double xSpeed, double ySpeed,
                                    double zSpeed, std::initializer_list<int32_t> parameters);
    void spawnParticle(EnumParticleTypes particleType, bool ignoreRange, double xCoord, double yCoord, double zCoord,
    ...
    virtual Explosion newExplosion(Entity *entityIn, double x, double y, double z, float strength, bool causesFire,
                                   bool damagesTerrain);
    ...
    std::vector<Entity *> getEntitiesInAABBexcluding(Entity *entityIn, AxisAlignedBB &boundingBox,
                                                     std::function<bool(Entity *)> predicate);
    template <class Class, typename Predicate> std::vector<Class *> getEntities(Predicate filter);
    template <class Class, typename Predicate> std::vector<Class *> getPlayers(Predicate filter);
    template <class Class> std::vector<Class *> getEntitiesWithinAABB(const AxisAlignedBB &bb);
    template <class Class, typename Predicate>
    std::vector<Class *> getEntitiesWithinAABB(const AxisAlignedBB &aabb, Predicate filter);
    template <class Class> Entity *findNearestEntityWithinAABB(AxisAlignedBB &aabb, Entity *closestTo);
    ...
    template <class Class> int32_t countEntities();
    ...
    EntityPlayer *getNearestAttackablePlayer(double posX, double posY, double posZ, double maxXZDistance,
                                             double maxYDistance, std::optional<Function> playerToDouble,
                                             Predicate predicate);
    ...
    template <class Class> WorldSavedData *loadData(std::string_view dataID);
  ...
  private:
    ...
    bool isAreaLoaded(int32_t xStart, int32_t yStart, int32_t zStart, int32_t xEnd, int32_t yEnd, int32_t zEnd,
                      bool allowEmpty);
    void spawnParticle(int32_t particleID, bool ignoreRange, double xCood, double yCoord, double zCoord, double xSpeed,
                       double ySpeed, double zSpeed, std::initializer_list<int32_t> parameters);
    bool getCollisionBoxes(Entity *entityIn, AxisAlignedBB aabb, bool p_191504_3_,
...
template <class Class, class Predicate> std::vector<Class *> World::getEntities(Predicate filter)
{
...
template <class Class, typename Predicate> std::vector<Class *> World::getPlayers(Predicate filter)
{
    std::vector<Entity *> list;
...
template <class Class> std::vector<Class *> World::getEntitiesWithinAABB(const AxisAlignedBB &bb)
{
...
template <class Class> Entity *World::findNearestEntityWithinAABB(AxisAlignedBB &aabb, Entity *closestTo)
{
...
template <class Class> int32_t World::countEntities()
{
    auto j2      = 0;
                                                ...
                                                Predicate predicate)
{
    ...
    for (auto entityplayer1 : playerEntities)
    {
        if (!entityplayer1->capabilities.disableDamage && entityplayer1->isEntityAlive() &&
            !entityplayer1->isSpectator() && (!predicate || predicate(entityplayer1)))
        {
            ...
            if ((maxYDistance < 0.0 || MathHelper::abs(entityplayer1->posY - posY) < maxYDistance * maxYDistance) &&
                (maxXZDistance < 0.0 || d1 < d2 * d2) && (d0 == -1.0 || d1 < d0))
            {
                d0           = d1;
                entityplayer = entityplayer1;
            }
...
template <class Class> WorldSavedData *World::loadData(std::string_view dataID)
{
