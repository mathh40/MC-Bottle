...
Explosion::Explosion(World* worldIn, Entity* entityIn, double x, double y, double z, float size, bool causesFire,
	bool damagesTerrain)
	:world(worldIn), exploder(entityIn), size(size),x(x),y(y),z(z), causesFire(causesFire), damagesTerrain(damagesTerrain)
{
}
			...
			for (auto l = 0; l < 16; ++l) 
			{
				if (j == 0 || j == 15 || k == 0 || k == 15 || l == 0 || l == 15) 
				{
					...
					for (float var21 = 0.3F; f > 0.0F; f -= 0.22500001F) {
						...
						{
							float f2 = exploder != nullptr ? exploder->getExplosionResistance(this, world, blockpos, iblockstate) : iblockstate->getBlock()->getExplosionResistance(std::nullopt);
							f -= (f2 + 0.3F) * 0.3F;
						}

						if (f > 0.0F && (exploder == nullptr || exploder->canExplosionDestroyBlock(this, world, blockpos, iblockstate, f))) 
						{
...
EntityLivingBase* Explosion::getExplosivePlacedBy()
{
	...
	else if (Util::instanceof<EntityTNTPrimed>(exploder)) 
	{
		...
		return Util::instanceof <EntityLivingBase>(exploder) ? reinterpret_cast<EntityLivingBase*>(exploder) : nullptr;
	}
}
