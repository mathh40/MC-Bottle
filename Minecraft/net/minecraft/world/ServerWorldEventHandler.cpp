...
{
	server.getPlayerList().sendToAllNearExcept(player, x, y, z, volume > 1.0F ? (16.0F * volume) : 16.0, world.provider.getDimensionType().getId(),SPacketSoundEffect(soundIn, category, x, y, z, volume, pitch));
}
...
void ServerWorldEventHandler::spawnParticle(int32_t particleID, bool ignoreRange, double xCoord, double yCoord, double zCoord, double xSpeed, double ySpeed, double zSpeed, std::initializer_list<int32_t> parameters)
{
}

void ServerWorldEventHandler::spawnParticle(int32_t id, bool ignoreRange, bool minimiseParticleLevel, double x,
	double y, double z, double xSpeed, double ySpeed, double zSpeed, std::initializer_list<int32_t> parameters)
{
}
...
{
	server.getPlayerList().sendToAllNearExcept(player, blockPosIn.getx(), blockPosIn.gety(), blockPosIn.getz(), 64.0, world.provider.getDimensionType().getId(), SPacketEffect(type, blockPosIn, data, false));
}
