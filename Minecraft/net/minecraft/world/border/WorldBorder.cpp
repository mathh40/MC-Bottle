...
{
    return (double)(pos.getx() + 1) > minX() && (double)pos.getx() < maxX() && (double)(pos.getz() + 1) > minZ() &&
           (double)pos.getz() < maxZ();
}

bool WorldBorder::contains(ChunkPos &range)
{
    return (double)range.getXEnd() > minX() && (double)range.getXStart() < maxX() && (double)range.getZEnd() > minZ() &&
           (double)range.getZStart() < maxZ();
}
