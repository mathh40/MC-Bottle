...
IBlockState* BiomeMesa::COARSE_DIRT = Blocks.DIRT.getDefaultState().withProperty(BlockDirt.VARIANT, BlockDirt.DirtType.COARSE_DIRT);
...
IBlockState* BiomeMesa::ORANGE_STAINED_HARDENED_CLAY = STAINED_HARDENED_CLAY.withProperty(BlockColored.COLOR, EnumDyeColor.ORANGE);
IBlockState* BiomeMesa::RED_SAND = Blocks.SAND.getDefaultState().withProperty(BlockSand.VARIANT, BlockSand.EnumType.RED_SAND);
...
BiomeMesa::BiomeMesa(bool p_i46704_1_, bool p_i46704_2_, BiomeProperties properties)
	:Biome(properties)
{
...
{
	auto i = MathHelper::round(clayBandsOffsetNoise.getValue((double)p_180629_1_ / 512.0, (double)p_180629_1_ / 512.0) * 2.0);
...
void BiomeMesa::genTerrainBlocks(World* worldIn, pcg32& rand, ChunkPrimer chunkPrimerIn, int32_t x, int32_t z, double noiseVal)
{
	...
	if (brycePillars) 
	{
		auto k1 = (x & -16) + (z & 15);
		auto l1 = (z & -16) + (x & 15);
		auto d0 = MathHelper::min(MathHelper::abs(noiseVal), pillarNoise.getValue((double)k1 * 0.25, (double)l1 * 0.25));
		if (d0 > 0.0) 
		{
			auto d1 = 0.001953125;
