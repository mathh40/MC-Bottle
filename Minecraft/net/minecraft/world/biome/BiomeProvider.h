...
class BiomeProvider
{
  public:
    ...
    virtual std::vector<Biome *> getBiomesForGeneration(std::vector<Biome *> biomes, int32_t x, int32_t z,
                                                        int32_t width, int32_t height);
    virtual std::vector<Biome *> getBiomes(std::vector<Biome *> oldBiomeList, int32_t x, int32_t z, int32_t width,
                                           int32_t depth);
    virtual std::vector<Biome *> getBiomes(std::vector<Biome *> listToReuse, int32_t x, int32_t z, int32_t width,
                                           int32_t length, bool cacheFlag);
    virtual bool areBiomesViable(int32_t x, int32_t z, int32_t radius, std::vector<Biome *> allowed);
    virtual std::optional<BlockPos> findBiomePosition(int32_t x, int32_t z, int32_t range, std::vector<Biome *> biomes,
                                                      pcg32 &random);
    void cleanupCache();
