...
Chunk::Chunk(World* worldIn, int32_t xIn, int32_t zIn)
	:queuedLightChecks(4096), world(worldIn),x(xIn),z(zIn)
{
...
Chunk::Chunk(World* worldIn, ChunkPrimer primer, int32_t x, int32_t z)
	:Chunk(worldIn, x, z)
{
...
TileEntity* Chunk::createNewTileEntity(BlockPos& pos)
{
	...
	return !block->hasTileEntity() ? nullptr : block->createNewTileEntity(world, iblockstate->getBlock()->getMetaFromState(iblockstate));
}
...
void Chunk::onTick(bool skipRecheckGaps)
{
	...
	{
		if (getTileEntity(blockpos, Chunk.EnumCreateEntityType.CHECK) == nullptr && getBlockState(blockpos).getBlock().hasTileEntity()) 
		{
...
void Chunk::setStorageArrays(std::array<ExtendedBlockStorage*, 16> newStorageArrays)
{
	...
	{
		LOGGER->warn("Could not set level chunk sections, array length is {} instead of {}", newStorageArrays.size(), storageArrays.size());
	}
	else 
	{
...
void Chunk::setBiomeArray(std::array<unsigned char, 256> biomeArray)
{
	...
	{
		LOGGER->warn("Could not set level chunk biomes, array length is {} instead of {}", biomeArray.size(), blockBiomeArray.size());
	}
	else 
	{
...
void Chunk::setHeightMap(std::array<int32_t, 256> newHeightMap)
{
	...
	{
		LOGGER->warn("Could not set level chunk heightmap, array length is {} instead of {}", newHeightMap.size(), heightMap.size());
	}
	else 
	{
		...
		for (auto i = 0; i < 16; ++i) 
		{
			for (auto j = 0; j < 16; ++j)
			{
				if (updateSkylightColumns[i + j * 16]) 
				{
					...
					for (var8 = Plane::HORIZONTAL.iterator(); var8.hasNext(); j1 = MathHelper::min(j1, world->getChunksLowestHorizon(l + enumfacing1.getXOffset(), i1 + enumfacing1.getZOffset()))) 
					{
