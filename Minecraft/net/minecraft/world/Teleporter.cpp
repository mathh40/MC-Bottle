...
Teleporter::Teleporter(WorldServer& worldIn)
	:world(worldIn),random(worldIn.getSeed())
{	
}
		...
		for (auto j1 = -2; j1 <= 2; ++j1) {
			for (auto k1 = -2; k1 <= 2; ++k1) {
				for (auto l1 = -1; l1 < 3; ++l1) {
					...
					auto k2 = k + k1 * 0 - j1 * 1;
					bool flag = l1 < 0;
					world.setBlockState(BlockPos(i2, j2, k2), flag ? Blocks.OBSIDIAN.getDefaultState() : Blocks.AIR.getDefaultState());
				}
			}
		}
...
bool Teleporter::placeInExistingPortal(Entity* entityIn, float rotationYaw)
{
	...
	if (d0 >= 0.0) 
	{
		...
		auto d2 = blockpattern$patternhelper.getForwards().getAxis() == Axis::X ? (double)blockpattern$patternhelper.getFrontTopLeft().getZ() : blockpattern$patternhelper.getFrontTopLeft().getX();
		auto d6 = (blockpattern$patternhelper.getFrontTopLeft().getY() + 1) - entityIn->getLastPortalVec().y * blockpattern$patternhelper.getHeight();
		if (flag1) 
		{
			++d2;
		}
		...
		{
			d7 = d2 + (1.0 - entityIn.getLastPortalVec().x) * blockpattern$patternhelper.getWidth() * blockpattern$patternhelper.getForwards().rotateY().getAxisDirection().getOffset();
		}
		else 
		{
			d5 = d2 + (1.0 - entityIn.getLastPortalVec().x) * blockpattern$patternhelper.getWidth() * blockpattern$patternhelper.getForwards().rotateY().getAxisDirection().getOffset();
		}
						...
						for (auto j10 = 0; j10 < 3; ++j10) 
						{
							for (auto j11 = 0; j11 < 4; ++j11) 
							{
								for (auto j12 = -1; j12 < 4; ++j12) 
								{
									...
									if (j12 < 0 && !world.getBlockState(mutableblockpos).getMaterial().isSolid() || j12 >= 0 && !world.isAirBlock(mutableblockpos)) 
									{
										continue label291;
									}
								}
		...
		for (auto j7 = -1; j7 <= 1; ++j7) 
		{
			for (auto i7 = 1; i7 < 3; ++i7) 
			{
				for (auto k7 = -1; k7 < 3; ++k7) 
				{
					...
					auto flag = k7 < 0;
					world.setBlockState(new BlockPos(j8, j9, j10), flag ? Blocks::OBSIDIAN.getDefaultState() : Blocks::AIR.getDefaultState());
				}
			}
		}
	}
	...
	for (auto i7 = 0; i7 < 4; ++i7) 
	{
		for (auto k7 = 0; k7 < 4; ++k7) 
		{
			for (auto j8 = -1; j8 < 4; ++j8)
			{
				...
				world.setBlockState(new BlockPos(j9, j10, j11), flag1 ? Blocks::OBSIDIAN.getDefaultState() : iblockstate, 2);
			}
		}
