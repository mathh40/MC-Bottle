...
public:
    ...
    void setByteArray(std::string key, ByteBuffer &value);
    ...
    ByteBuffer getByteArray(std::string key) const;
...
private:
    ...
    std::unique_ptr<NBTBase> readNBT(uint8_t id, std::string key, std::istream &input, int depth,
