...
void NBTTagCompound::setByteArray(std::string key, std::vector<uint8_t>& value)
{
...
std::vector<uint8_t> NBTTagCompound::getByteArray(std::string key)const
{
	if (hasKey(key, 7)) {
...
std::shared_ptr<NBTBase> NBTTagCompound::readNBT(uint8_t id, std::string key, std::istream & input, int depth, NBTSizeTracker sizeTracker)
{
