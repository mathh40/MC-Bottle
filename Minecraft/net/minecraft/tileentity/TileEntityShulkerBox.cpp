...
TileEntityShulkerBox::TileEntityShulkerBox() : TileEntityShulkerBox(std::nullopt)
{
}
...
{
    updateAnimation();
    if (animationStatus == TileEntityShulkerBox::AnimationStatus::OPENING ||
...
{
    return Block::FULL_BLOCK_AABB.expand((double)(0.5F * getProgress(1.0F) * (float)p_190587_1_.getXOffset()),
                                         (double)(0.5F * getProgress(1.0F) * (float)p_190587_1_.getYOffset()),
                                         (double)(0.5F * getProgress(1.0F) * (float)p_190587_1_.getZOffset()));
}
...
void TileEntityShulkerBox::openInventory(EntityPlayer *player)
{
    if (!player->isSpectator())
    {
        ...
        if (openCount == 1)
        {
            world->playSound(nullptr, pos, SoundEvents::BLOCK_SHULKER_BOX_OPEN, SoundCategory::BLOCKS, 0.5F,
                             MathHelper::nextFloat(world->rand) * 0.1F + 0.9F);
        }
    }
}
...
void TileEntityShulkerBox::closeInventory(EntityPlayer *player)
{
    if (!player->isSpectator())
    {
        ...
        if (openCount <= 0)
        {
            world->playSound(nullptr, pos, SoundEvents::BLOCK_SHULKER_BOX_CLOSE, SoundCategory::BLOCKS, 0.5F,
                             MathHelper::nextFloat(world->rand) * 0.1F + 0.9F);
        }
    }
}
...
AxisAlignedBB TileEntityShulkerBox::getTopBoundingBox(EnumFacing p_190588_1_)
{
    ...
    return getBoundingBox(p_190588_1_)
        ...
        if (!list.empty())
        {
            for (int i = 0; i < list.size(); ++i)
            {
                ...
                if (entity->getPushReaction() != EnumPushReaction::IGNORE)
                {
                    ...
                    entity->move(MoverType::SHULKER_BOX, d0 * (double)enumfacing.getXOffset(),
                                 d1 * (double)enumfacing.getYOffset(), d2 * (double)enumfacing.getZOffset());
                }
            }
        }
    }
