...
void TileEntityEndGateway::teleportEntity(Entity *entityIn)
{
    if (!world->isRemote && !isCoolingDown())
    {
        ...
        if (exitPortal.has_value)
        {
            ...
            entityIn->setPositionAndUpdate((double)blockpos.getx() + 0.5, (double)blockpos.gety() + 0.5,
...
void TileEntityEndGateway::findExitPortal()
{
    ...
    int var3;
    for (var3   = 16; getChunk(world, vec3d1).getTopFilledSegment() > 0 && var3-- > 0;
...
BlockPos TileEntityEndGateway::findHighestBlock(World *p_184308_0_, BlockPos p_184308_1_, int32_t p_184308_2_,
                                                bool p_184308_3_)
{
    BlockPos blockpos;
