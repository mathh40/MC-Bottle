...
TileEntityHopper::TileEntityHopper() : inventory(5, ItemStack::EMPTY), transferCooldown(-1)
{
}
...
bool TileEntityHopper::pullItems(IHopper *hopper)
{
    ...
    {
        Iterator var9 =
            getCaptureItems(hopper.getWorld(), hopper.getXPos(), hopper.getYPos(), hopper.getZPos()).iterator();
...
bool TileEntityHopper::pullItemFromSlot(IHopper *hopper, IInventory *inventoryIn, int32_t index, EnumFacing direction)
{
    ...
    if (!itemstack.isEmpty() && canExtractItemFromSlot(inventoryIn, itemstack, index, direction))
    {
        ...
        ItemStack itemstack2 =
            putStackInInventoryAllSlots(inventoryIn, hopper, inventoryIn.decrStackSize(index, 1), std::nullopt_t);
...
bool TileEntityHopper::canInsertItemInSlot(IInventory *inventoryIn, ItemStack stack, int32_t index, EnumFacing side)
{
    ...
    {
        return !(Util:: instanceof <ISidedInventory>(inventoryIn)) ||
...
IInventory *TileEntityHopper::getInventoryForHopperTransfer()
{
    ...
    return getInventoryAtPosition(getWorld(), getXPos() + (double)enumfacing.getXOffset(),
                                  getYPos() + (double)enumfacing.getYOffset(),
                                  getZPos() + (double)enumfacing.getZOffset());
}
...
std::vector<Entity *> TileEntityHopper::getCaptureItems(World *worldIn, double p_184292_1_, double p_184292_3_,
                                                        double p_184292_5_)
{
    return worldIn->getEntitiesWithinAABB<EntityItem>(AxisAlignedBB(p_184292_1_ - 0.5, p_184292_3_, p_184292_5_ - 0.5,
                                                                    p_184292_1_ + 0.5, p_184292_3_ + 1.5,
                                                                    p_184292_5_ + 0.5),
...
IInventory *TileEntityHopper::getInventoryAtPosition(World *worldIn, double x, double y, double z)
{
    ...
    if (iinventory == nullptr)
    {
        auto list = worldIn->getEntitiesInAABBexcluding(
            (Entity) nullptr, AxisAlignedBB(x - 0.5, y - 0.5, z - 0.5, x + 0.5, y + 0.5, z + 0.5),
