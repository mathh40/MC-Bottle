...
TileEntityBeacon::TileEntityBeacon() : payment(ItemStack::EMPTY)
{
}
...
void TileEntityBeacon::addEffectsToPlayers()
{
    if (isComplete && levels > 0 && !world->isRemote && primaryEffect.has_value())
    {
        ...
        AxisAlignedBB axisalignedbb =
            (AxisAlignedBB((double)k, (double)l, (double)i1, (double)(k + 1), (double)(l + 1), (double)(i1 + 1)))
                .grow(d0)
...
void TileEntityBeacon::updateSegmentColors()
{
    ...
    TileEntityBeacon::BeamSegment tileentitybeacon$beamsegment =
        TileEntityBeacon::BeamSegment(EnumDyeColor.WHITE.getColorComponentValues());
    ...
    if (!world.isRemote && l < levels)
    {
        Iterator var14 =
            world
                .getEntitiesWithinAABB(EntityPlayerMP.class, (new AxisAlignedBB((double)i, (double)j, (double)k,
                                                                                (double)i, (double)(j - 4), (double)k))
...
TileEntityBeacon::BeamSegment::BeamSegment(std::vector<float> colorsIn) : colors(colorsIn), height(1)
{
}
