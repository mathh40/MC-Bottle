...
{
	return (ITextComponent)(hasCustomName() ? TextComponentString(getName()) : TextComponentTranslation(getName(), new Object[0]));
}
...
int32_t TileEntityBanner::getPatterns(ItemStack stack)
{
	...
	return nbttagcompound != nullptr && nbttagcompound.hasKey("Patterns") ? nbttagcompound.getTagList("Patterns", 10).tagCount() : 0;
}
...
EnumDyeColor TileEntityBanner::getColor(ItemStack p_190616_0_)
{
	...
	return nbttagcompound != nullptr && nbttagcompound.hasKey("Base") ? EnumDyeColor::byDyeDamage(nbttagcompound.getInteger("Base")) : EnumDyeColor::BLACK;
}
