...
bool BredAnimalsTrigger::Instance::test(EntityPlayerMP *player, EntityAnimal *parent1In, EntityAnimal *parent2In,
                                        EntityAgeable *childIn) const
{
    ...
    {
        return parent->test(player, parent1In) && partner->test(player, parent2In) ||
               parent->test(player, parent2In) && partner->test(player, parent1In);
    }
}

BredAnimalsTrigger::Listeners::Listeners(const PlayerAdvancements &playerAdvancementsIn)
    : playerAdvancements(&playerAdvancementsIn)
{
}
bool BredAnimalsTrigger::Listeners::isEmpty() const
{
...
void BredAnimalsTrigger::Listeners::remove(ICriterionTrigger::Listener *listener)
{
    listeners.erase(listener);
}

void BredAnimalsTrigger::Listeners::trigger(EntityPlayerMP *player, EntityAnimal *parent1, EntityAnimal *parent2,
                                            EntityAgeable *child) const
{
    std::vector<ICriterionTrigger::Listener *> list;
    for (auto listener1 : listeners)
    {
        if (reinterpret_cast<BredAnimalsTrigger::Instance *>(listener1->getCriterionInstance())
