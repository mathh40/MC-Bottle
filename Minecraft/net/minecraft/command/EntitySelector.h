...
namespace EntitySelector {
    ...
    constexpr auto IS_VALID_ARGUMENT = [](std::string_view p_apply_1_) {
        constexpr std::string_view score = "score_";
         return !p_apply_1_.empty() && (EntitySelector::VALID_ARGUMENTS.contains(p_apply_1_) || p_apply_1_.size() > score.size() && Util::startwith(score,p_apply_1_));
    };
    constexpr std::set<std::string_view> WORLD_BINDING_ARGS = {ARGUMENT_COORDINATE_X, ARGUMENT_COORDINATE_Y, ARGUMENT_COORDINATE_Z, ARGUMENT_DELTA_X, ARGUMENT_DELTA_Y, ARGUMENT_DELTA_Z, ARGUMENT_RANGE_MIN, ARGUMENT_RANGE_MAX};
   ...
   EntityPlayerMP *matchOnePlayer(ICommandSender *sender, std::string_view token);
    ...
    template<class T>
    Entity* matchOneEntity(ICommandSender* sender, std::string_view token){
    ...
    std::vector<World*> getWorlds(ICommandSender* sender, std::unordered_map<std::string_view, std::string_view>& argumentMap);
    bool isEntityTypeValid(ICommandSender* commandSender, std::unordered_map<std::string_view, std::string_view>& params);
    std::vector<std::function<bool(Entity* p_apply_1_)>> getTypePredicates(std::unordered_map<std::string_view, std::string_view>& params, std::string_view type);
    std::vector<std::function<bool(Entity* p_apply_1_)>> getXpLevelPredicates(std::unordered_map<std::string_view, std::string_view>& params);
    std::vector<std::function<bool(Entity* p_apply_1_)>> getGamemodePredicates(std::unordered_map<std::string_view, std::string_view>& params);
   std::vector<std::function<bool(Entity* p_apply_1_)>> getTeamPredicates(std::unordered_map<std::string_view, std::string_view>& params);
   std::vector<std::function<bool(Entity* p_apply_1_)>> getScorePredicates(ICommandSender* sender, std::unordered_map<std::string_view, std::string_view>& params);
   std::vector<std::function<bool(Entity* p_apply_1_)>> getNamePredicates(std::unordered_map<std::string_view, std::string_view>& params); 
  std::vector<std::function<bool(Entity* p_apply_1_)>> getTagPredicates(std::unordered_map<std::string_view, std::string_view>& params);
   std::vector<std::function<bool(Entity* p_apply_1_)>> getRadiusPredicates(std::unordered_map<std::string_view, std::string_view>& params, Vec3d pos); 
   std::vector<std::function<bool(Entity* p_apply_1_)>> getRotationsPredicates(std::unordered_map<std::string_view, std::string_view>& params);
   ...
   double getCoordinate(std::unordered_map<std::string_view, std::string_view> &params, std::string_view key, double defaultD,
                        bool offset);
   ...
   bool isSelector(std::string_view selectorStr);
    ...
    std::vector<entityClass*> filterResults(std::unordered_map<std::string_view, std::string_view>& params, std::vector<std::function<bool(Entity* p_apply_1_)>>& inputList, std::string_view type, World *worldIn,
      ...
      if (!params.contains(ARGUMENT_DELTA_X) && !params.contains(ARGUMENT_DELTA_Y) && !params.contains(ARGUMENT_DELTA_Z)) {
         if (l >= 0) {
            axisalignedbb1 = AxisAlignedBB((double)(position.getx() - l), (double)(position.gety() - l), (double)(position.getz() - l), (double)(position.getx() + l + 1), (double)(position.gety() + l + 1), (double)(position.getz() + l + 1));
            if (flag && !flag1) {
   ...
   template <class T>
   std::vector<T*> getEntitiesFromPredicates(std::vector<T*> matchingEntities, Map params, ICommandSender* sender, T targetClass, std::string_view type, Vec3d pos) {
