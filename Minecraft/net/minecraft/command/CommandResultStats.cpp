...
CommandSender::CommandSender(ICommandSender *senderIn)
    :sender(senderIn)
{
}
...
void CommandResultStats::setCommandStatForSender(MinecraftServer* server, ICommandSender* sender, const CommandResultStatsType& typeIn, int32_t p_184932_4_)
{
...
void CommandResultStats::removeScoreBoardStat(CommandResultStats resultStatsIn, const CommandResultStatsType& resultTypeIn)
{
    if (resultStatsIn.entitiesID != STRING_RESULT_TYPES && resultStatsIn.objectives != STRING_RESULT_TYPES) 
    {
        ...
        {
            CommandResultStats::Type commandresultstats$type = var3[var5];
            if (resultStatsIn.entitiesID[commandresultstats$type.getTypeID()] != nullptr && resultStatsIn.objectives[commandresultstats$type.getTypeID()] != nullptr) 
            {
                flag = false;
                break;
            }
...
void CommandResultStats::setScoreBoardStat(CommandResultStats stats, const CommandResultStatsType& resultType, std::optional<std::string_view> entityID, std::optional<std::string_view> objectiveName)
{
