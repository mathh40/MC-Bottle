...
FunctionObject::CacheableFunction::CacheableFunction(std::optional<ResourceLocation> idIn) : id(idIn)
{
}

FunctionObject::CacheableFunction::CacheableFunction(FunctionObject functionIn) : id(std::nullopt), function(functionIn)
{
}
...
FunctionObject::FunctionEntry::FunctionEntry(const FunctionObject &functionIn) : function(functionIn)
{
}
...
FunctionObject::CommandEntry::CommandEntry(std::string p_i47534_1_) : command(p_i47534_1_)
{
}
...
FunctionObject FunctionObject::create(const FunctionManager &functionManagerIn,
    ...
    for (auto s : commands)
    {
        ...
        if (!Util::startwith("#", s) && !s.empty())
        {
            ...
            if (!functionManagerIn.getCommandManager().getCommands().containsKey(s1))
            {
                ...
                {
                    throw std::logic_error("Unknown or invalid command '" + s1 + "' (did you mean '" + s1.substr(1) +
...
FunctionObject::CacheableFunction FunctionObject::CacheableFunction::EMPTY =
    FunctionObject::CacheableFunction(std::nullopt);
