...
PotionEffect::PotionEffect(Potion *potionIn)
  :PotionEffect(potionIn, 0, 0)
{
}

PotionEffect::PotionEffect(Potion *potionIn, int32_t durationIn)
  :PotionEffect(potionIn, durationIn, 0)
{
}
...
PotionEffect::PotionEffect(Potion *potionIn, int32_t durationIn, int32_t amplifierIn, bool ambientIn, bool showParticlesIn)
  :potion(potionIn),duration(durationIn),amplifier(amplifierIn),isAmbient(ambientIn),showParticles(showParticlesIn)
{
}
...
{
  return a.duration == b.duration && a.amplifier == b.amplifier && a.isSplashPotion == b.isSplashPotion && a.isAmbient == b.isAmbient && (a.potion) == (b.potion);
}

bool operator>(const PotionEffect &a, const PotionEffect &b)
{
  int i = true;
  return a.getDuration() > 32147 && b.getDuration() > 32147 || a.getAmbient() && b.getAmbient() ? ComparisonChain.start().compare(this.getIsAmbient(), p_compareTo_1_.getIsAmbient()).compare(a.getPotion().getLiquidColor(), b.getPotion().getLiquidColor()).result() : ComparisonChain.start().compare(a.getAmbient(), b.getAmbient()).compare(a.getDuration(), b.getDuration()).compare(a.getPotion().getLiquidColor(), b.getPotion().getLiquidColor()).result();
}
