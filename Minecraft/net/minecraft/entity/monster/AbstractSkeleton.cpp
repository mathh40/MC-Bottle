...
DataParameter AbstractSkeleton::SWINGING_ARMS = EntityDataManager.createKey(AbstractSkeleton.class, DataSerializers::BOOLEAN);
...
AbstractSkeleton::AbstractSkeleton(World *worldIn)
    :EntityMob(worldIn){
    setSize(0.6F, 1.99F);
    setCombatTask();
}
...
void AbstractSkeleton::onLivingUpdate() {
    if (world->isDaytime() && !world->isRemote) {
         ...
         BlockPos blockpos = Util::instanceof<EntityBoat>(getRidingEntity()) ? (BlockPos(posX, (double)MathHelper::round(posY), posZ)).up() : BlockPos(posX, (double)MathHelper::round(posY), posZ);
    ...
    livingdata = EntityMob::onInitialSpawn(difficulty, livingdata);
      ...
      if (getItemStackFromSlot(EntityEquipmentSlot::HEAD).isEmpty()) {
         ...
         if (calendar.get(2) + 1 == 10 && calendar.get(5) == 31 && MathHelper::nextFloat(rand) < 0.25F) {
            setItemStackToSlot(EntityEquipmentSlot::HEAD, ItemStack(MathHelper::nextFloat(rand) < 0.1F ? Blocks::LIT_PUMPKIN : Blocks::PUMPKIN));
