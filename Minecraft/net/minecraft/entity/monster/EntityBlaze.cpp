...
AIFireballAttack::AIFireballAttack(EntityBlaze *blazeIn):
    blaze(blazeIn) {
    setMutexBits(3);
}
...
EntityBlaze::EntityBlaze(World *worldIn)
    :EntityMob(worldIn){
...
void EntityBlaze::updateAITasks() {
    if (isWet()) {
      ...
      if (entitylivingbase != nullptr && entitylivingbase->posY + (double)entitylivingbase->getEyeHeight() > posY + (double)getEyeHeight() + (double)heightOffset) {
    ...
    if (!onGround && motionY < 0.0) {
      ...
      if (world->isRemote) {
         if (rand(24) == 0 && !isSilent()) {
            world->playSound(posX + 0.5, posY + 0.5, posZ + 0.5, SoundEvents::ENTITY_BLAZE_BURN, getSoundCategory(), 1.0F + MathHelper::nextFloat(rand), MathHelper::nextFloat(rand) * 0.7F + 0.3F, false);
         }

         for(auto i = 0; i < 2; ++i) {
            world->spawnParticle(EnumParticleTypes::SMOKE_LARGE, posX + (MathHelper::nextDouble(rand) - 0.5) * (double)width, posY + MathHelper::nextDouble(rand) * (double)height, posZ + (MathHelper::nextDouble(rand) - 0.5) * (double)width, 0.0, 0.0, 0.0,{});
         }
      }
