...
DataParameter EntityGuardian::TARGET_ENTITY =
    EntityDataManager.createKey(EntityGuardian.class, DataSerializers::VARINT);
...
GuardianTargetSelector::GuardianTargetSelector(EntityGuardian *guardian) : parentEntity(guardian)
{
}

bool GuardianTargetSelector::operator()(EntityLivingBase *p_apply_1_) const
{
    return (Util:: instanceof <EntityPlayer>(p_apply_1_) || Util:: instanceof <EntitySquid>(p_apply_1_)) &&
           p_apply_1_->getDistanceSq(parentEntity) > 9.0;
}

EntityGuardian::EntityGuardian(World *worldIn) : EntityMob(worldIn)
{
    experienceValue = 10;
    setSize(0.85F, 0.85F);
...
float EntityGuardian::getBlockPathWeight(BlockPos pos)
{
    return world->getBlockState(pos)->getMaterial() == Material::WATER ? 10.0F + world->getLightBrightness(pos) - 0.5F
                                                                       : EntityMob::getBlockPathWeight(pos);
}
        ...
        if (isMoving() && isInWater())
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                world->spawnParticle(EnumParticleTypes::WATER_BUBBLE,
                                     posX + (MathHelper::nextDouble(rand) - 0.5) * (double)width - vec3d.getx() * 1.5,
                                     posY + MathHelper::nextDouble(rand) * (double)height - vec3d.gety() * 1.5,
                                     posZ + (MathHelper::nextDouble(rand) - 0.5) * (double)width - vec3d.getz() * 1.5,
                                     0.0, 0.0, 0.0, {});
            }
        }
        ...
        if (hasTargetedEntity())
        {
            ...
            if (entitylivingbase != nullptr)
            {
                ...
                while (d4 < d3)
                {
                    ...
                    world->spawnParticle(EnumParticleTypes::WATER_BUBBLE, posX + d0 * d4,
                                         posY + d1 * d4 + (double)getEyeHeight(), posZ + d2 * d4, 0.0, 0.0, 0.0, {});
                }
            }
        }
    }
...
{
    if (!isMoving() && !source.isMagicDamage() && Util:: instanceof <EntityLivingBase>(source.getImmediateSource()))
    {
...
void EntityGuardian::GuardianMoveHelper::onUpdateMoveHelper()
{
    if (action == EntityMoveHelper::Action::MOVE_TO && !entityGuardian->getNavigator()->noPath())
    {
        ...
        float f1 = (float)(speed * entityGuardian->getEntityAttribute(SharedMonsterAttributes::MOVEMENT_SPEED.get())
                                       ->getAttributeValue());
        entityGuardian->setAIMoveSpeed(entityGuardian->getAIMoveSpeed() +
                                       (f1 - entityGuardian->getAIMoveSpeed()) * 0.125F);
        double d4 =
            MathHelper::sin((double)(entityGuardian->ticksExisted + entityGuardian->getEntityId()) * 0.5) * 0.05;
    ...
    else
    {
        ...
        else if (tickCounter >= guardian->getAttackDuration())
        {
            ...
            entitylivingbase->attackEntityFrom(
                DamageSource::causeMobDamage(guardian),
                (float)guardian->getEntityAttribute(SharedMonsterAttributes::ATTACK_DAMAGE.get())->getAttributeValue());
            guardian->setAttackTarget(nullptr);
        }
