...
AttributeModifier ATTACKING_SPEED_BOOST = (AttributeModifier(EntityEnderman::ATTACKING_SPEED_BOOST_ID, "Attacking speed boost", 0.15000000596046448, 0)).setSaved(false);
...
EntityEnderman::EntityEnderman(World *worldIn)
    :EntityMob(worldIn){
    setSize(0.6F, 2.9F);
    stepHeight = 1.0F;
...
void EntityEnderman::onLivingUpdate() {
    if (world->isRemote) {
         for(int i = 0; i < 2; ++i) {
            world->spawnParticle(EnumParticleTypes::PORTAL, posX + (MathHelper::nextDouble(rand) - 0.5) * (double)width, posY + MathHelper::nextDouble(rand) * (double)height - 0.25, posZ + (MathHelper::nextDouble(rand) - 0.5) * (double)width, (MathHelper::nextDouble(rand) - 0.5) * 2.0, -MathHelper::nextDouble(rand), (MathHelper::nextDouble(rand) - 0.5) * 2.0,{});
         }
      }
...
EntityEnderman::AITakeBlock::AITakeBlock(EntityEnderman *p_i45841_1_):
...
EntityEnderman::AIPlaceBlock::AIPlaceBlock(EntityEnderman *p_i45843_1_):
...
EntityEnderman::AIFindPlayer::AIFindPlayer(EntityEnderman *p_i45842_1_):
    EntityAINearestAttackableTarget(p_i45842_1_, typeid(EntityPlayer), false)
...
bool EntityEnderman::AIFindPlayer::shouldExecute() {
    ...
    player = enderman->world->getNearestAttackablePlayer(enderman->posX, enderman->posY, enderman->posZ, d0, d0,
                                                         std::nullopt, [](EntityPlayer *p_apply_1_)
                                                         {
                                                             return p_apply_1_ != nullptr && AIFindPlayer::enderman->
                                                                    shouldAttackPlayer(p_apply_1_);
                                                         });
...
void EntityEnderman::initEntityAI() {
    tasks.addTask(0, new EntityAISwimming(this));
      ...
      targetTasks.addTask(3, new EntityAINearestAttackableTarget(this, EntityEndermite.class, 10, true, false, [](EntityEndermite* p_apply_1_) {
...
bool EntityEnderman::teleportToEntity(Entity *p_70816_1_) {
    Vec3d vec3d = Vec3d(posX - p_70816_1_->posX, getEntityBoundingBox().getminY() + (double)(height / 2.0F) - p_70816_1_->posY + (double)p_70816_1_->getEyeHeight(), posZ - p_70816_1_->posZ);
...
bool EntityEnderman::shouldAttackPlayer(EntityPlayer* player) {
    ItemStack itemstack = (ItemStack)player->inventory.armorInventory.get(3);
      if (itemstack.getItem() == Item::getItemFromBlock(Blocks::PUMPKIN)) {
         ...
         Vec3d vec3d1 = Vec3d(posX - player->posX, getEntityBoundingBox().getminY() + (double)getEyeHeight() - (player->posY + (double)player->getEyeHeight()), posZ - player->posZ);
...
bool EntityEnderman::teleportTo(double x, double y, double z) {
    bool flag = attemptTeleport(x, y, z);
      if (flag) {
         world->playSound(nullptr, prevPosX, prevPosY, prevPosZ, SoundEvents::ENTITY_ENDERMEN_TELEPORT, getSoundCategory(), 1.0F, 1.0F);
