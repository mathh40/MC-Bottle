...
EntityElderGuardian::EntityElderGuardian(World *worldIn) : EntityGuardian(worldIn)
{
...
void EntityElderGuardian::updateAITasks()
{
    ...
    if ((ticksExisted + getEntityId()) % 1200 == 0)
    {
        ...
        auto list     = world->getPlayers(EntityPlayerMP.class, [](EntityPlayerMP *p_apply_1_) {
            return EntityElderGuardian::getDistanceSq(p_apply_1_) < 2500.0 &&
    ...
    label28:
        while (true)
        {
            ...
            } while (entityplayermp->isPotionActive(potion) &&
