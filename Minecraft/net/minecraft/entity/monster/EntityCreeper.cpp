...
EntityCreeper::EntityCreeper(World *worldIn)
    :EntityMob(worldIn){
    setSize(0.6F, 1.7F);
}
...
void EntityCreeper::onDeath(DamageSource::DamageSource cause) {
    EntityMob::onDeath(cause);
      if (world->getGameRules().getBoolean("doMobLoot")) {
         ...
         } else if (Util::instanceof<EntityCreeper>(cause.getTrueSource()) && cause.getTrueSource() != this && ((EntityCreeper*)cause.getTrueSource())->getPowered() && ((EntityCreeper*)cause.getTrueSource())->ableToCauseSkullDrop()) {
...
bool EntityCreeper::processInteract(EntityPlayer *player, EnumHand hand) {
    ItemStack itemstack = player->getHeldItem(hand);
      if (itemstack.getItem() == Items::FLINT_AND_STEEL) {
         world->playSound(player, posX, posY, posZ, SoundEvents::ITEM_FLINTANDSTEEL_USE, getSoundCategory(), 1.0F, MathHelper::nextFloat(rand) * 0.4F + 0.8F);
