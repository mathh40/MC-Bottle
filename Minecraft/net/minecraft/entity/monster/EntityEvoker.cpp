...
auto wololoSelector = [](EntitySheep* p_apply_1_) {
    return p_apply_1_->getFleeceColor() == EnumDyeColor::BLUE;
};


EntityEvoker::EntityEvoker(World *worldIn)
    :EntitySpellcasterIllager(worldIn){
    setSize(0.6F, 1.95F);
    experienceValue = 10;
}
...
bool EntityEvoker::isOnSameTeam(Entity *entityIn) {
    if (entityIn == nullptr) {
      ...
      } else if (Util::instanceof<EntityLivingBase>(entityIn) && ((EntityLivingBase*)entityIn)->getCreatureAttribute() == EnumCreatureAttribute::ILLAGER) {
...
bool EntityEvoker::AISummonSpell::shouldExecute() {
    if (!AIUseSpell::shouldExecute()) {
        ...
        auto i = entityEvoker->world->getEntitiesWithinAABB(EntityVex.class,
                                                            entityEvoker->getEntityBoundingBox().grow(16.0)).size();
...
void EntityEvoker::AIAttackSpell::castSpell() {
    ...
    float f = MathHelper::atan2(entitylivingbase->posZ - entityEvoker->posZ,
                                entitylivingbase->posX - entityEvoker->posX);
    int k;
...
void EntityEvoker::AIAttackSpell::spawnFangs(double p_190876_1_, double p_190876_3_, double p_190876_5_,
    ...
    do {
        if (!entityEvoker->world->isBlockNormalCube(blockpos, true) && entityEvoker->world->isBlockNormalCube(
                blockpos.down(), true)) {
    ...
    if (flag) {
        EntityEvokerFangs *entityevokerfangs = new EntityEvokerFangs(entityEvoker->world, p_190876_1_,
                                                                     (double)blockpos.gety() + d0, p_190876_3_,
