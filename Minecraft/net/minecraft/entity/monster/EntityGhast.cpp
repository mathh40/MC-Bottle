...
EntityGhast::GhastMoveHelper::GhastMoveHelper(EntityGhast *ghast):
    EntityMoveHelper(ghast), parentEntity(ghast) {
}
...
EntityGhast::AIRandomFly::AIRandomFly(EntityGhast *ghast):
    parentEntity(ghast) {
    setMutexBits(1);
}
...
EntityGhast::AILookAround::AILookAround(EntityGhast *ghast):
    parentEntity(ghast) {
    setMutexBits(2);
}
...
void EntityGhast::AILookAround::updateTask() {
    if (parentEntity->getAttackTarget() == nullptr) {
        parentEntity->rotationYaw = -((float)MathHelper::atan2(parentEntity->motionX, parentEntity->motionZ)) *
                                    57.295776F;
...
EntityGhast::EntityGhast(World *worldIn)
    :EntityFlying(worldIn){
      setSize(4.0F, 4.0F);
...
bool EntityGhast::attackEntityFrom(DamageSource::DamageSource source, float amount) {
    if (isEntityInvulnerable(source)) {
         return false;
      } else if (Util::instanceof<EntityLargeFireball>(source.getImmediateSource()) && 
          Util::instanceof<EntityPlayer>(source.getTrueSource())) {
