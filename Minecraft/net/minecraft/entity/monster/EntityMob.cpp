...
EntityMob::EntityMob(World *worldIn) : EntityCreature(worldIn)
{
    experienceValue = 5;
}
...
bool EntityMob::attackEntityAsMob(Entity *entityIn)
{
    ...
    {
        f += EnchantmentHelper::getModifierForCreature(getHeldItemMainhand(),
    ...
    if (flag)
    {
        ...
        if (Util:: instanceof <EntityPlayer>(entityIn))
        {
            ...
            if (!itemstack.isEmpty() && !itemstack1.isEmpty() && Util:: instanceof
                <ItemAxe>(itemstack.getItem()) && itemstack1.getItem() == Items::SHIELD)
            {
