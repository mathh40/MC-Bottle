...
auto IS_HANGING_ENTITY = [](Entity* p_apply_1_)->bool
{
    return Util::instanceof<EntityHanging>(p_apply_1_);
};

EntityHanging::EntityHanging(World *worldIn)
    :Entity(worldIn){
    setSize(0.5F, 0.5F);
}

EntityHanging::EntityHanging(World *worldIn, BlockPos hangingPositionIn) 
    :Entity(worldIn){
...
bool EntityHanging::hitByEntity(Entity *entityIn) {
    return Util::instanceof<EntityPlayer>(entityIn) ? attackEntityFrom(DamageSource::causePlayerDamage((EntityPlayer*)entityIn), 0.0F) : false;
}
...
void EntityHanging::readEntityFromNBT(NBTTagCompound *compound) {
    hangingPosition = BlockPos(compound->getInteger("TileX"), compound->getInteger("TileY"), compound->getInteger("TileZ"));
...
EntityItem * EntityHanging::entityDropItem(ItemStack stack, float offsetY) {
    EntityItem* entityitem = new EntityItem(world, posX + (double)((float)facingDirection.getXOffset() * 0.15F), posY + (double)offsetY, posZ + (double)((float)facingDirection.getZOffset() * 0.15F), stack);
