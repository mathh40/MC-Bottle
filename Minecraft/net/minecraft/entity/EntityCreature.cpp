...
AttributeModifier EntityCreature::FLEEING_SPEED_MODIFIER = AttributeModifier(FLEEING_SPEED_MODIFIER_UUID, "Fleeing speed bonus", 2.0, 2).setSaved(false);

EntityCreature::EntityCreature(World *worldIn)
    :EntityLiving(worldIn),homePosition(BlockPos::ORIGIN),maximumHomeDistance(-1.0f),restoreWaterCost(PathNodeType::WATER.getPriority())
{
}
...
bool EntityCreature::getCanSpawnHere() {
    return EntityLiving::getCanSpawnHere() && getBlockPathWeight(BlockPos(posX, getEntityBoundingBox().getminY(), posZ)) >= 0.0F;
}
    ...
    EntityLiving::updateLeashedState();
      if (getLeashed() && getLeashHolder() != nullptr && getLeashHolder()->world == world) {
         ...
         if (f > 10.0F) {
            ...
            getNavigator()->tryMoveToXYZ(posX + vec3d.getx(), posY + vec3d.gety(), posZ + vec3d.getz(), followLeashSpeed());
         }
      }
}
