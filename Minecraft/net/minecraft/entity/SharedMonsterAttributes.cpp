...
std::shared_ptr<spdlog::logger> SharedMonsterAttributes::LOGGER =
    spdlog::get("Minecraft")->clone("SharedMonsterAttributes");

std::unique_ptr<IAttribute> SharedMonsterAttributes::MAX_HEALTH =
    (std::make_unique<RangedAttribute>(nullptr, "generic.maxHealth", 20.0, 0.0, 1024.0))
        .setDescription("Max Health")
        .setShouldWatch(true);
std::unique_ptr<IAttribute> SharedMonsterAttributes::FOLLOW_RANGE =
    (std::make_unique<RangedAttribute>(nullptr, "generic.followRange", 32.0, 0.0, 2048.0))
...
std::unique_ptr<IAttribute> SharedMonsterAttributes::ATTACK_DAMAGE =
    std::make_unique<RangedAttribute>(nullptr, "generic.attackDamage", 2.0, 0.0, 2048.0);
...
std::optional<AttributeModifier> SharedMonsterAttributes::readAttributeModifierFromNBT(NBTTagCompound *compound)
{
    ...
    try
    {
        return AttributeModifier(uuid, compound->getString("Name"), compound->getDouble("Amount"),
