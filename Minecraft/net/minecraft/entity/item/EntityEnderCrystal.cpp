...
DataParameter EntityEnderCrystal::BEAM_TARGET = EntityDataManager::createKey(EntityEnderCrystal.class, DataSerializers.OPTIONAL_BLOCK_POS);
DataParameter EntityEnderCrystal::SHOW_BOTTOM = EntityDataManager::createKey(EntityEnderCrystal.class, DataSerializers.BOOLEAN);


EntityEnderCrystal::EntityEnderCrystal(World *worldIn)
    :Entity(worldIn),innerRotation(rand(100000)){
        preventEntitySpawning = true;
        setSize(2.0F, 2.0F);
}

EntityEnderCrystal::EntityEnderCrystal(World *worldIn, double x, double y, double z)
    :EntityEnderCrystal(worldIn){
    setPosition(x, y, z);
}
...
void EntityEnderCrystal::onUpdate() {
    ...
    if (!world->isRemote) {
        ...
        if (Util::instanceof<WorldProviderEnd>(world->provider) && world->getBlockState(blockpos)->getBlock() != Blocks::FIRE) {
