...
EntityEnderPearl::EntityEnderPearl(World *worldIn)
    :EntityThrowable(worldIn){
}

EntityEnderPearl::EntityEnderPearl(World *worldIn, EntityLivingBase *throwerIn)
    :EntityThrowable(worldIn,throwerIn){
    perlThrower = throwerIn;
}

EntityEnderPearl::EntityEnderPearl(World *worldIn, double x, double y, double z)
...
void EntityEnderPearl::onUpdate() {
    EntityLivingBase* entitylivingbase = getThrower();
      if (entitylivingbase != nullptr && Util::instanceof<EntityPlayer>(entitylivingbase) && !entitylivingbase->isEntityAlive()) {
         setDead();
      } else {
...
void EntityEnderPearl::onImpact(RayTraceResult result) {
    EntityLivingBase* entitylivingbase = getThrower();
      ...
      for(int i = 0; i < 32; ++i) {
         world->spawnParticle(EnumParticleTypes::PORTAL, posX, posY + MathHelper::nextDouble(rand) * 2.0, posZ, MathHelper::nextGaussian<double>(rand), 0.0, MathHelper::nextGaussian<double>(rand));
      }
