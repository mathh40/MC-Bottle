...
EntityBoat::EntityBoat(World *worldIn)
    :Entity(worldIn){
        preventEntitySpawning  = true;
        setSize(1.375F, 0.5625F);
}

EntityBoat::EntityBoat(World *worldIn, double x, double y, double z)
    :EntityBoat(worldIn){
        setPosition(x, y, z);
        motionX = 0.0;
        motionY = 0.0;
...
bool EntityBoat::attackEntityFrom(DamageSource::DamageSource source, float amount) {
    if (isEntityInvulnerable(source)) {
         ...
         if (Util::instanceof<DamageSource::EntityDamageSourceIndirect>(&source) && source.getTrueSource() != nullptr && isPassenger(source.getTrueSource())) {
            return false;
         } else {
...
void EntityBoat::onUpdate() {
    previousStatus = status;
      ...
      for(auto i = 0; i <= 1; ++i) {
         if (getPaddleState(i)) {
            if (!isSilent() && (double)( MathHelper::fmod(paddlePositions[i],6.2831855F)) <= 0.7853981633974483 && ((double)paddlePositions[i] + 0.39269909262657166) % 6.283185307179586 >= 0.7853981633974483) {
               ...
               if (soundevent != nullptr) {
                  ...
                  world->playSound(nullptr, posX + d0, posY, posZ + d1, soundevent, getSoundCategory(), 1.0F, 0.8F + 0.4F * MathHelper::nextFloat(rand));
               }
            }
      ...
      doBlockCollisions();
      auto list = world->getEntitiesInAABBexcluding(this, getEntityBoundingBox().grow(0.20000000298023224, -0.009999999776482582, 0.20000000298023224), EntitySelectors::getTeamCollisionPredicate(this));
      if (!list.isEmpty()) {
         ...
         for(auto j = 0; j < list.size(); ++j) {
            ...
            if (!entity.isPassenger(this)) {
               if (flag && getPassengers().size() < 2 && !entity.isRiding() && entity.width < width && Util::instanceof<EntityLivingBase>(entity) && !(Util::instanceof<EntityWaterMob>(entity)) && !(Util::instanceof<EntityPlayer>(entity))) {
...
float EntityBoat::getRowingTime(int side, float limbSwing) {
    return getPaddleState(side) ? (float)MathHelper::clampedLerp((double)paddlePositions[side] - 0.39269909262657166, (double)paddlePositions[side], (double)limbSwing) : 0.0F;
}
            ...
            for(auto l1 = i; l1 < j; ++l1) {
               for(auto i2 = i1; i2 < j1; ++i2) {
                  ...
                  if (iblockstate->getMaterial() == Material::WATER) {
                     f = MathHelper::max(f, BlockLiquid::getBlockLiquidHeight(iblockstate, world, blockpos$pooledmutableblockpos));
                  }
...
float EntityBoat::getBoatGlide() {
    AxisAlignedBB axisalignedbb = getEntityBoundingBox();
      AxisAlignedBB axisalignedbb1(axisalignedbb.getminX(), axisalignedbb.getminY() - 0.001, axisalignedbb.getminZ(), axisalignedbb.getmaxX(), axisalignedbb.getminY(), axisalignedbb.getmaxZ());
...
EntityBoat::Type::Type(int32_t metadataIn, std::string_view nameIn)
    :name(nameIn),metadata(metadataIn){
            ...
            if (fallDistance > 3.0F) {
               ...
               if (!world->isRemote && !isDead) {
                  ...
                  if (world->getGameRules().getBoolean("doEntityDrops")) {
                     int32_t j;
                     for(j = 0; j < 3; ++j) {
                        entityDropItem(ItemStack(Item::getItemFromBlock(Blocks::PLANKS), 1, getBoatType().getMetadata()), 0.0F);
                     }
