...
EntityEnderEye::EntityEnderEye(World *worldIn)
    :Entity(worldIn){
    setSize(0.25F, 0.25F);
}
...
void EntityEnderEye::onUpdate() {
    lastTickPosX = posX;
      ...
      if (isInWater()) {
         ...
         world->spawnParticle(EnumParticleTypes::PORTAL, posX - motionX * 0.25 + MathHelper::nextDouble(rand) * 0.6 - 0.3, posY - motionY * 0.25 - 0.5, posZ - motionZ * 0.25 + MathHelper::nextDouble(rand) * 0.6 - 0.3, motionX, motionY, motionZ);
      }
...
EntityEnderEye::EntityEnderEye(World *worldIn, double x, double y, double z)
    :Entity(worldIn),despawnTimer(0){
    setSize(0.25F, 0.25F);
    setPosition(x, y, z);
}
