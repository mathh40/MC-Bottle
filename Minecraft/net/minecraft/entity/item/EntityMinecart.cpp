...
DataParameter EntityMinecart::ROLLING_AMPLITUDE =
    EntityDataManager::createKey(EntityMinecart.class, DataSerializers::VARINT);
DataParameter EntityMinecart::ROLLING_DIRECTION =
    EntityDataManager::createKey(EntityMinecart.class, DataSerializers::VARINT);
DataParameter EntityMinecart::DAMAGE = EntityDataManager::createKey(EntityMinecart.class, DataSerializers::FLOAT);
DataParameter EntityMinecart::DISPLAY_TILE =
    EntityDataManager::createKey(EntityMinecart.class, DataSerializers::VARINT);
DataParameter EntityMinecart::DISPLAY_TILE_OFFSET =
    EntityDataManager::createKey(EntityMinecart.class, DataSerializers::VARINT);
...
EntityMinecart::EntityMinecart(World *worldIn) : Entity(worldIn)
{
    preventEntitySpawning = true;
    setSize(0.98F, 0.7F);
}

EntityMinecart::EntityMinecart(World *worldIn, double x, double y, double z) : EntityMinecart(worldIn)
{
    setPosition(x, y, z);
    motionX  = 0.0;
    motionY  = 0.0;
...
void EntityMinecart::onUpdate()
{
    ...
    if (world->isRemote)
    {
        if (turnProgress > 0)
        {
            ...
            rotationPitch =
                (float)((double)rotationPitch + (minecartPitch - (double)rotationPitch) / (double)turnProgress);
            --turnProgress;
    ...
    else
    {
        ...
        if (getType() == EntityMinecart::Type::RIDEABLE && motionX * motionX + motionZ * motionZ > 0.01)
        {
            auto list = world->getEntitiesInAABBexcluding(
                this, getEntityBoundingBox().grow(0.20000000298023224, 0.0, 0.20000000298023224),
                EntitySelectors::getTeamCollisionPredicate(this));
            for (auto entity1 : list)
            {
                if (!(Util:: instanceof <EntityPlayer>(entity1)) && !(Util:: instanceof <EntityIronGolem>(entity1)) &&
                    !(Util:: instanceof <EntityMinecart>(entity1)) && !isBeingRidden() && !entity1->isRiding())
                {
        ...
        {
            auto var12 = world->getEntitiesWithinAABBExcludingEntity(
                this, getEntityBoundingBox().grow(0.20000000298023224, 0.0, 0.20000000298023224));
...
AxisAlignedBB EntityMinecart::getRenderBoundingBox()
{
    ...
    return hasDisplayTile() ? axisalignedbb.grow((double)MathHelper::abs(getDisplayTileOffset()) / 16.0)
                            : axisalignedbb;
}
    ...
    if (!world->isRemote && !entityIn->noClip && !noClip && !isPassenger(entityIn))
    {
        ...
        if (d2 >= 9.999999747378752E-5)
        {
            ...
            if (Util:: instanceof <EntityMinecart>(entityIn))
            {
                ...
                Vec3d vec3d1 = (Vec3d((double)MathHelper::cos(rotationYaw * 0.017453292F), 0.0,
                                      (double)MathHelper::sin(rotationYaw * 0.017453292F)))
                                   .normalize();
                ...
                if (((EntityMinecart *)entityIn)->getType() == EntityMinecart::Type::FURNACE &&
                    getType() != EntityMinecart::Type::FURNACE)
                {
...
void EntityMinecart::moveAlongTrack(const BlockPos &pos, IBlockState *state)
{
    ...
    double d0 = 0.0078125;
    BlockRailBase::EnumRailDirection blockrailbase$enumraildirection =
        (BlockRailBase::EnumRailDirection)state->getValue(blockrailbase.getShapeProperty());
    ...
    if (aint[0][1] != 0 && MathHelper::floor(posX) - pos.getx() == aint[0][0] &&
        MathHelper::floor(posZ) - pos.getz() == aint[0][2])
    {
        setPosition(posX, posY + (double)aint[0][1], posZ);
    }
    else if (aint[1][1] != 0 && MathHelper::floor(posX) - pos.getx() == aint[1][0] &&
             MathHelper::floor(posZ) - pos.getz() == aint[1][2])
    {
