...
DataParameter EntityFireworkRocket::FIREWORK_ITEM =
    EntityDataManager::createKey(EntityFireworkRocket.class, DataSerializers::ITEM_STACK);
DataParameter EntityFireworkRocket::BOOSTED_ENTITY_ID =
    EntityDataManager::createKey(EntityFireworkRocket.class, DataSerializers::VARINT);

EntityFireworkRocket::EntityFireworkRocket(World *worldIn) : Entity(worldIn)
{
    setSize(0.25F, 0.25F);
}
...
void EntityFireworkRocket::onUpdate()
{
    ...
    for (rotationPitch = (MathHelper::atan2(motionY, f) * 57.29577951308232);
    ...
    {
        world->playSound(nullptr, posX, posY, posZ, SoundEvents::ENTITY_FIREWORK_LAUNCH, SoundCategory::AMBIENT, 3.0F,
                         1.0F);
    }
    ...
    {
        world->spawnParticle(EnumParticleTypes::FIREWORKS_SPARK, posX, posY - 0.3, posZ,
                             MathHelper::nextGaussian<double>(rand) * 0.05, -motionY * 0.5,
                             MathHelper::nextGaussian<double>(rand) * 0.05);
    }
...
void EntityFireworkRocket::dealExplosionDamage()
{
    ...
    if (f > 0.0F)
    {
        ...
        while (true)
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                auto raytraceresult = world->rayTraceBlocks(
                    vec3d,
                    Vec3d(entitylivingbase->posX,
                          entitylivingbase->posY + (double)entitylivingbase->height * 0.5 * (double)i,
