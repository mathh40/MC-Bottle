...
{
    return Util::instanceof<EntityMinecart>(p_apply_1_) && ((EntityMinecart*)p_apply_1_)->getType() == EntityMinecart::Type::RIDEABLE;
};

DataParameter EntityArmorStand::STATUS = EntityDataManager::createKey(EntityArmorStand.class, DataSerializers::BYTE);
DataParameter EntityArmorStand::HEAD_ROTATION = EntityDataManager::createKey(EntityArmorStand.class, DataSerializers::ROTATIONS);
DataParameter EntityArmorStand::BODY_ROTATION = EntityDataManager::createKey(EntityArmorStand.class, DataSerializers::ROTATIONS);
DataParameter EntityArmorStand::LEFT_ARM_ROTATION = EntityDataManager::createKey(EntityArmorStand.class, DataSerializers::ROTATIONS);
DataParameter EntityArmorStand::RIGHT_ARM_ROTATION = EntityDataManager::createKey(EntityArmorStand.class, DataSerializers::ROTATIONS);
DataParameter EntityArmorStand::LEFT_LEG_ROTATION = EntityDataManager::createKey(EntityArmorStand.class, DataSerializers::ROTATIONS);
DataParameter EntityArmorStand::RIGHT_LEG_ROTATION = EntityDataManager::createKey(EntityArmorStand.class, DataSerializers::ROTATIONS);
...
EntityArmorStand::EntityArmorStand(World *worldIn)
    ...
    ,headRotation(DEFAULT_HEAD_ROTATION),bodyRotation(DEFAULT_BODY_ROTATION),leftArmRotation(DEFAULT_LEFTARM_ROTATION)
    ,rightArmRotation(DEFAULT_RIGHTARM_ROTATION),leftLegRotation(DEFAULT_LEFTLEG_ROTATION),rightLegRotation(DEFAULT_RIGHTLEG_ROTATION){
    noClip = hasNoGravity();
         ...
         if (DamageSource::OUT_OF_WORLD == source) {
            ...
            if (source.isExplosion()) {
               ...
               if (!flag1 && !flag) {
                  ...
                  if (Util::instanceof<EntityPlayer>(source.getTrueSource()) && !((EntityPlayer*)source.getTrueSource())->capabilities.allowEdit) {
                     return false;
...
EntityEquipmentSlot EntityArmorStand::getClickedSlot(Vec3d p_190772_1_) {
    EntityEquipmentSlot entityequipmentslot = EntityEquipmentSlot::MAINHAND;
      ...
      if (d0 >= 0.1 && d0 < 0.1 + (flag ? 0.8 : 0.45) && hasItemInSlot(EntityEquipmentSlot::FEET)) {
         entityequipmentslot = EntityEquipmentSlot::FEET;
      } else if (d0 >= 0.9 + (flag ? 0.3 : 0.0) && d0 < 0.9 + (flag ? 1.0 : 0.7) && hasItemInSlot(EntityEquipmentSlot::CHEST)) {
         entityequipmentslot = EntityEquipmentSlot::CHEST;
...
void EntityArmorStand::swapItem(EntityPlayer *player, EntityEquipmentSlot p_184795_2_, ItemStack p_184795_3_,
    ...
    ItemStack itemstack = getItemStackFromSlot(p_184795_2_);
      if ((itemstack.isEmpty() || (disabledSlots & 1 << p_184795_2_.getSlotIndex() + 8) == 0) && (!itemstack.isEmpty() || (disabledSlots & 1 << p_184795_2_.getSlotIndex() + 16) == 0)) {
...
void EntityArmorStand::playParticles() {
    if (Util::instanceof<WorldServer>(world)) {
         ((WorldServer*)world)->spawnParticle(EnumParticleTypes::BLOCK_DUST, posX, posY + (double)height / 1.5, posZ, 10, (double)(width / 4.0F), (double)(height / 4.0F), (double)(width / 4.0F), 0.05, Block::getStateId(Blocks::PLANKS.getDefaultState()));
      }
}
