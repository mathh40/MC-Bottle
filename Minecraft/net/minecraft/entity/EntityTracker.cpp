...
{
  public:
    EntityTrackerCrashReportDetail(int32_t updateFrequencyIn) : updateFrequency(updateFrequencyIn)
    {
    }
...
void EntityTracker::track(Entity *entityIn, int32_t trackingRange, int32_t updateFrequency, bool sendVelocityUpdates)
{
    try
    {
        ...
        EntityTrackerEntry entitytrackerentry(entityIn, trackingRange, maxTrackingDistanceThreshold, updateFrequency,
                                              sendVelocityUpdates);
    ...
    catch (std::exception &var10)
    {
        ...
        (trackedEntityHashTable[entityIn->getEntityId()])
            .getTrackedEntity()
    ...
    if (!list.empty())
    {
        for (auto entity2 : list)
        {
            player->connection.sendPacket(
                new SPacketEntityAttach(entity2, ((EntityLiving *)entity2)->getLeashHolder()));
        }
    }
