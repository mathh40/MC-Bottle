...
std::shared_ptr<spdlog::logger> EntityAIFindEntityNearest::LOGGER =
    spdlog::get("Minecraft")->clone("EntityAIFindEntityNearest");
    ...
    : mob(mobIn), classToCheck(p_i45884_2_)
{
    ...
    predicate = [&](EntityLivingBase *p_apply_1_) -> bool {
        ...
        {
            return p_apply_1_->getDistance(mob) > d0 ? false
                                                     : EntityAITarget::isSuitableTarget(mob, p_apply_1_, false, true);
        }
    };
...
bool EntityAIFindEntityNearest::shouldExecute()
{
    double d0 = getFollowRange();
    auto list =
        mob->world.getEntitiesWithinAABB(classToCheck, mob->getEntityBoundingBox().grow(d0, 4.0, d0), predicate);
