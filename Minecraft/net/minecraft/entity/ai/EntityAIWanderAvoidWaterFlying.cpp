...
std::optional<Vec3d> EntityAIWanderAvoidWaterFlying::getTreePos()
{
    ...
    auto iterable = getAllInBoxMutable(MathHelper::floor(entity->posX - 3.0), MathHelper::floor(entity->posY - 6.0),
                                       MathHelper::floor(entity->posZ - 3.0), MathHelper::floor(entity->posX + 3.0),
                                       MathHelper::floor(entity->posY + 6.0), MathHelper::floor(entity->posZ + 3.0));
    ...
    do
    {
        ...
        Block *block =
            entity->world.getBlockState(blockpos$mutableblockpos1.setPos((Vec3i)blockpos1).move(EnumFacing::DOWN))
                .getBlock();
        flag = Util:: instanceof <BlockLeaves>(block) || block == Blocks::LOG || block == Blocks::LOG2;
    } while (!flag || !entity->world.isAirBlock(blockpos1) ||
             !entity->world.isAirBlock(blockpos$mutableblockpos.setPos((Vec3i)blockpos1).move(EnumFacing::UP)));
