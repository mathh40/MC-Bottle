...
void EntityAIHurtByTarget::alertOthers()
{
    ...
    auto var3 = taskOwner->world->getEntitiesWithinAABB(taskOwner->getClass(), (AxisAlignedBB(taskOwner->posX, taskOwner->posY, taskOwner->posZ, taskOwner->posX + 1.0D, taskOwner->posY + 1.0D, taskOwner->posZ + 1.0)).grow(d0, 10.0, d0)).iterator();
    ...
    while(true) 
    {
        ...
        do 
        {
            ...
            while(Util::instanceof<EntityTameable>(taskOwner) && ((EntityTameable*)taskOwner)->getOwner() != ((EntityTameable*)entitycreature)->getOwner());
        }
