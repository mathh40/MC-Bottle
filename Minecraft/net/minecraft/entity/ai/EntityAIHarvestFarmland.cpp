...
void EntityAIHarvestFarmland::updateTask()
{
    ...
    villager->getLookHelper().setLookPosition(destinationBlock.getx() + 0.5, destinationBlock.gety() + 1,
                                              destinationBlock.getz() + 0.5, 10.0F, villager->getVerticalFaceSpeed());
...
bool EntityAIHarvestFarmland::shouldMoveTo(World *worldIn, BlockPos pos)
{
    ...
    if (block == Blocks::FARMLAND)
    {
        ...
        if (Util:: instanceof <BlockCrops>(block) && ((BlockCrops *)block).isMaxAge(iblockstate) && wantsToReapStuff &&
