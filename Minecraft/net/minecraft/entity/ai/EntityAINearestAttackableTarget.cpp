...
bool EntityAINearestAttackableTarget::shouldExecute()
{
    ...
    else
    {
        this.targetEntity = this.taskOwner.world.getNearestAttackablePlayer(
            ...
            new Function() {
                @Nullable public Double apply(@Nullable EntityPlayer p_apply_1_)
                {
                    ItemStack itemstack = p_apply_1_.getItemStackFromSlot(EntityEquipmentSlot.HEAD);
                    if (itemstack.getItem() == Items.SKULL)
                    {
                        int i        = itemstack.getItemDamage();
                        boolean flag = EntityAINearestAttackableTarget.this.taskOwner instanceof
                            EntitySkeleton && i == 0;
                        boolean flag1 = EntityAINearestAttackableTarget.this.taskOwner instanceof
                            EntityZombie && i == 2;
                        boolean flag2 = EntityAINearestAttackableTarget.this.taskOwner instanceof
                            EntityCreeper && i == 4;
                        if (flag || flag1 || flag2)
                        {
                            return 0.5D;
...
EntityAINearestAttackableTarget::EntityAINearestAttackableTarget(EntityCreature *creature, std::type_index classTarget,
                                                                 int32_t chance, bool checkSight, bool onlyNearby,
                                                                 std::function<bool(EntityLivingBase *)> targetSelector)
    : EntityAITarget(creature, checkSight, onlyNearby), targetClass(classTarget), targetChance(chance),
      sorter(EntityAINearestAttackableTarget::Sorter(creature))
{
    setMutexBits(1);
