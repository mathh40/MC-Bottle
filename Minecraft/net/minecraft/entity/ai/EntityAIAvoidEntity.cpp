...
auto alwaysTrue = [](Entity* p_apply_1_)->bool
{
    return true;
};
...
{
    auto list = entity->world->getEntitiesWithinAABB(classToAvoid, entity->getEntityBoundingBox().grow(avoidDistance, 3.0, avoidDistance), {EntitySelectors::CAN_AI_TARGET, canBeSeenSelector, avoidTargetSelector});
    if (list.isEmpty()) 
    {
        return false;
    }
    else 
    {
        ...
        auto vec3d = RandomPositionGenerator::findRandomTargetBlockAwayFrom(entity, 16, 7, Vec3d(closestLivingEntity->posX, closestLivingEntity->posY, closestLivingEntity->posZ));
        if (!vec3d.has_value()) 
        {
            return false;
        }
        else if (closestLivingEntity->getDistanceSq(vec3d->getx(), vec3d->gety(), vec3d->getz()) < closestLivingEntity->getDistanceSq((Entity)entity)) 
        {
            return false;
        }
        else 
...
EntityAIAvoidEntity::EntityAIAvoidEntity(EntityCreature* entityIn, std::type_index classToAvoidIn,
                                         std::function<bool(Entity p_apply_1_*)> avoidTargetSelectorIn, float avoidDistanceIn, double farSpeedIn, double nearSpeedIn)
...
}),entity(entityIn),classToAvoid(classToAvoidIn),avoidTargetSelector(avoidTargetSelectorIn),avoidDistance(avoidDistanceIn)
,farSpeed(farSpeedIn),nearSpeed(nearSpeedIn),navigation(entityIn->getNavigator())
{
    setMutexBits(1);
}
