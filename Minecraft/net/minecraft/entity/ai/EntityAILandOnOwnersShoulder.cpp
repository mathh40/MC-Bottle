...
EntityAILandOnOwnersShoulder::EntityAILandOnOwnersShoulder(EntityShoulderRiding *entityIn) : entity(entityIn)
{
}
...
bool EntityAILandOnOwnersShoulder::shouldExecute()
{
    ...
    bool flag = entitylivingbase != nullptr && !((EntityPlayer *)entitylivingbase)->isSpectator() &&
...
void EntityAILandOnOwnersShoulder::updateTask()
{
    if (!isSittingOnShoulder && !entity->isSitting() && !entity->getLeashed() &&
        entity->getEntityBoundingBox().intersects(owner->getEntityBoundingBox()))
    {
