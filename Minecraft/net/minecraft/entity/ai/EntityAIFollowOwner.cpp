...
EntityAIFollowOwner::EntityAIFollowOwner(EntityTameable *tameableIn, double followSpeedIn, float minDistIn,
                                         float maxDistIn)
    : tameable(tameableIn), world(tameableIn->world), followSpeed(followSpeedIn),
      petPathfinder(tameableIn->getNavigator()), minDist(minDistIn), maxDist(maxDistIn)
{
    setMutexBits(3);
    if (!Util:: instanceof <PathNavigateGround>(tameableIn->getNavigator()) && !Util:: instanceof
        <PathNavigateFlying>(tameableIn->getNavigator()))
    {
...
void EntityAIFollowOwner::updateTask()
{
    ...
    {
        timeToRecalcPath = 10;
        if (!petPathfinder->tryMoveToEntityLiving(owner, followSpeed) && !tameable->getLeashed() &&
            !tameable->isRiding() && tameable->getDistanceSq(owner) >= 144.0)
        {
            ...
            for (auto l = 0; l <= 4; ++l)
            {
                for (auto i1 = 0; i1 <= 4; ++i1)
                {
                    ...
                    {
                        tameable->setLocationAndAngles(i + l + 0.5F, k, j + i1 + 0.5F, tameable->rotationYaw,
                                                       tameable->rotationPitch);
                        petPathfinder->clearPath();
                        return;
                    }
                }
...
bool EntityAIFollowOwner::isTeleportFriendlyBlock(int32_t x, int32_t z, int32_t y, int32_t xOffset, int32_t zOffset)
{
    ...
    return iblockstate->getBlockFaceShape(world, blockpos, EnumFacing::DOWN) == BlockFaceShape::SOLID &&
           iblockstate->canEntitySpawn(tameable) && world->isAirBlock(blockpos.up()) &&
