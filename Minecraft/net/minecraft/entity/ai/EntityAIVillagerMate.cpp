...
EntityAIVillagerMate::EntityAIVillagerMate(EntityVillager *villagerIn) : villager(villagerIn), world(villagerIn->world)
{
    setMutexBits(3);
}
...
bool EntityAIVillagerMate::shouldExecute()
{
    ...
    else
    {
        ...
        {
            Entity *entity = world->findNearestEntityWithinAABB(
                typeid(EntityVillager), villager->getEntityBoundingBox().grow(8.0, 3.0, 8.0), villager);
            if (entity == nullptr)
            {
                return false;
            }
...
{
    return matingTimeout >= 0 && checkSufficientDoorsPresentForNewVillager() && villager->getGrowingAge() == 0 &&
           villager->getIsWillingToMate(false);
}
