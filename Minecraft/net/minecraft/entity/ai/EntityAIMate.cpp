...
EntityAIMate::EntityAIMate(EntityAnimal *animal, double speedIn) : EntityAIMate(animal, speedIn, typeid(animal))
{
}
...
void EntityAIMate::spawnBaby()
{
    ...
    if (entityageable != nullptr)
    {
        ...
        for (auto i = 0; i < 7; ++i)
        {
            ...
            world->spawnParticle(EnumParticleTypes::HEART, animal->posX + d3, animal->posY + d4, animal->posZ + d5, d0,
                                 d1, d2);
        }
