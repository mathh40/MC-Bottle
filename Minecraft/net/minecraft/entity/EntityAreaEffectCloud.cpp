        ...
        if (flag) 
        {
            if (randombool(rand)) 
            {
                for(auto i = 0; i < 2; ++i) 
                {
                    ...
                    {
                        world->spawnAlwaysVisibleParticle(enumparticletypes.getParticleID(), posX + (double)f3, posY, posZ + (double)f4, 0.0, 0.0, 0.0, aint);
                    }
                }
            }
        }
        else 
        {
            ...
            for(auto k1 = 0; (float)k1 < f5; ++k1) 
            {
                ...
                if (enumparticletypes == EnumParticleTypes::SPELL_MOB) 
                {
                    ...
                    i1 = k >> 8 & 255;
                    auto j1 = k & 255;
                    world->spawnAlwaysVisibleParticle(EnumParticleTypes::SPELL_MOB.getParticleID(), posX + (double)f4, posY, posZ + (double)f9, (double)((float)l / 255.0F), (double)((float)i1 / 255.0F), (double)((float)j1 / 255.0F));
                }
                else 
                {
                    world->spawnAlwaysVisibleParticle(enumparticletypes.getParticleID(), posX + (double)f4, posY, posZ + (double)f9, (0.5 - MathHelper::nextDouble(rand)) * 0.15, 0.009999999776482582, (0.5 - MathHelper::nextDouble(rand)) * 0.15, aint);
                }
            }
        }
    }
                    ...
                    while(true) {
                        ...
                        while(var15.hasNext()) {
                            ...
                            if (potioneffect.getPotion().isInstant()) {
                                potioneffect.getPotion().affectEntity(this, getOwner(), entitylivingbase, potioneffect.getAmplifier(), 0.5);
                            } else {
