...
EntityThrowable::EntityThrowable(World *worldIn)
    :Entity(worldIn),xTile(-1),yTile(-1),zTile(-1){
    setSize(0.25F, 0.25F);
}

EntityThrowable::EntityThrowable(World *worldIn, double x, double y, double z)
    :EntityThrowable(worldIn){
    setPosition(x, y, z);
}

EntityThrowable::EntityThrowable(World *worldIn, EntityLivingBase *throwerIn)
    :EntityThrowable(worldIn, throwerIn->posX, throwerIn->posY + (double)throwerIn->getEyeHeight() - 0.10000000149011612, throwerIn->posZ){
}
...
void EntityThrowable::onUpdate() {
    lastTickPosX = posX;
      ...
      auto list = world->getEntitiesWithinAABBExcludingEntity(this, getEntityBoundingBox().expand(motionX, motionY, motionZ).grow(1.0));
      double d0 = 0.0;
      bool flag = false;
    ...
    for(auto entity1 : list){
      ...
      if (raytraceresult.has_value()) {
         if (raytraceresult->typeOfHit == TraceType::BLOCK && world->getBlockState(raytraceresult->getBlockPos())->getBlock() == Blocks::PORTAL) {
      ...
      for(rotationPitch = (float)(MathHelper::atan2(motionY, (double)f) * 57.29577951308232); rotationPitch - prevRotationPitch < -180.0F; prevRotationPitch -= 360.0F) {
      }
