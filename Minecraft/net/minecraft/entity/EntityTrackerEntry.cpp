...
EntityTrackerEntry::EntityTrackerEntry(Entity *entityIn, int32_t rangeIn, int32_t maxRangeIn, int32_t updateFrequencyIn,
    ...
    : trackedEntity(entityIn), range(rangeIn), maxRange(maxRangeIn), updateFrequency(updateFrequencyIn),
      sendVelocityUpdates(sendVelocityUpdatesIn), encodedPosX(EntityTracker::getPositionLong(entityIn->posX)),
...
void EntityTrackerEntry::updatePlayerList(const std::vector<EntityPlayerMP *> &players)
{
    ...
    if (!updatedPlayerVisibility ||
        trackedEntity->getDistanceSq(lastTrackedEntityPosX, lastTrackedEntityPosY, lastTrackedEntityPosZ) > 16.0)
    {
    ...
    if (updateCounter % updateFrequency == 0 || trackedEntity->isAirBorne || trackedEntity->getDataManager().isDirty())
    {
        ...
        if (trackedEntity->isRiding())
        {
            ...
            bool flag3 =
                MathHelper::abs(k1 - encodedRotationYaw) >= 1 || MathHelper::abs(l1 - encodedRotationPitch) >= 1;
            if (flag3)
            {
                sendPacketToTrackedPlayers(new SPacketEntity.S16PacketEntityLook(
                    trackedEntity->getEntityId(), (std::byte)k1, (std::byte)l1, trackedEntity->onGround));
        ...
        else
        {
            ...
            bool flag1 =
                MathHelper::abs(k2 - encodedRotationYaw) >= 1 || MathHelper::abs(i - encodedRotationPitch) >= 1;
            if (updateCounter > 0 || Util:: instanceof <EntityArrow>(trackedEntity))
            {
                if (j >= -32768L && j < 32768L && k >= -32768L && k < 32768L && l >= -32768L && l < 32768L &&
                    ticksSinceLastForcedTeleport <= 400 && !ridingEntity && onGround == trackedEntity->onGround)
                {
                    if ((!flag || !flag1) && !(Util:: instanceof <EntityArrow>(trackedEntity)))
                    {
                        if (flag)
                        {
                            packet1 = new SPacketEntity.S15PacketEntityRelMove(trackedEntity->getEntityId(), j, k, l,
                                                                               trackedEntity->onGround);
                        }
                        else if (flag1)
                        {
                            packet1 = new SPacketEntity.S16PacketEntityLook(trackedEntity->getEntityId(), (std::byte)k2,
                                                                            (std::byte)i, trackedEntity->onGround);
                        }
                    }
                    else
                    {
                        packet1 = new SPacketEntity.S17PacketEntityLookMove(trackedEntity->getEntityId(), j, k, l,
                                                                            (std::byte)k2, (std::byte)i,
                                                                            trackedEntity->onGround);
                    }
                }
                else
            ...
            if (Util:: instanceof <EntityLivingBase>(trackedEntity) &&
                                      ((EntityLivingBase *)trackedEntity)->isElytraFlying())
            {
                flag2 = true;
            }
            ...
            if (flag2 && updateCounter > 0)
            {
                ...
                if (d4 > 4.0E-4 || d4 > 0.0 && trackedEntity->motionX == 0.0 && trackedEntity->motionY == 0.0 &&
                                       trackedEntity->motionZ == 0.0)
                {
                    ...
                    sendPacketToTrackedPlayers(new SPacketEntityVelocity(
                        trackedEntity->getEntityId(), lastTrackedEntityMotionX, lastTrackedEntityMotionY, motionZ));
                }
            }
...
void EntityTrackerEntry::updatePlayerEntity(EntityPlayerMP *playerMP)
{
    if (playerMP != trackedEntity)
    {
        ...
        {
            if (!trackingPlayers.contains(playerMP) &&
                (isPlayerWatchingThisChunk(playerMP) || trackedEntity->forceSpawn))
            {
                ...
                if (!trackedEntity->getDataManager().isEmpty())
                {
                    playerMP->connection.sendPacket(
                        new SPacketEntityMetadata(trackedEntity->getEntityId(), trackedEntity->getDataManager(), true));
                }
                ...
                if (Util:: instanceof <EntityLivingBase>(trackedEntity))
                {
                    ...
                    {
                        playerMP->connection.sendPacket(
                            new SPacketEntityProperties(trackedEntity->getEntityId(), collection));
                    }
                ...
                {
                    playerMP->connection.sendPacket(
                        new SPacketEntityVelocity(trackedEntity->getEntityId(), trackedEntity->motionX,
                                                  trackedEntity->motionY, trackedEntity->motionZ));
                }
                ...
                if (Util:: instanceof <EntityLivingBase>(trackedEntity))
                {
                    ...
                    {
                        ItemStack itemstack =
                            ((EntityLivingBase *)trackedEntity)->getItemStackFromSlot(entityequipmentslot);
                        if (!itemstack.isEmpty())
                        {
                            playerMP->connection.sendPacket(new SPacketEntityEquipment(trackedEntity->getEntityId(),
                                                                                       entityequipmentslot, itemstack));
                        }
                    }
                }
                ...
                if (Util:: instanceof <EntityLivingBase>(trackedEntity))
                {
                    ...
                    {
                        playerMP->connection.sendPacket(
                            new SPacketEntityEffect(trackedEntity->getEntityId(), potioneffect));
                    }
                }
...
bool EntityTrackerEntry::isVisibleTo(EntityPlayerMP *playerMP) const
{
    ...
    return d0 >= (double)(-i) && d0 <= (double)i && d1 >= (double)(-i) && d1 <= (double)i &&
           trackedEntity->isSpectatedByPlayer(playerMP);
}
...
{
    return playerMP->getServerWorld().getPlayerChunkMap().isPlayerWatchingChunk(playerMP, trackedEntity->chunkCoordX,
                                                                                trackedEntity->chunkCoordZ);
}
...
Packet EntityTrackerEntry::createSpawnPacket()
{
    ...
    else if (trackedEntity instanceof EntityFishHook)
    {
        ...
        return new SPacketSpawnObject(trackedEntity, 90,
                                      entity2 == null ? trackedEntity->getEntityId() : entity2.getEntityId());
    }
    else
    {
        Entity entity;
        if (trackedEntity instanceof EntitySpectralArrow)
        {
            ...
            return new SPacketSpawnObject(trackedEntity, 91,
                                          1 + (entity == null ? trackedEntity->getEntityId() : entity.getEntityId()));
        }
        else if (trackedEntity instanceof EntityTippedArrow)
        {
            entity = ((EntityArrow)trackedEntity).shootingEntity;
            return new SPacketSpawnObject(trackedEntity, 60,
                                          1 + (entity == null ? trackedEntity->getEntityId() : entity.getEntityId()));
        }
        ...
        else if (trackedEntity instanceof EntityFireball)
        {
            ...
            if (entityfireball.shootingEntity != null)
            {
                spacketspawnobject = new SPacketSpawnObject(
                    trackedEntity, i, ((EntityFireball)trackedEntity).shootingEntity.getEntityId());
            }
            else
            {
        ...
        else if (trackedEntity instanceof EntityItemFrame)
        {
            ...
            return new SPacketSpawnObject(trackedEntity, 71, entityitemframe.facingDirection.getHorizontalIndex(),
