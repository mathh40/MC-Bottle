...
EntityLightningBolt::EntityLightningBolt(World *worldIn, double x, double y, double z, bool effectOnlyIn)
    :EntityWeatherEffect(worldIn),lightningState(2),boltVertex(MathHelper::nextLong(rand)),boltLivingTime(rand(3) + 1)
...
void EntityLightningBolt::onUpdate() {
    EntityWeatherEffect::onUpdate();
      if (lightningState == 2) {
         world->playSound(nullptr, posX, posY, posZ, SoundEvents::ENTITY_LIGHTNING_THUNDER, SoundCategory::WEATHER, 10000.0F, 0.8F + MathHelper::nextFloat(rand) * 0.2F);
         world->playSound(nullptr, posX, posY, posZ, SoundEvents::ENTITY_LIGHTNING_IMPACT, SoundCategory::WEATHER, 2.0F, 0.5F + MathHelper::nextFloat(rand) * 0.2F);
      }
      ...
      if (lightningState >= 0) {
         if (world->isRemote) {
            ...
            auto list = world->getEntitiesWithinAABBExcludingEntity(this, AxisAlignedBB(posX - 3.0, posY - 3.0, posZ - 3.0, posX + 3.0, posY + 6.0 + 3.0, posZ + 3.0));
