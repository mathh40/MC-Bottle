...
EntityAgeable::EntityAgeable(World *worldIn) : EntityCreature(worldIn)
{
}
        ...
        if (forcedAgeTimer > 0)
        {
            if (forcedAgeTimer % 4 == 0)
            {
                world->spawnParticle(EnumParticleTypes::VILLAGER_HAPPY,
                                     ...
                                     posY + 0.5 + (double)(MathHelper::nextFloat(rand) * this.height),
                                     this.posZ + (double)(MathHelper::nextFloat(rand) * width * 2.0F) - (double)width,
                                     0.0, 0.0, 0.0);
            }
