...
EntityFlying::EntityFlying(World *worldIn) 
...
void EntityFlying::travel(float strafe, float vertical, float forward) {
    if (isInWater()) {
         ...
         float f = 0.91F;
         if (onGround) {
            f = world->getBlockState(BlockPos(MathHelper::floor(posX), MathHelper::floor(getEntityBoundingBox().getminY()) - 1, MathHelper::floor(posZ)))->getBlock()->slipperiness * 0.91F;
         }
