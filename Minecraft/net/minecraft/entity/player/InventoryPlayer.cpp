...
InventoryPlayer::InventoryPlayer(EntityPlayer *playerIn) :
    ...
    allInventories({&mainInventory,&armorInventory,&offHandInventory}),
    itemStack(ItemStack::EMPTY), player(playerIn) {
}
...
int32_t InventoryPlayer::clearMatchingItems(Item *itemIn, int32_t metadataIn, int32_t removeCount,
    ...
    auto i = 0;
      for(auto l = 0; l < getSizeInventory(); ++l) {
         ...
         if (!itemstack.isEmpty() && (itemIn == nullptr || itemstack.getItem() == itemIn) && (metadataIn <= -1 || itemstack.getMetadata() == metadataIn) && (itemNBT == nullptr || NBTUtil::areNBTEquals(itemNBT, itemstack.getTagCompound(), true))) {
...
ItemStack InventoryPlayer::decrStackSize(int32_t index, int32_t count) {
    std::vector<ItemStack>* list = nullptr;
      ...
      return list != nullptr && !((*list)[index]).isEmpty() ? ItemStackHelper::getAndSplit(*list, index, count) : ItemStack::EMPTY;
}
