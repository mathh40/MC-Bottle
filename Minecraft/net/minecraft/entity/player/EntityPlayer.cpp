...
DataParameter EntityPlayer::LEFT_SHOULDER_ENTITY = EntityDataManager.createKey(EntityPlayer.class, DataSerializers.COMPOUND_TAG);
DataParameter EntityPlayer::RIGHT_SHOULDER_ENTITY = EntityDataManager.createKey(EntityPlayer.class, DataSerializers.COMPOUND_TAG);
    ...
    NBTTagCompound* process(IDataFixer* fixer, NBTTagCompound* compound, int32_t versionIn) {
            ...
            if (compound->hasKey("ShoulderEntityLeft", 10)) {
               compound->setTag("ShoulderEntityLeft",
                                fixer->process(FixTypes::ENTITY, compound->getCompoundTag("ShoulderEntityLeft"),
                                              versionIn));
            }

            if (compound->hasKey("ShoulderEntityRight", 10)) {
               compound->setTag("ShoulderEntityRight",
                                fixer->process(FixTypes::ENTITY, compound->getCompoundTag("ShoulderEntityRight"),
                                              versionIn));
            }
...
EntityPlayer::SleepEnemyPredicate::SleepEnemyPredicate(EntityPlayer *playerIn)
    :player(playerIn){
...
EntityPlayer::EntityPlayer(World *worldIn, const GameProfile &gameProfileIn)
    :EntityLivingBase(worldIn),inventory(this),itemStackMainHand(ItemStack::EMPTY),cooldownTracker(createCooldownTracker()),gameProfile(gameProfileIn)
    ,inventoryContainer(std::make_unique<ContainerPlayer>(&inventory, !worldIn->isRemote, this)),openContainer(inventoryContainer.get()){
    ...
    if (flyToggleTimer > 0) {
      ...
      if (getHealth() > 0.0F && !isSpectator()) {
         ...
         if (isRiding() && !getRidingEntity()->isDead) {
            axisalignedbb = getEntityBoundingBox().aunion(getRidingEntity()->getEntityBoundingBox()).grow(1.0, 0.0, 1.0);
         } else {
...
EntityItem * EntityPlayer::dropItem(ItemStack droppedItem, bool dropAround, bool traceItem) {
    if (droppedItem.isEmpty()) {
         ...
         if (dropAround) {
            ...
            f = 0.3F;
            entityitem->motionX = (double)(-MathHelper::sin(rotationYaw * 0.017453292F) * MathHelper::cos(rotationPitch * 0.017453292F) * f);
            entityitem->motionZ = (double)(MathHelper::cos(rotationYaw * 0.017453292F) * MathHelper::cos(rotationPitch * 0.017453292F) * f);
...
void EntityPlayer::readEntityFromNBT(NBTTagCompound *compound) {
    EntityLivingBase::readEntityFromNBT(compound);
      ...
      if (compound->hasKey("SpawnX", 99) && compound->hasKey("SpawnY", 99) && compound->hasKey("SpawnZ", 99)) {
         spawnPos = BlockPos(compound->getInteger("SpawnX"), compound->getInteger("SpawnY"), compound->getInteger("SpawnZ"));
...
EnumActionResult EntityPlayer::interactOn(Entity *entityToInteractOn, EnumHand hand) {
    if (isSpectator()) {
         ...
         if (entityToInteractOn->processInitialInteract(this, hand)) {
            if (capabilities.isCreativeMode && itemstack == getHeldItem(hand) && itemstack.getCount() < itemstack1.getCount()) {
...
void EntityPlayer::attackTargetEntityWithCurrentItem(Entity *targetEntity) {
    if (targetEntity->canBeAttackedWithItem() && !targetEntity->hitByEntity(this)) {
         ...
         if (f > 0.0F || f1 > 0.0F) {
            ...
            if (isSprinting() && flag) {
               world->playSound(nullptr, posX, posY, posZ, SoundEvents::ENTITY_PLAYER_ATTACK_KNOCKBACK, getSoundCategory(), 1.0F, 1.0F);
               ++i;
               flag1 = true;
            }

            bool flag2 = flag && fallDistance > 0.0F && !onGround && !isOnLadder() && !isInWater() && !isPotionActive(MobEffects::BLINDNESS) && !isRiding() && Util::instanceof<EntityLivingBase>(targetEntity);
            ...
            if (flag5) {
               if (i > 0) {
                  if (Util::instanceof<EntityLivingBase>(targetEntity)) {
                     ...
                     targetEntity->addVelocity((double)(-MathHelper::sin(rotationYaw * 0.017453292F) * (float)i * 0.5F), 0.1, (double)(MathHelper::cos(rotationYaw * 0.017453292F) * (float)i * 0.5F));
                  }
               ...
               if (flag3) {
                  ...
                  auto var23 = world->getEntitiesWithinAABB<EntityLivingBase>(targetEntity->getEntityBoundingBox().grow(1.0, 0.25, 1.0));

                   for(auto entitylivingbase : var23){
                     if (entitylivingbase != this && entitylivingbase != targetEntity && !isOnSameTeam(entitylivingbase) && getDistanceSq(entitylivingbase) < 9.0) {
                  ...
                  world->playSound(nullptr, posX, posY, posZ, SoundEvents::ENTITY_PLAYER_ATTACK_SWEEP, getSoundCategory(), 1.0F, 1.0F);
                  spawnSweepParticles();
               }
               ...
               if (flag2) {
                  world->playSound(nullptr, posX, posY, posZ, SoundEvents::ENTITY_PLAYER_ATTACK_CRIT, getSoundCategory(), 1.0F, 1.0F);
                  onCriticalHit(targetEntity);
               }

               if (!flag2 && !flag3) {
                  if (flag) {
                     world->playSound(nullptr, posX, posY, posZ, SoundEvents::ENTITY_PLAYER_ATTACK_STRONG, getSoundCategory(), 1.0F, 1.0F);
                  } else {
                     world->playSound(nullptr, posX, posY, posZ, SoundEvents::ENTITY_PLAYER_ATTACK_WEAK, getSoundCategory(), 1.0F, 1.0F);
                  }
               }
               ...
               if (Util::instanceof<EntityLivingBase>(targetEntity)) {
                  ...
                  if (Util::instanceof<WorldServer>(world) && f5 > 2.0F) {
                     ...
                     ((WorldServer*)world)->spawnParticle(EnumParticleTypes::DAMAGE_INDICATOR, targetEntity->posX, targetEntity->posY + (double)(targetEntity->height * 0.5F), targetEntity->posZ, k, 0.1, 0.0, 0.1, 0.2,{});
                  }
               }

               addExhaustion(0.1F);
            } else {
               world->playSound(nullptr, posX, posY, posZ, SoundEvents::ENTITY_PLAYER_ATTACK_NODAMAGE, getSoundCategory(), 1.0F, 1.0F);
               if (flag4) {
...
void EntityPlayer::spawnSweepParticles() const {
    ...
    if (Util::instanceof<WorldServer>(world)) {
        ((WorldServer*)world)->spawnParticle(EnumParticleTypes::SWEEP_ATTACK, posX + d0, posY + (double)height * 0.5, posZ + d1, 0, d0, 0.0, d1, 0.0,{});
    }
}
...
EntityPlayer::SleepResult EntityPlayer::trySleep(const BlockPos &bedLocationin) {
    EnumFacing enumfacing = world->getBlockState(bedLocationin)->getValue(BlockHorizontal::FACING);
      if (!world->isRemote) {
         ...
         double d0 = 8.0;
         double d1 = 5.0;
         auto list = world->getEntitiesWithinAABB<EntityMob>(AxisAlignedBB((double)bedLocationin.getx() - 8.0, (double)bedLocationin.gety() - 5.0, (double)bedLocationin.getz() - 8.0, (double)bedLocationin.getx() + 8.0, (double)bedLocationin.gety() + 5.0, (double)bedLocationin.getz() + 8.0), EntityPlayer::SleepEnemyPredicate(this));
         if (!list.empty()) {
      ...
      if (world->isBlockLoaded(bedLocationin)) {
         ...
         setPosition((double)((float)bedLocationin.getx() + f1), (double)((float)bedLocationin.gety() + 0.6875F), (double)((float)bedLocationin.getz() + f));
      } else {
         setPosition((double)((float)bedLocationin.getx() + 0.5F), (double)((float)bedLocationin.gety() + 0.6875F), (double)((float)bedLocationin.getz() + 0.5F));
      }
...
float EntityPlayer::getBedOrientationInDegrees() {
    if (bedLocation.has_value()) {
         EnumFacing enumfacing = (EnumFacing)world->getBlockState(bedLocation.value())->getValue(BlockHorizontal::FACING);
         switch(enumfacing) {
...
void EntityPlayer::addMovementStat(double p_71000_1_, double p_71000_3_, double p_71000_5_) {
    if (!isRiding()) {
         ...
         if (isInsideOfMaterial(Material::WATER)) {
            k = MathHelper::round(MathHelper::sqrt(p_71000_1_ * p_71000_1_ + p_71000_3_ * p_71000_3_ + p_71000_5_ * p_71000_5_) * 100.0F);
            if (k > 0) {
...
void EntityPlayer::onKillEntity(EntityLivingBase *entityLivingIn) {
    EntityList::EntityEggInfo entitylist$entityegginfo = (EntityList::EntityEggInfo)EntityList::ENTITY_EGGS.at(EntityList::getKey((Entity*)entityLivingIn));
...
void EntityPlayer::addExperienceLevel(int32_t levels) {
    experienceLevel += levels;
      ...
      if (levels > 0 && experienceLevel % 5 == 0 && (float)lastXPSound < (float)ticksExisted - 100.0F) {
         ...
         world->playSound(nullptr, posX, posY, posZ, SoundEvents::ENTITY_PLAYER_LEVELUP, getSoundCategory(), f * 0.75F, 1.0F);
...
ItemStack EntityPlayer::getItemStackFromSlot(EntityEquipmentSlot slotIn) {
    if (slotIn == EntityEquipmentSlot::MAINHAND) {
         ...
         return slotIn.getSlotType() == EntityEquipmentSlot::Type::ARMOR ? (ItemStack)inventory.armorInventory[slotIn.getIndex()] : ItemStack::EMPTY;
      }
}
...
bool EntityPlayer::isInvisibleToPlayer(EntityPlayer *player) {
    if (!isInvisible()) {
         ...
         Team* team = getTeam();
         return team == nullptr || player == nullptr || player->getTeam() != team || !team->getSeeFriendlyInvisiblesEnabled();
      }
}
...
bool EntityPlayer::canOpen(LockCode code) {
    if (code.isEmpty()) {
         ...
         ItemStack itemstack = getHeldItemMainhand();
         return !itemstack.isEmpty() && itemstack.hasDisplayName() ? itemstack.getDisplayName() == (code.getLock()) : false;
      }
}
    ...
    if (inventorySlot >= 0 && inventorySlot < inventory.mainInventory.size()) {
         ...
         if (inventorySlot == 98) {
            ...
            if (!itemStackIn.isEmpty()) {
               if (!(Util::instanceof<ItemArmor>(itemStackIn.getItem())) && !(Util::instanceof<ItemElytra>(itemStackIn.getItem()))) {
...
void EntityPlayer::updateSize() {
    float f;
      ...
      if (f != width || f1 != height) {
         ...
         axisalignedbb = AxisAlignedBB(axisalignedbb.getminX(), axisalignedbb.getminY(), axisalignedbb.getminZ(), axisalignedbb.getminX() + (double)f, axisalignedbb.getminY() + (double)f1, axisalignedbb.getminZ() + (double)f);
...
bool EntityPlayer::bedInRange(const BlockPos &p_190774_1_, EnumFacing p_190774_2_) const {
    if (MathHelper::abs(posX - (double)p_190774_1_.getx()) <= 3.0 && MathHelper::abs(posY - (double)p_190774_1_.gety()) <= 2.0 && MathHelper::abs(posZ - (double)p_190774_1_.getz()) <= 3.0) {
         return true;
      } else {
         BlockPos blockpos = p_190774_1_.offset(p_190774_2_.getOpposite());
         return MathHelper::abs(posX - (double)blockpos.getx()) <= 3.0 && MathHelper::abs(posY - (double)blockpos.gety()) <= 2.0 && MathHelper::abs(posZ - (double)blockpos.getz()) <= 3.0;
      }
}
...
void EntityPlayer::addMountedMovementStat(double p_71015_1_, double p_71015_3_, double p_71015_5_) {
    if (isRiding()) {
         int i = MathHelper::round(MathHelper::sqrt(p_71015_1_ * p_71015_1_ + p_71015_3_ * p_71015_3_ + p_71015_5_ * p_71015_5_) * 100.0F);
         if (i > 0) {
