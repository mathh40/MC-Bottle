...
EntityLiving::EntityLiving(World *worldIn)
    : EntityLivingBase(worldIn), inventoryHands(2, ItemStack::EMPTY), inventoryHandsDropChances(2, 0.085F),
      inventoryArmor(4, ItemStack::EMPTY), inventoryArmorDropChances(4, 0.085F),
      moveHelper(std::make_unique<EntityMoveHelper>(this)), jumpHelper(std::make_unique<EntityJumpHelper>(this)),
      ...
      lookHelper(std::make_unique<EntityLookHelper>(this)), senses(this)
{
    ...
    if (world->isRemote)
    {
        for (auto i = 0; i < 20; ++i)
        {
            ...
            world->spawnParticle(
                ...
                posX + (double)(MathHelper::nextFloat(rand) * width * 2.0F) - (double)width - d0 * 10.0,
                posY + (double)(MathHelper::nextFloat(rand) * height) - d1 * 10.0,
                posZ + (double)(MathHelper::nextFloat(rand) * width * 2.0F) - (double)width - d2 * 10.0, d0, d1, d2);
        }
    }
    else
    {
...
{
    fixer.registerWalker(FixTypes::ENTITY, new ItemStackDataLists(name, std::string[]{"ArmorItems", "HandItems"}));
}
...
void EntityLiving::writeEntityToNBT(NBTTagCompound *compound)
{
    ...
    for (auto var3 = inventoryArmor.begin(); var3 != inventoryArmor.end();
         nbttaglist->appendTag(nbttagcompound), ++var3)
    {
    ...
    for (auto var3 = inventoryHands.begin(); var3 != inventoryHands.end();
         nbttaglist->appendTag(nbttagcompound), ++var3)
    {
...
void EntityLiving::faceEntity(Entity *entityIn, float maxYawIncrease, float maxPitchIncrease)
{
    ...
    {
        d1 = (entityIn->getEntityBoundingBox().getminY() + entityIn->getEntityBoundingBox().getmaxY()) / 2.0 -
...
{
    return !world->containsAnyLiquid(getEntityBoundingBox()) &&
...
IEntityLivingData *EntityLiving::onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData *livingdata)
{
    getEntityAttribute(SharedMonsterAttributes::FOLLOW_RANGE)
        .applyModifier(AttributeModifier("Random spawn bonus", MathHelper::nextGaussian<double>(rand) * 0.05, 1));
...
void EntityLiving::setEnchantmentBasedOnDifficulty(DifficultyInstance difficulty)
{
    ...
    if (!getHeldItemMainhand().isEmpty() && MathHelper::nextFloat(rand) < 0.25F * f)
    {
        setItemStackToSlot(EntityEquipmentSlot::MAINHAND,
                           EnchantmentHelper::addRandomEnchantment(rand, getHeldItemMainhand(),
                                                                   (int)(5.0F + f * (float)rand(18)), false));
    }
    ...
    for (auto entityequipmentslot : EntityEquipmentSlot::values())
    {
        if (entityequipmentslot->getSlotType() == EntityEquipmentSlot::Type::ARMOR)
        {
            ...
            {
                setItemStackToSlot(entityequipmentslot, EnchantmentHelper::addRandomEnchantment(
                                                            rand, itemstack, (int)(5.0F + f * (float)rand(18)), false));
            }
        }
    }
}
...
void EntityLiving::entityInit()
{
    ...
    dataManager.registe(AI_FLAGS, 0);
}
...
void EntityLiving::dropLoot(bool wasRecentlyHit, int32_t lootingModifier, DamageSource::DamageSource source)
{
    ...
    if (resourcelocation)
    {
        ...
        LootContext::Builder lootcontext$builder =
            (LootContext::Builder((WorldServer *)world))->withLootedEntity(this).withDamageSource(source);
        ...
        Iterator var7 = loottable
                            .generateLootForPools(deathLootTableSeed == 0L ? rand : pcg32(deathLootTableSeed),
...
void EntityLiving::updateEquipmentIfNeeded(EntityItem *itemEntity)
{
    ...
    if (!itemstack1.isEmpty())
    {
        ...
        else if (itemstack.getItem() instanceof ItemArmor && itemstack1.getItem() instanceof
                 ItemArmor && !EnchantmentHelper.hasBindingCurse(itemstack1))
        {
...
void EntityLiving::dropEquipment(bool wasRecentlyHit, int32_t lootingModifier)
{
    ...
    for (auto entityequipmentslot : var3)
    {
        ...
        bool flag = d0 > 1.0;
        if (!itemstack.isEmpty() && !EnchantmentHelper::hasVanishingCurse(itemstack) && (wasRecentlyHit || flag) &&
            (double)(MathHelper::nextFloat(rand) - (float)lootingModifier * 0.01F) < d0)
        {
            if (!flag && itemstack.isItemStackDamageable())
            {
                itemstack.setItemDamage(itemstack.getMaxDamage() -
                                        rand(1 + rand(MathHelper::max(itemstack.getMaxDamage() - 3, 1))));
            }
...
void EntityLiving::recreateLeash()
{
    if (isLeashed && leashNBTTag != nullptr)
    {
        ...
        {
            BlockPos blockpos =
                BlockPos(leashNBTTag->getInteger("X"), leashNBTTag->getInteger("Y"), leashNBTTag->getInteger("Z"));
