...
private:
    ...
    template<class T>
    static void registers(int32_t id, std::string_view name, std::string_view oldName);
};

template <class T>
void EntityList::registers(int32_t id, std::string_view name, std::string_view oldName) {
        auto clazz =[](World* world)
        {
            return new T(world);
        };
