...
EntityDragon::EntityDragon(World *worldIn)
    :EntityLiving(worldIn)
    ,growlTime(100),phaseManager(this){
    ...
    float f11;
      ...
      if (world->isRemote) {
         ...
         if (!isSilent()) {
            ...
            if (f1 <= -0.3F && f11 >= -0.3F) {
               world->playSound(posX, posY, posZ, SoundEvents::ENTITY_ENDERDRAGON_FLAP, getSoundCategory(), 5.0F, 0.8F + MathHelper::nextFloat(rand) * 0.3F, false);
            }

            if (!phaseManager.getCurrentPhase().getIsStationary() && --growlTime < 0) {
               world->playSound(posX, posY, posZ, SoundEvents::ENTITY_ENDERDRAGON_GROWL, getSoundCategory(), 2.5F, 0.8F + MathHelper::nextFloat(rand) * 0.3F, false);
      ...
      if (getHealth() <= 0.0F) {
         ...
         world->spawnParticle(EnumParticleTypes::EXPLOSION_LARGE, posX + (double)f11, posY + 2.0 + (double)f1, posZ + (double)f14, 0.0, 0.0, 0.0,{});
      } else {
         ...
         if (isAIDisabled()) {
            ...
            if (world->isRemote) {
               ...
               if (vec3d != nullptr) {
                  ...
                  rotationYaw = MathHelper::wrapDegrees(rotationYaw);
                  double d4 = MathHelper::clamp(MathHelper::wrapDegrees(180.0 - MathHelper::atan2(d6, d8) * 57.29577951308232 - (double)rotationYaw), -50.0, 50.0);
                  Vec3d vec3d1 = (Vec3d(vec3d.getx() - posX, vec3d.gety() - posY, vec3d.getz() - posZ)).normalize();
                  Vec3d vec3d2 = (Vec3d((double)MathHelper::sin(rotationYaw * 0.017453292F), motionY, (double)(-MathHelper::cos(rotationYaw * 0.017453292F)))).normalize();
                  ...
                  if (slowed) {
                     move(MoverType::SELF, motionX * 0.800000011920929, motionY * 0.800000011920929, motionZ * 0.800000011920929);
                  } else {
           ...
           std::array<Vec3d,8> avec3d;
            ...
            dragonPartWing1.setLocationAndAngles(posX + (double)(f18 * 4.5F), posY + 2.0, posZ + (double)(f3 * 4.5F), 0.0F, 0.0F);
            dragonPartWing2.onUpdate();
            dragonPartWing2.setLocationAndAngles(posX - (double)(f18 * 4.5F), posY + 2.0, posZ - (double)(f3 * 4.5F), 0.0F, 0.0F);
            if (!world->isRemote && hurtTime == 0) {
               collideWithEntities(world->getEntitiesWithinAABBExcludingEntity(this, dragonPartWing1.getEntityBoundingBox().grow(4.0, 2.0, 4.0).offset(0.0, -2.0, 0.0)));
               collideWithEntities(world->getEntitiesWithinAABBExcludingEntity(this, dragonPartWing2.getEntityBoundingBox().grow(4.0, 2.0, 4.0).offset(0.0, -2.0, 0.0)));
               attackEntitiesInList(world->getEntitiesWithinAABBExcludingEntity(this, dragonPartHead.getEntityBoundingBox().grow(1.0)));
               attackEntitiesInList(world->getEntitiesWithinAABBExcludingEntity(this, dragonPartNeck.getEntityBoundingBox().grow(1.0)));
            }
            ...
            for(k = 0; k < 3; ++k) {
               ...
               multipartentitypart->setLocationAndAngles(posX - (double)((f3 * 1.5F + f6 * f24) * f16), posY + (adouble1[1] - adouble[1]) - (double)((f24 + 1.5F) * f2) + 1.5, posZ + (double)((f18 * 1.5F + f22 * f24) * f16), 0.0F, 0.0F);
            }
...
bool EntityDragon::attackEntityFrom(DamageSource::DamageSource source, float amount) {
    if (Util::instanceof<DamageSource::EntityDamageSource>(source) && ((DamageSource::EntityDamageSource)source).getIsThornsDamage()) {
...
int32_t EntityDragon::initPathPoints() {
    if (pathPoints[0] == std::nullopt) {
         for(int i = 0; i < 24; ++i) {
            ...
            lvt_3_1_ = MathHelper::max(world->getSeaLevel() + 10, world->getTopSolidOrLiquidBlock(BlockPos(l, 0, i1)).gety() + j);
...
void EntityDragon::onCrystalDestroyed(EntityEnderCrystal *crystal, const BlockPos &pos,
    ...
    EntityPlayer* entityplayer;
      ...
      if (crystal == healingEnderCrystal) {
         attackEntityFromPart(dragonPartHead, DamageSource::causeExplosionDamage((EntityLivingBase*)entityplayer), 10.0F);
      }
...
void EntityDragon::onDeathUpdate() {
    if (fightManager.has_value()) {
      ...
      if (deathTicks >= 180 && deathTicks <= 200) {
         ...
         world->spawnParticle(EnumParticleTypes::EXPLOSION_HUGE, posX + (double)f, posY + 2.0 + (double)f1, posZ + (double)f2, 0.0, 0.0, 0.0,{});
      }
...
void EntityDragon::collideWithEntities(const std::vector<Entity *> &p_70970_1_) {
    double d0 = (dragonPartBody.getEntityBoundingBox().getminX() + dragonPartBody.getEntityBoundingBox().getmaxX()) / 2.0;
      double d1 = (dragonPartBody.getEntityBoundingBox().getminZ() + dragonPartBody.getEntityBoundingBox().getmaxZ()) / 2.0;
