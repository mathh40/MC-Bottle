...
DataParameter EntityWither::FIRST_HEAD_TARGET =
    EntityDataManager::createKey(EntityWither.class, DataSerializers.VARINT);
DataParameter EntityWither::SECOND_HEAD_TARGET =
    EntityDataManager::createKey(EntityWither.class, DataSerializers.VARINT);
DataParameter EntityWither::THIRD_HEAD_TARGET =
    EntityDataManager::createKey(EntityWither.class, DataSerializers.VARINT);
std::array<DataParameter, 3> EntityWither::HEAD_TARGETS = {FIRST_HEAD_TARGET, SECOND_HEAD_TARGET, THIRD_HEAD_TARGET};
DataParameter EntityWither::INVULNERABILITY_TIME =
    EntityDataManager.createKey(EntityWither.class, DataSerializers.VARINT);
...
void EntityWither::onLivingUpdate()
{
    ...
    for (i1 = 0; i1 < 3; ++i1)
    {
        ...
        world->spawnParticle(EnumParticleTypes::SMOKE_NORMAL,
                             d10 + MathHelper::nextGaussian<double>(rand) * 0.30000001192092896,
                             d2 + MathHelper::nextGaussian<double>(rand) * 0.30000001192092896,
                             d4 + MathHelper::nextGaussian<double>(rand) * 0.30000001192092896, 0.0, 0.0, 0.0, {});
        if (flag && world->rand(4) == 0)
        {
            world->spawnParticle(EnumParticleTypes::SPELL_MOB,
                                 d10 + MathHelper::nextGaussian<double>(rand) * 0.30000001192092896,
                                 d2 + MathHelper::nextGaussian<double>(rand) * 0.30000001192092896,
                                 d4 + MathHelper::nextGaussian<double>(rand) * 0.30000001192092896, 0.699999988079071,
    ...
    if (getInvulTime() > 0)
    {
        for (i1 = 0; i1 < 3; ++i1)
        {
            world->spawnParticle(EnumParticleTypes::SPELL_MOB, posX + MathHelper::nextGaussian<double>(rand),
                                 posY + (double)(MathHelper::nextFloat(rand) * 3.3F),
                                 posZ + MathHelper::nextGaussian<double>(rand), 0.699999988079071, 0.699999988079071,
                                 0.8999999761581421, {});
        }
    }
}
...
bool EntityWither::canDestroyBlock(Block *blockIn)
{
    return blockIn != Blocks::BEDROCK && blockIn != Blocks::END_PORTAL && blockIn != Blocks::END_PORTAL_FRAME &&
           ...
           blockIn != Blocks::CHAIN_COMMAND_BLOCK && blockIn != Blocks::BARRIER && blockIn != Blocks::STRUCTURE_BLOCK &&
           blockIn != Blocks::STRUCTURE_VOID && blockIn != Blocks::PISTON_EXTENSION && blockIn != Blocks::END_GATEWAY;
}
...
bool EntityWither::attackEntityFrom(DamageSource::DamageSource source, float amount)
{
    ...
    else if (source != DamageSource::DROWN && !(Util:: instanceof <EntityWither>(source.getTrueSource())))
    {
        ...
        else
        {
            ...
            if (entity1 != nullptr && !(Util:: instanceof <EntityPlayer>(entity1)) && Util:: instanceof
...
void EntityWither::updateAITasks()
{
    ...
    if (getInvulTime() > 0)
    {
        ...
        if (i <= 0)
        {
            world->newExplosion(this, posX, posY + (double)getEyeHeight(), posZ, 7.0F, false,
        ...
        for (i = 1; i < 3; ++i)
        {
            if (ticksExisted >= nextHeadUpdate[i - 1])
            {
                ...
                if (l1 > 0)
                {
                    ...
                    if (entity != nullptr && entity->isEntityAlive() && getDistanceSq(entity) <= 900.0 &&
                ...
                {
                    auto list = world->getEntitiesWithinAABB<EntityLivingBase>(
                        getEntityBoundingBox().grow(20.0, 8.0, 20.0),
                        [](const Entity *entity) { NOT_UNDEAD(entity) && EntitySelectors::NOT_SPECTATING(entity); });
...
{
    launchWitherSkullToCoords(p_82216_1_, p_82216_2_->posX, p_82216_2_->posY + (double)p_82216_2_->getEyeHeight() * 0.5,
                              p_82216_2_->posZ, p_82216_1_ == 0 && MathHelper::nextFloat(rand) < 0.001F);
}
