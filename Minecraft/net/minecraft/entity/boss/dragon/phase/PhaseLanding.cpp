...
PhaseLanding::PhaseLanding(EntityDragon *dragonIn) : PhaseBase(dragonIn)
{
}
...
void PhaseLanding::doClientRenderEffects()
{
    ...
    for (auto i = 0; i < 8; ++i)
    {
        ...
        dragon->world->spawnParticle(
            EnumParticleTypes::DRAGON_BREATH, d3, d4, d5, -vec3d.getx() * 0.07999999821186066 + dragon->motionX,
            -vec3d.gety() * 0.30000001192092896 + dragon->motionY, -vec3d.z * 0.07999999821186066 + dragon->motionZ);
...
void PhaseLanding::doLocalUpdate()
{
    ...
    {
        ((PhaseSittingFlaming *)dragon->getPhaseManager().getPhase(PhaseList::SITTING_FLAMING.get()))
            ->resetFlameCount();
