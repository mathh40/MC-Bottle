...
PhaseStrafePlayer::PhaseStrafePlayer(EntityDragon *dragonIn) : PhaseBase(dragonIn)
{
}
...
void PhaseStrafePlayer::doLocalUpdate()
{
    ...
    else
    {
        ...
        d12 = targetLocation == std::nullopt
                  ? 0.0
        ...
        if (attackTarget->getDistanceSq(dragon) < 4096.0)
        {
            if (dragon->canEntityBeSeen(attackTarget))
            {
                ++fireballCharge;
                Vec3d vec3d1 =
                    (Vec3d(attackTarget->posX - dragon->posX, 0.0, attackTarget->posZ - dragon->posZ)).normalize();
                Vec3d vec3d = (Vec3d((double)MathHelper::sin(dragon->rotationYaw * 0.017453292F), 0.0,
                ...
                if (fireballCharge >= 5 && f >= 0.0F && f < 10.0F)
                {
                    ...
                    double d10   = attackTarget->posY + (double)(attackTarget->height / 2.0F) -
                                 (d7 + (double)(dragon->dragonPartHead.height / 2.0F));
                    ...
                    EntityDragonFireball *entitydragonfireball =
                        new EntityDragonFireball(dragon->world, dragon, d9, d10, d11);
