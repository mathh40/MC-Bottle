...
PhaseSittingAttacking::PhaseSittingAttacking(EntityDragon *dragonIn)
    :PhaseSittingBase(dragonIn){
}

void PhaseSittingAttacking::doClientRenderEffects() {
    dragon->world->playSound(dragon->posX, dragon->posY, dragon->posZ, SoundEvents::ENTITY_ENDERDRAGON_GROWL, dragon->getSoundCategory(), 2.5F, 0.8F + MathHelper::nextFloat(dragon->getRNG()) * 0.3F, false);
}
