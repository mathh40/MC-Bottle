...
PhaseSittingFlaming::PhaseSittingFlaming(EntityDragon *dragonIn) : PhaseSittingBase(dragonIn)
{
}
        ...
        for (int i = 0; i < 8; ++i)
        {
            ...
            for (int j = 0; j < 6; ++j)
            {
                dragon->world->spawnParticle(
                    EnumParticleTypes::DRAGON_BREATH, d3, d4, d5, -vec3d.getx() * 0.07999999821186066 * (double)j,
                    -vec3d.gety() * 0.6000000238418579, -vec3d.getz() * 0.07999999821186066 * (double)j, {});
            }
...
void PhaseSittingFlaming::doLocalUpdate()
{
    ...
    {
        Vec3d vec3d =
            (Vec3d(dragon->dragonPartHead.posX - dragon->posX, 0.0, dragon->dragonPartHead.posZ - dragon->posZ))
                .normalize();
        float f   = 5.0F;
        ...
        BlockPos blockpos$mutableblockpos =
            BlockPos(MathHelper::floor(d0), MathHelper::floor(d2), MathHelper::floor(d1));
