...
PhaseSittingScanning::PhaseSittingScanning(EntityDragon *dragonIn)
    ...
    ++scanningTime;
      ...
      if (entitylivingbase != nullptr) {
         if (scanningTime > 25) {
            ...
            Vec3d vec3d = Vec3d(entitylivingbase->posX - dragon->posX, 0.0, entitylivingbase->posZ - dragon->posZ).normalize();
            Vec3d vec3d1 = Vec3d((double)MathHelper::sin(dragon->rotationYaw * 0.017453292F), 0.0, (double)(-MathHelper::cos(dragon->rotationYaw * 0.017453292F))).normalize();
            ...
            if (f1 < 0.0F || f1 > 10.0F) {
               ...
               double d2 = MathHelper::clamp(MathHelper::wrapDegrees(180.0 - MathHelper::atan2(d0, d1) * 57.29577951308232 - (double)dragon->rotationYaw), -100.0, 100.0);
         ...
         if (entitylivingbase != nullptr) {
            ...
            ((PhaseChargingPlayer*)dragon->getPhaseManager().getPhase(PhaseList::CHARGING_PLAYER.get()))->setTarget(Vec3d(entitylivingbase->posX, entitylivingbase->posY, entitylivingbase->posZ));
         }
      }
}
