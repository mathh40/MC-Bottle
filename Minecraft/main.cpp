...
int main(int argc, char** argv)
{
	...
	args::ValueFlag<std::string> resourcePackDir(parser, "resourcePackDir", "Resource Pack Direction", { "resourcePackDir" });
	...
	args::ValueFlag<std::string> profileProperties(parser, "profileProperties", "profile Properties", { "profileProperties" },"{}");
	...
	GameConfiguration gameconfiguration = GameConfiguration(GameConfiguration::UserInformation(session, propertymap, propertymap1), GameConfiguration::DisplayInformation(width.Get(), height.Get(), fullscreen.Get(), checkGlErrors.Get()), GameConfiguration::FolderInformation(gameDir.Get(), resourcePackDir.Get(), assetsDir.Get(), assetIndex.Get()), GameConfiguration::GameInformation(demo, version.Get(), versionType.Get()), GameConfiguration::ServerInformation(server.Get(), port.Get()));
